# OpenAI-Compatible API Server for Cyberfox Agent
...
SSE streaming. By exposing this endpoint, cyberfox-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  cyberfox-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.cyberfox/.env`
2. Run `cyberfox gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with cyberfox-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns cyberfox-agent as a model) |
...
Authorization: Bearer cyberfox-api-key-here
...
But cyberfox-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "cyberfox-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured cyberfox-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of cyberfox-agent's core prompt
...
Once implemented, cyberfox-agent works as a drop-in backend for:
