# Cyberfox Agent Security Policy

This document describes Cyberfox Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/Sarthak5-t/Cyberfox/security/advisories/new)
or **security@sarthak5t.github.io**. Do not open public issues for
security vulnerabilities. **Cyberfox Agent does not operate a bug
bounty program.**
...
- Environment details (`cyberfox version`, commit SHA, OS, Python
  version).
...
Cyberfox Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Cyberfox Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Cyberfox Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Cyberfox Agent's documentation
...
Cyberfox Agent supports two OS-level isolation postures. They address
...
Cyberfox Agent supports this in two ways:

- **Cyberfox Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Cyberfox Agent's in-process heuristics
...
Cyberfox Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Cyberfox Agent
itself. Bugs in Cyberfox Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Cyberfox Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Cyberfox Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Cyberfox Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  cyberfox-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Cyberfox Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Cyberfox Agent. Bugs in Cyberfox
...
- Cyberfox Agent includes supply-chain guards for MCP server
...
Cyberfox Agent includes the Ares plugin for penetration testing and
red team operations. Users should:

- Only test systems they have authorization to test
- Follow responsible disclosure practices
- Comply with all applicable laws and regulations
- Use the agent's safety features (scope validation, approval gates)
