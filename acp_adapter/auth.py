"""ACP auth helpers — detect and advertise Cyberfox authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "cyberfox-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Cyberfox runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Cyberfox can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Cyberfox.
    ...
    not have Cyberfox provider credentials configured yet, so Cyberfox always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Cyberfox using the currently configured "
