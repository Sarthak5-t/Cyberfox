"""CLI entry point for the cyberfox-agent ACP adapter.

Loads environment variables from ``~/.cyberfox/.env``, configures logging
...
try:
    ...
    # shadowing Cyberfox's own modules — ``cyberfox acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    cyberfox_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from CYBERFOX_HOME (default ``~/.cyberfox``)."""
    from cyberfox_cli.env_loader import load_cyberfox_dotenv

    cyberfox_home = get_cyberfox_home()
    loaded = load_cyberfox_dotenv(cyberfox_home=cyberfox_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", cyberfox_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="cyberfox-acp",
        description="Run Cyberfox Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Cyberfox version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Cyberfox provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.cyberfox/node/ "
...
def _print_version() -> None:
    ...
    print(cyberfox_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import CyberfoxACPAgent  # noqa: F401

    print("Cyberfox ACP check OK")


def _run_setup() -> None:
    from cyberfox_cli.main import main as cyberfox_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "cyberfox", "model"]
        cyberfox_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``cyberfox postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting cyberfox-agent ACP adapter")
    ...
    import acp
    from .server import CyberfoxACPAgent
