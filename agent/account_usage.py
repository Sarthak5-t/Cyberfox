...
class CreditsView:
    ...
    Fail-open: when not logged in or the provider is unreachable,
    ``logged_in`` is False and callers degrade gracefully.
    """
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    Reports not logged in and returns no balance/top-up data. Fail-open →
