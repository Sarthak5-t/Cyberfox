...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://github.com/Sarthak5-t/Cyberfox",
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    — most commonly when a long-lived process pinned an OpenRouter model
    that has since been dropped from the catalog. OpenRouter returns 404 with
    a body like::
...
def call_llm(
    ...
    *,
    ...
    try:
        ...
        # When the resolved provider returns 401 and neither explicit provider
        # credential refresh applies, fall back to an alternative provider
        # instead of dropping the auxiliary task on the floor (silent compression
        # failure / message loss). Auth is NOT a capacity error: it only bypasses
