...
``kind: backend``) or ``~/.cyberfox/plugins/browser/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class BrowserProvider(abc.ABC):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by :mod:`cyberfox_cli.tools_config` to inject this provider as a
        ...
        expose API key prompts, badges, managed gating, and the
