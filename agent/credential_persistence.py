...
``auth.json``.  They intentionally have no dependency on ``cyberfox_cli.auth`` so
...
# Sources Cyberfox owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Cyberfox-owned OAuth/device-code state)
