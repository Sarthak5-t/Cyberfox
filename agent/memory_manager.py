...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_CYBERFOX_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Cyberfox expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``cyberfox_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_cyberfox_home()`` themselves.
        """
        if "cyberfox_home" not in kwargs:
            from cyberfox_constants import get_cyberfox_home
            kwargs["cyberfox_home"] = str(get_cyberfox_home())
        for provider in self._providers:
            try:
