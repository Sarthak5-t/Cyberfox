...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `cyberfox backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - cyberfox_home (str): The active CYBERFOX_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.cyberfox``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "cyberfox").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'cyberfox memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], cyberfox_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'cyberfox memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``cyberfox_home`` is the active CYBERFOX_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE CYBERFOX_HOME.

        ``cyberfox backup`` only walks CYBERFOX_HOME, so any provider state kept
        ...
        of the archive, and ``cyberfox import`` restores them to their original
