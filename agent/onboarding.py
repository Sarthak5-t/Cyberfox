...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Cyberfox starts and finds ``~/.openclaw/``.

    Points users at ``cyberfox claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``cyberfox claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Cyberfox, run "
        "`cyberfox claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `cyberfox claw cleanup` (renames it to ~/.openclaw.pre-migration — "
