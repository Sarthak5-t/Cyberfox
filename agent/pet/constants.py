...
row taxonomy is inferred from the atlas shape so Cyberfox can render both legacy
...
# rows (1536x1872), while older Cyberfox/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Cyberfox' activity state names. They are not always identical to the
...
# Legacy Cyberfox/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Cyberfox activity names -> accepted row-name aliases in descending
