...
Plugins built on Cyberfox Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Cyberfox
...
:class:`~cyberfox_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Cyberfox' main config
...
handles every provider, fallback chain, and per-task override Cyberfox
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`cyberfox_cli.plugins.PluginContext`
