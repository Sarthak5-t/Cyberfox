...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the CYBERFOX_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Cyberfox runtime / deployment
    "CYBERFOX_HOME", "CYBERFOX_PROFILE", "CYBERFOX_GATEWAY_LOCK_DIR",
    "CYBERFOX_MAX_ITERATIONS", "CYBERFOX_MAX_TOKENS", "CYBERFOX_API_TIMEOUT",
    "CYBERFOX_REDACT_SECRETS", "CYBERFOX_NOUS_TIMEOUT_SECONDS",
    "_CYBERFOX_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "CYBERFOX_KANBAN_DB", "CYBERFOX_KANBAN_WORKSPACES_ROOT", "CYBERFOX_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "CYBERFOX_KANBAN_",
    "CYBERFOX_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(cyberfox_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(cyberfox_home) / ".env")
