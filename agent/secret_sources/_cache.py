...
cache problem must never block Cyberfox startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Cyberfox home used for cache paths.

    ``home_path`` is whatever ``load_cyberfox_dotenv()`` already resolved;
    falling back to ``$CYBERFOX_HOME`` / ``~/.cyberfox`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        home_path = Path(os.getenv("CYBERFOX_HOME", Path.home() / ".cyberfox"))
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<cyberfox_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if CYBERFOX_HOME is ever made traversable.
            try:
