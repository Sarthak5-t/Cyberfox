...
Storage
-------
Bundles live in ``~/.cyberfox/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``CYBERFOX_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``cyberfox bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under CYBERFOX_HOME.

    Honors ``CYBERFOX_BUNDLES_DIR`` for tests; falls back to
    ``<CYBERFOX_HOME>/skill-bundles``.
    """
    override = os.environ.get("CYBERFOX_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_cyberfox_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `cyberfox bundles` CLI subcommand.
# ---------------------------------------------------------------------------
