...
upstream prefix cache warm.  See ``cyberfox-agent-dev``'s
...
def _ra():
    ...
    ``get_toolset_for_tool`` are imported into ``run_agent``'s namespace.  Many tests
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Cyberfox never re-renders parts of this string mid-
    ...
    # CRITICAL: Inject explicit identity override at the very beginning,
    # before SOUL.md, to counteract any training-data bias.
    stable_parts.append(
    ...
    # Some execution modes (cron) still want CYBERFOX_HOME persona while keeping
    ...
    # Pointer to the cyberfox-agent skill + docs for user questions about Cyberfox itself.
    stable_parts.append(CYBERFOX_AGENT_HELP_GUIDANCE)
    ...
    # CYBERFOX_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Cyberfox, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Cyberfox profile the agent is running
    # under so it doesn't conflate ~/.cyberfox/skills/ (default profile) with
    # ~/.cyberfox/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Cyberfox profile: default. Other profiles (if any) live "
            "under ~/.cyberfox/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Cyberfox profile: {active_profile}. This session reads "
            f"and writes ~/.cyberfox/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.cyberfox/skills/, ~/.cyberfox/plugins/, "
            f"~/.cyberfox/cron/, ~/.cyberfox/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Cyberfox never rebuilds or reinjects parts of it
