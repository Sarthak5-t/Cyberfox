...
projection into Cyberfox' display, approval bridging, transcript projection into
...
"codex_app_server"`. Cyberfox' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Cyberfox secret — gateway
        ...
        spawn_env = cyberfox_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("CYBERFOX_KANBAN_TASK"):
            kanban_db = spawn_env.get("CYBERFOX_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "CYBERFOX_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("CYBERFOX_HOME", os.path.expanduser("~/.cyberfox")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "cyberfox",
        client_title: str = "Cyberfox Agent",
