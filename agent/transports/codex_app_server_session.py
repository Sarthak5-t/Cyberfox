...
Owns one Codex thread per Cyberfox session. Drives `turn/start`, consumes
...
# Cyberfox' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_CYBERFOX_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Cyberfox/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Cyberfox session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _CYBERFOX_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("CYBERFOX_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="cyberfox",
            client_title="Cyberfox Agent",
            client_version=_get_cyberfox_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Cyberfox' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Cyberfox' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own cyberfox-tools callback we
            # auto-accept — the user already approved Cyberfox' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Cyberfox approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Cyberfox returns 'once', 'session', 'always', or 'deny'.
...
def _get_cyberfox_version() -> str:
    """Best-effort Cyberfox version string for codex's userAgent line."""
    try:
