...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Cyberfox with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.cyberfox/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Cyberfox
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``cyberfox tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``cyberfox setup`` for
