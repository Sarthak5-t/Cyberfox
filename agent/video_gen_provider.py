...
as ``kind: backend``) or ``~/.cyberfox/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``cyberfox tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$CYBERFOX_HOME/cache/videos/``, creating parents as needed."""
    from cyberfox_constants import get_cyberfox_home

    path = get_cyberfox_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$CYBERFOX_HOME/cache/videos/``.
