...
``kind: backend``) or ``~/.cyberfox/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`cyberfox_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.cyberfox/.env``) so credentials set
    through Cyberfox' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``cyberfox tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``cyberfox tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``cyberfox tools`` picker.

        Used by ``cyberfox_cli/tools_config.py`` to inject this provider as a
