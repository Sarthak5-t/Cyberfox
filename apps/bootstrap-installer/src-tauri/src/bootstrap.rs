...
//! CYBERFOX_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for CYBERFOX_HOME. Tests use this; production
...
async fn run_bootstrap(
    ...
    // the repo INTO at $HermesHome\hermes-agent. Mirrors cyberfox_constants.
    ...
    // Copy ourselves to CYBERFOX_HOME/hermes-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_hermes_home() {
        tracing::warn!(?err, "failed to copy installer into CYBERFOX_HOME (non-fatal)");
        emit_log(&format!(
