...
//!
//! Mirrors `cyberfox_constants.get_hermes_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\hermes
//!   macOS:   ~/.hermes
//!   Linux:   ~/.hermes  (override via $CYBERFOX_HOME)
//!
...
/// Returns the canonical Hermes home directory, respecting $CYBERFOX_HOME if set.
pub fn hermes_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("CYBERFOX_HOME") {
...
/// CYBERFOX_HOME so it survives repo checkout deletion (unlike anything under
/// hermes-agent/).
///
...
///
/// Lives directly under CYBERFOX_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves CYBERFOX_HOME identically and pins it into
...
pub fn copy_self_to_hermes_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to CYBERFOX_HOME");
    Ok(())
}
...
/// Initializes tracing to bootstrap-installer.log under CYBERFOX_HOME/logs/.
