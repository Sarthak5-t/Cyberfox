...
/// desktop app handing off via `Cyberfox-Setup.exe --update`.
...
export const $cyberfoxHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.cyberfox/logs/bootstrap-installer.log')
    $cyberfoxHome.set('~/.cyberfox')
  ...
  try {
    const [logPath, cyberfoxHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_cyberfox_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $cyberfoxHome.set(cyberfoxHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Cyberfox" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Cyberfox-Setup.exe --update);
...
export async function launchCyberfoxDesktop(): Promise<void> {
  ...
  await invoke('launch_cyberfox_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Cyberfox repository' },
