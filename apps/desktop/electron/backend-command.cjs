...
// Backend subcommand routing for the desktop-managed Cyberfox process.
//
// The desktop app launches its own headless backend via `cyberfox serve` — it
...
// app hasn't updated yet, or an older `cyberfox` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Cyberfox profile to pin via `--profile`.
 */
 ...
 * `-m cyberfox_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `cyberfox_cli/subcommands/dashboard.py` source registers
