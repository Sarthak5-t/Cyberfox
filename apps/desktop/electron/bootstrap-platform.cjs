 ...
 * null to keep it enabled. `CYBERFOX_DESKTOP_DISABLE_GPU` overrides detection
...
function detectRemoteDisplay(options = {}) {
  ...
  const override = String(env.CYBERFOX_DESKTOP_DISABLE_GPU || '')
    .trim()
    .toLowerCase()
  if (GPU_OVERRIDE_ON.has(override)) return 'override (CYBERFOX_DESKTOP_DISABLE_GPU)'
