 ...
 *
 * Drives apps/desktop's first-launch install of Cyberfox Agent by spawning
...
function bootstrapCacheDir(cyberfoxHome) {
  return path.join(cyberfoxHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.cyberfox/cyberfox-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(cyberfoxHome) {
  if (!cyberfoxHome) return null
  const candidate = path.join(cyberfoxHome, 'cyberfox-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(cyberfoxHome, commit) {
  return path.join(bootstrapCacheDir(cyberfoxHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/cyberfox-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(cyberfoxHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(cyberfoxHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, cyberfoxHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass CYBERFOX_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          CYBERFOX_HOME: cyberfoxHome || process.env.CYBERFOX_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, cyberfoxHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, cyberfoxHome }) {
  const args = ['--dir', activeRoot, '--cyberfox-home', cyberfoxHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, cyberfoxHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, cyberfoxHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, cyberfoxHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, cyberfoxHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(cyberfoxHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, cyberfoxHome, emit })
