...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'cyberfox-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/cyberfox-runner-test',
    sourceRepoRoot: null,
    cyberfoxHome: '/tmp/cyberfox-runner-test',
    logRoot: '/tmp/cyberfox-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'cyberfox-agent', 'scripts')
