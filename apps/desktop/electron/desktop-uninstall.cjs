 ...
 * the three user-facing uninstall modes to the `cyberfox uninstall` CLI flags,
 ...
 *              `cyberfox uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `cyberfox uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `cyberfox uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `cyberfox` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m cyberfox_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `cyberfox_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'cyberfox_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Cyberfox.app/Contents/MacOS/Cyberfox  → …/Cyberfox.app
 *   Windows: …\Cyberfox\Cyberfox.exe                 → …\Cyberfox  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/Cyberfox.app/Contents/MacOS/Cyberfox → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Cyberfox.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs Cyberfox.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]Cyberfox$/i.test(dir) || /[\\/]cyberfox-desktop$/i.test(dir)) return dir
    return null
  }

  // Linux: an AppImage exposes its own path via the APPIMAGE env var.
  if (env.APPIMAGE) return env.APPIMAGE
  // Unpacked electron-builder tree: …/linux-unpacked/cyberfox
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import cyberfox_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, cyberfoxHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export CYBERFOX_HOME=${q(cyberfoxHome)}`
  ]
  if (pythonPath) {
 ...
 * modes + `pythonPath`=agentRoot so `import cyberfox_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Cyberfox install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "CYBERFOX_HOME=${String(cyberfoxHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
  if (pythonPath) {
