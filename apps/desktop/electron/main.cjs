...
const { canImportCyberfoxCli, verifyCyberfoxCli } = require('./backend-probes.cjs')
const {
...
const { buildDesktopBackendEnv, normalizeCyberfoxHomeRoot } = require('./backend-env.cjs')
...
const USER_DATA_OVERRIDE = process.env.CYBERFOX_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.CYBERFOX_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with CYBERFOX_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[cyberfox] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[cyberfox] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('cyberfox:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[cyberfox] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[cyberfox] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[cyberfox] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// CYBERFOX_HOME — the user-facing root for everything Cyberfox-related. Mirrors
// scripts/install.ps1's $CyberfoxHome and scripts/install.sh's $CYBERFOX_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\cyberfox (matches install.ps1)
//   macOS / Linux: ~/.cyberfox (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.cyberfox directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\cyberfox yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// CYBERFOX_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// CYBERFOX_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.cyberfox / %LOCALAPPDATA%\cyberfox.
function resolveCyberfoxHome() {
  if (process.env.CYBERFOX_HOME) return normalizeCyberfoxHomeRoot(process.env.CYBERFOX_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'cyberfox-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a CYBERFOX_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\cyberfox and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('CYBERFOX_HOME')
    if (fromRegistry) return normalizeCyberfoxHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'cyberfox')
    const legacy = path.join(app.getPath('home'), '.cyberfox')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.cyberfox setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.cyberfox')
}

const CYBERFOX_HOME = resolveCyberfoxHome()

function cyberfoxManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_cyberfox_node_dirs() in
  // cyberfox_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(CYBERFOX_HOME, 'node')
  const bin = path.join(root, 'bin')
  const entries = IS_WINDOWS ? [root, bin] : [bin, root]
  return entries.filter(directoryExists)
}

function pathWithCyberfoxManagedNode(...entries) {
  return [...cyberfoxManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_CYBERFOX_ROOT — the canonical mutable Cyberfox install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_CYBERFOX_ROOT = path.join(CYBERFOX_HOME, 'cyberfox-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_CYBERFOX_ROOT, 'venv')
...
// boots, going straight to `resolveCyberfoxBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_CYBERFOX_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_CYBERFOX_ROOT, '.cyberfox-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which Cyberfox profile the desktop launches its
// local backend as. When set, startCyberfox() passes `cyberfox --profile <name>
// dashboard …`, which deterministically pins CYBERFOX_HOME (see
// _apply_profile_override in cyberfox_cli/main.py) and bypasses the sticky
// ~/.cyberfox/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors cyberfox_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// cyberfoxDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under CYBERFOX_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by cyberfox_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(CYBERFOX_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (cyberfox_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.CYBERFOX_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.CYBERFOX_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
  if (!Number.isFinite(raw) || raw <= 0) return 650
  return Math.max(120, raw)
})()
const APP_NAME = 'Cyberfox'
...
// 'cyberfox:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.cyberfox) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.cyberfox')
}
// Seed the native About panel with the live Cyberfox version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `cyberfox update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveCyberfoxVersion(),
...
const MEDIA_PROTOCOL = 'cyberfox-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(cyberfox-media://…)`.
...
let mainWindow = null
let cyberfoxProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by cyberfoxProcess +
// connectionPromise + startCyberfox(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.CYBERFOX_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.CYBERFOX_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startCyberfox() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning cyberfox serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Cyberfox backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (!text) return
  const lines = text.split(/\r?\n/).map(line => `[cyberfox] ${line}`)
  cyberfoxLog.push(...lines)
  if (cyberfoxLog.length > 300) {
    cyberfoxLog.splice(0, cyberfoxLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-cyberfox-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('cyberfox:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the cyberfox:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('cyberfox:bootstrap:event', ev)
}
...
// The Tauri updater writes CYBERFOX_HOME/.cyberfox-update-in-progress for the whole
...
// (`force_kill_other_cyberfox`, taskkill /IM cyberfox.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(CYBERFOX_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Cyberfox will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(CYBERFOX_HOME)
  }
  if (marker) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `cyberfox` — must not shadow `cyberfox.cmd`/`cyberfox.exe`.
...
function unwrapWindowsVenvCyberfoxCommand(command, backendArgs) {
  if (!IS_WINDOWS || !command || isCommandScript(command)) return null

  const resolved = path.resolve(String(command))
  if (!/^cyberfox(?:\.exe)?$/i.test(path.basename(resolved))) return null
  ...
  // died mid-`pip install` still has python.exe + cyberfox.exe on disk, but the
  ...
  if (
    ...
    rememberLog(
      `Ignoring venv Cyberfox at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
    return null
  }

  return {
    label: `existing Cyberfox Python at ${python}`,
    command: python,
    args: ['-m', 'cyberfox_cli.main', ...backendArgs],
    bootstrap: false,
    env: buildDesktopBackendEnv({
      cyberfoxHome: CYBERFOX_HOME,
...
// spawns `cyberfox serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `cyberfox` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'cyberfox_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, CYBERFOX_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isCyberfoxSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'cyberfox_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.CYBERFOX_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Cyberfox deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Cyberfox' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\cyberfox\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'cyberfox', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'cyberfox', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal CYBERFOX_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\cyberfox\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'cyberfox', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'cyberfox', 'git', 'bin', 'git.exe'))
  }
...
function recentCyberfoxLog() {
  return cyberfoxLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's cyberfox root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_CYBERFOX_ROOT.
// CYBERFOX_DESKTOP_CYBERFOX_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.CYBERFOX_DESKTOP_CYBERFOX_ROOT && path.resolve(process.env.CYBERFOX_DESKTOP_CYBERFOX_ROOT),
    !IS_PACKAGED && isCyberfoxSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isCyberfoxSourceRoot(ACTIVE_CYBERFOX_ROOT) ? ACTIVE_CYBERFOX_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_CYBERFOX_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('cyberfox:updates:progress', merged)
  }
}
...
// CYBERFOX_HOME/cyberfox-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_cyberfox_home). That binary owns
// ALL repo mutation — running `cyberfox update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'cyberfox-setup.exe' : 'cyberfox-setup'
  const candidate = path.join(CYBERFOX_HOME, name)
...
// Path to the venv shim whose lock decides whether `cyberfox update` can write
// fresh entry points. On Windows this is the file the running backend
// `cyberfox.exe` holds open; on POSIX it's never mandatory-locked.
function venvCyberfoxShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'cyberfox.exe')
    : path.join(updateRoot, 'venv', 'bin', 'cyberfox')
}
...
// only signals the direct child, so on Windows a backend `cyberfox.exe` that
// spawned its own grandchildren (a `cyberfox` REPL, a pty terminal session, the
...
// `cyberfoxProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `cyberfox.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a cyberfox
// REPL, a pty terminal, the gateway) keep `cyberfox.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (cyberfoxProcess && Number.isInteger(cyberfoxProcess.pid)) pids.push(cyberfoxProcess.pid)
  ...
  if (cyberfoxProcess && !cyberfoxProcess.killed) {
    try {
      cyberfoxProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvCyberfoxShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const stragglers = []
    if (cyberfoxProcess && Number.isInteger(cyberfoxProcess.pid)) stragglers.push(cyberfoxProcess.pid)
    for (const entry of backendPool.values()) {
...
// `cyberfox update`). Instead we spawn the staged Cyberfox-Setup binary with
// --update and quit, so it can run `cyberfox update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri cyberfox-setup. Unlike Windows
      ...
      // whole update itself: `cyberfox update` (backend) + `cyberfox desktop
    ...
    if (!updater) {
      ...
      // `cyberfox desktop`, never the Tauri installer that self-copies
      // cyberfox-setup.exe into CYBERFOX_HOME). They DO have a working `cyberfox`
      ...
      // checkout they're on — bare `cyberfox update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'cyberfox update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `cyberfox update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `cyberfox update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, cyberfoxRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Cyberfox — this window will close and the updater will open. Don’t reopen Cyberfox yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // cyberfox.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running cyberfox, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Cyberfox install open ' +
        '(a second Cyberfox window or a terminal running cyberfox?). Close it and retry.'
    ...
    // `cyberfox update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: CYBERFOX_HOME,
      env: {
        ...process.env,
        CYBERFOX_HOME,
        PATH: pathWithCyberfoxManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(CYBERFOX_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvCyberfox = path.join(venvBin, IS_WINDOWS ? 'cyberfox.exe' : 'cyberfox')
  ...
  // not just the `cyberfox.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvCyberfox) || fileExists(path.join(updateRoot, '.cyberfox-bootstrap-complete'))
...
// Resolve the cyberfox CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `cyberfox` on PATH.
function resolveCyberfoxCliBinary(updateRoot) {
  const venvCyberfox = path.join(updateRoot, 'venv', 'bin', 'cyberfox')
  if (fileExists(venvCyberfox)) return venvCyberfox
  return findOnPath('cyberfox') || null
}
...
// macOS/Linux in-app update: backend (`cyberfox update`) + OS-aware GUI rebuild
// (`cyberfox desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const cyberfox = resolveCyberfoxCliBinary(updateRoot)
  if (!cyberfox) {
    emitUpdateProgress({ stage: 'manual', message: 'cyberfox update', percent: null })
    return { ok: true, manual: true, command: 'cyberfox update', cyberfoxRoot: updateRoot }
  }

  // Put the Cyberfox-managed Node and the venv on PATH so `cyberfox desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\cyberfox\node, not node\bin.
  const env = {
    CYBERFOX_HOME,
    PATH: pathWithCyberfoxManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `cyberfox update` reaps stale `cyberfox serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads CYBERFOX_DESKTOP_CHILD_PID and excludes
  ...
  if (cyberfoxProcess && Number.isInteger(cyberfoxProcess.pid)) {
    desktopChildPids.push(cyberfoxProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.CYBERFOX_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Cyberfox (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(cyberfox, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'cyberfox update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'cyberfox update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(cyberfox, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Cyberfox to retry.',
      error: rebuilt.error || 'rebuild-failed'
    })
    return { ok: false, backendUpdated: true, error: 'desktop rebuild failed' }
  }

  // Linux in-app update terminal state (#45205). `cyberfox desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Cyberfox…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `cyberfox-desktop-update-${Date.now()}.sh`)
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Cyberfox to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Cyberfox desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Cyberfox to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Cyberfox.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Cyberfox.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Cyberfox to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.cyberfox-update-new"; then
    rm -rf "$DST.cyberfox-update-old" 2>/dev/null || true
    mv "$DST" "$DST.cyberfox-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.cyberfox-update-new" "$DST"
    rm -rf "$DST.cyberfox-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Cyberfox to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_CYBERFOX_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isCyberfoxSourceRoot(ACTIVE_CYBERFOX_ROOT) &&
    fileExists(venvPython) &&
    canImportCyberfoxCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_CYBERFOX_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `cyberfox
...
function resolveWebDist() {
  const override = process.env.CYBERFOX_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set CYBERFOX_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: cyberfox desktop --force-build`
  )
  return candidates[0]
}
...
function resolveCyberfoxCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Cyberfox.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.CYBERFOX_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveCyberfoxCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_CYBERFOX_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Cyberfox at ${ACTIVE_CYBERFOX_ROOT}`,
    command,
    args: ['-m', 'cyberfox_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      cyberfoxHome: CYBERFOX_HOME,
      pythonPathEntries: [ACTIVE_CYBERFOX_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_CYBERFOX_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveCyberfoxBackend(backendArgs) {
  // 1. Explicit override -- CYBERFOX_DESKTOP_CYBERFOX_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.CYBERFOX_DESKTOP_CYBERFOX_ROOT && path.resolve(process.env.CYBERFOX_DESKTOP_CYBERFOX_ROOT)
  if (overrideRoot && isCyberfoxSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Cyberfox source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `cyberfox` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isCyberfoxSourceRoot.)
  if (!IS_PACKAGED && isCyberfoxSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Cyberfox source at ${SOURCE_REPO_ROOT}`, backendArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_CYBERFOX_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\cyberfox\cyberfox-agent (Windows) or ~/.cyberfox/cyberfox-agent.
  ...
  //    to spawning cyberfox. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `cyberfox update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `cyberfox` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    CYBERFOX_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.CYBERFOX_DESKTOP_IGNORE_EXISTING !== '1') {
    let cyberfoxCommand = null
    const cyberfoxOverride = process.env.CYBERFOX_DESKTOP_CYBERFOX

    if (cyberfoxOverride) {
      const resolvedOverride = findOnPath(cyberfoxOverride)
      if (resolvedOverride) {
        cyberfoxCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(cyberfoxOverride, { isWsl: IS_WSL })) {
        cyberfoxCommand = cyberfoxOverride
      } else {
        rememberLog(`Ignoring Windows Cyberfox override under WSL: ${cyberfoxOverride}`)
      }
    } else {
      cyberfoxCommand = findOnPath('cyberfox')
    }

    if (cyberfoxCommand) {
      if (looksLikeDesktopAppBinary(cyberfoxCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Cyberfox CLI: ${cyberfoxCommand}`)
        cyberfoxCommand = null
      }
    }

    if (cyberfoxCommand) {
      const unwrapped = unwrapWindowsVenvCyberfoxCommand(cyberfoxCommand, backendArgs)
      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `cyberfox` shim
      ...
      const shellForProbe = isCommandScript(cyberfoxCommand)
      if (verifyCyberfoxCli(cyberfoxCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvCyberfoxCommand(cyberfoxCommand, backendArgs) || {
            label: `existing Cyberfox CLI at ${cyberfoxCommand}`,
            command: cyberfoxCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
            kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Cyberfox CLI at ${cyberfoxCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed cyberfox_cli module via system Python.
  ...
  if (python) {
    ...
    // having cyberfox_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\cyberfox\cyberfox-agent\venv.
    if (canImportCyberfoxCli(python)) {
      return {
        kind: 'python',
        label: `installed cyberfox_cli module via ${python}`,
        command: python,
        args: ['-m', 'cyberfox_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: cyberfox_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveCyberfoxBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Cyberfox Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_CYBERFOX_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveCyberfoxBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Cyberfox install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error(
        'Cyberfox recovery was handed off to Cyberfox Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      cyberfoxHome: CYBERFOX_HOME,
      logRoot: path.join(CYBERFOX_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Cyberfox install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Cyberfox bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(CYBERFOX_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startCyberfox() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // cyberfox:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveCyberfoxBackend(backend.args))
  }
  ...
  if (!isCyberfoxSourceRoot(ACTIVE_CYBERFOX_ROOT)) {
    throw new Error(
      `Cyberfox install at ${ACTIVE_CYBERFOX_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Cyberfox' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\cyberfox\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `cyberfox` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Cyberfox on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Cyberfox.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isCyberfoxSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Cyberfox venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Cyberfox at ${ACTIVE_CYBERFOX_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Cyberfox runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Cyberfox backend URL protocol: ${parsed.protocol}`))
      return
    }
    ...
    const req = client.request(
      parsed,
      {
        ...
        headers: {
          ...
          'X-Cyberfox-Session-Token': token,
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Cyberfox backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Cyberfox backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Cyberfox-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('cyberfox:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveCyberfoxCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('cyberfox:preview-file-changed', payload)
}
...
async function waitForCyberfox(baseUrl, token) {
  ...
  throw new Error(`Cyberfox backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('cyberfox:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('cyberfox:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Cyberfox...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('cyberfox:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('cyberfox:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('cyberfox:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'cyberfox:desktop:zoomLevel'
...
//
// Hosted Cyberfox gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``cyberfox_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / cyberfox #37247).
//     Both are set as HttpOnly cookies (``cyberfox_session_at`` ~15 min,
//     ``cyberfox_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:cyberfox-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Cyberfox gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Cyberfox backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root CYBERFOX_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.CYBERFOX_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.CYBERFOX_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by CYBERFOX_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (cyberfox #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Cyberfox gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Cyberfox gateway is selected, but no session token is saved. ' +
...
//   2. env override (CYBERFOX_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.CYBERFOX_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.CYBERFOX_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'CYBERFOX_DESKTOP_REMOTE_URL is set but CYBERFOX_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Cyberfox backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.CYBERFOX_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Cyberfox
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startCyberfox())
  ...
  // connect to Cyberfox gateway". Mirror the renderer's connect here so the test
...
function resetCyberfoxConnection() {
  connectionPromise = null
  backendStartFailure = null

  stopBackendChild(cyberfoxProcess)

  cyberfoxProcess = null
...
// startCyberfox() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetCyberfoxConnection() nulls cyberfoxProcess.
  const dying = cyberfoxProcess && !cyberfoxProcess.killed ? cyberfoxProcess : null
  resetCyberfoxConnection()
...
// profile to startCyberfox() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startCyberfox() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForCyberfox(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: cyberfoxLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited CYBERFOX_HOME env (see _apply_profile_override
  // step 3 in cyberfox_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveCyberfoxBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const cyberfoxCwd = resolveCyberfoxCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Cyberfox backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: cyberfoxCwd,
        CYBERFOX_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        CYBERFOX_DESKTOP: '1',
        CYBERFOX_WEB_DIST: webDist,
        ...(readyFile ? { CYBERFOX_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Cyberfox backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Cyberfox backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Cyberfox backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForCyberfox(baseUrl, token), startFailed])
  ready = true
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Cyberfox backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_cyberfox_home() recreates the deleted profile directory.
...
async function startCyberfox() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startCyberfox() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Cyberfox backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Cyberfox backend at ${remote.baseUrl}`, 24)
      await waitForCyberfox(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Cyberfox backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.cyberfox/active_profile file) and
    // resolves CYBERFOX_HOME the same way `cyberfox -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Cyberfox runtime', 28)
    const backend = await ensureRuntime(resolveCyberfoxBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const cyberfoxCwd = resolveCyberfoxCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Cyberfox backend via ${backend.label}`, 84)
    rememberLog(`Starting Cyberfox backend via ${backend.label}`)

    cyberfoxProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: cyberfoxCwd,
        env: {
          ...process.env,
          // Explicitly pin CYBERFOX_HOME for the child so Python's get_cyberfox_home()
          // resolves to the SAME location our resolveCyberfoxHome() picked. Without
          // this pin, Python falls back to ~/.cyberfox on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\cyberfox, which differs from C:\Users\<u>\.cyberfox.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets CYBERFOX_HOME via setx; the desktop
    ...
    cyberfoxProcess.stdout.on('data', rememberLog)
    cyberfoxProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    cyberfoxProcess.once('error', error => {
      rememberLog(`Cyberfox backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Cyberfox backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    cyberfoxProcess.once('exit', (code, signal) => {
      rememberLog(`Cyberfox backend exited (${signal || code})`)
      cyberfoxProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Cyberfox backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Cyberfox backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentCyberfoxLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Cyberfox backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(cyberfoxProcess, { readyFile }),
      backendStartFailed
    ])
    if (readyFile) {
      fs.unlink(readyFile, () => {})
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Cyberfox backend to become ready', 90)
    await Promise.race([waitForCyberfox(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null cyberfoxProcess when the child dies.
      childAlive: () => cyberfoxProcess !== null && cyberfoxProcess.exitCode === null && !cyberfoxProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Cyberfox backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Cyberfox is
...
// pushes pet state over IPC (cyberfox:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via cyberfox:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Cyberfox app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Cyberfox icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see cyberfox:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Cyberfox in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('cyberfox:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startCyberfox().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('cyberfox:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Cyberfox…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('cyberfox:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetCyberfoxConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Cyberfox backend failed liveness probe; dropping stale connection.')
    resetCyberfoxConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('cyberfox:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('cyberfox:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('cyberfox:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('cyberfox:window:openNewSession', async () => {
...
ipcMain.handle('cyberfox:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('cyberfox:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('cyberfox:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('cyberfox:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('cyberfox:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('cyberfox:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('cyberfox:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('cyberfox:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('cyberfox:pet-overlay:control', payload)
})
ipcMain.handle('cyberfox:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startCyberfox() call restarts the
...
ipcMain.handle('cyberfox:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startCyberfox() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('cyberfox:bootstrap:cancel', async () => {
...
ipcMain.handle('cyberfox:boot-progress:get', async () => bootProgressState)
ipcMain.handle('cyberfox:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('cyberfox:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('cyberfox:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('cyberfox:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('cyberfox:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('cyberfox:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('cyberfox:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('cyberfox:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('cyberfox:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('cyberfox:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new CYBERFOX_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('cyberfox:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('cyberfox:requestMicrophoneAccess', async () => {
...
ipcMain.handle('cyberfox:api', async (_event, request) => {
  ...
  // backend calls ensure_cyberfox_home() which recreates the profile directory,
...
ipcMain.handle('cyberfox:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('cyberfox:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('cyberfox:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('cyberfox:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('cyberfox:readFileText', async (_event, filePath) => {
...
ipcMain.handle('cyberfox:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('cyberfox:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('cyberfox:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('cyberfox:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('cyberfox:saveClipboardImage', async () => {
...
ipcMain.handle('cyberfox:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('cyberfox:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('cyberfox:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('cyberfox:titlebar-theme', (_event, payload) => {
...
ipcMain.on('cyberfox:native-theme', (_event, mode) => {
...
ipcMain.on('cyberfox:translucency', (_event, payload) => {
...
ipcMain.handle('cyberfox:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('cyberfox:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveCyberfoxCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('cyberfox:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveCyberfoxCwd()
}))

ipcMain.handle('cyberfox:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('cyberfox:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('cyberfox:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('cyberfox:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('cyberfox:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('cyberfox:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: cyberfoxLog.slice(-200) }))
...
function terminalShellCommand() {
  // CYBERFOX_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.CYBERFOX_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Cyberfox' TUI into
  ...
  env.TERM_PROGRAM = 'Cyberfox'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a cyberfox/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from CYBERFOX_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.CYBERFOX_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `cyberfox:terminal:${id}:${suffix}`
}
...
ipcMain.handle('cyberfox:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('cyberfox:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('cyberfox:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('cyberfox:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('cyberfox:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('cyberfox:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('cyberfox:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('cyberfox:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('cyberfox:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('cyberfox:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('cyberfox:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('cyberfox:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('cyberfox:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('cyberfox:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('cyberfox:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('cyberfox:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('cyberfox:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('cyberfox:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('cyberfox:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('cyberfox:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('cyberfox:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('cyberfox:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('cyberfox:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('cyberfox:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('cyberfox:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('cyberfox:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Cyberfox.')
  }
...
ipcMain.handle('cyberfox:terminal:write', (_event, id, data) => {
...
ipcMain.handle('cyberfox:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('cyberfox:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('cyberfox:updates:check', async () =>
...
ipcMain.handle('cyberfox:updates:apply', async (_event, payload) =>
...
ipcMain.handle('cyberfox:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('cyberfox:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Cyberfox version (the one `release.py` bumps in
// cyberfox_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Cyberfox version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveCyberfoxVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'cyberfox_cli', '__init__.py')
...
// Re-resolve the live Cyberfox version and push it into the native About panel
// just before showing it, so an in-place `cyberfox update` is reflected without
...
ipcMain.handle('cyberfox:version', async () => ({
  appVersion: resolveCyberfoxVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  cyberfoxRoot: resolveUpdateRoot()
}))
...
// the actual removal via `cyberfox uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (cyberfox_cli/uninstall.py + cyberfox_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_CYBERFOX_ROOT
  ...
  const fallback = () => ({
    cyberfox_home: CYBERFOX_HOME,
    agent_installed: isCyberfoxSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'cyberfox_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, CYBERFOX_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Cyberfox agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import cyberfox_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_CYBERFOX_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_CYBERFOX_ROOT, 'uninstall')
  } catch (error) {
  ...
  const scriptArgs = {
    ...
    pythonExe: py,
    pythonPath,
    agentRoot: ACTIVE_CYBERFOX_ROOT,
    uninstallArgs,
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `cyberfox-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `cyberfox-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('cyberfox:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('cyberfox:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('cyberfox:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('cyberfox:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// cyberfox:// deep links (e.g. cyberfox://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const CYBERFOX_PROTOCOL = 'cyberfox'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${CYBERFOX_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // cyberfox://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('cyberfox:deep-link', payload)
...
ipcMain.handle('cyberfox:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${CYBERFOX_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(CYBERFOX_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(CYBERFOX_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `cyberfox://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching cyberfox:// URL is in our own argv.
