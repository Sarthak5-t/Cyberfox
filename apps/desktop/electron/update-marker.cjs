...
function markerPath(cyberfoxHome) {
  return path.join(cyberfoxHome, '.cyberfox-update-in-progress')
}
 ...
 * clock for tests.
 */
function readLiveUpdateMarker(cyberfoxHome, { kill, now = Date.now, maxAgeMs = UPDATE_MARKER_MAX_AGE_MS } = {}) {
  const file = markerPath(cyberfoxHome)
  let raw
  try {
 ...
 *
 * The Tauri-based cyberfox-setup.exe takes several seconds to initialise its
 ...
 */
function writeUpdateMarker(cyberfoxHome, pid, { now = Date.now } = {}) {
  const file = markerPath(cyberfoxHome)
