 ...
 * Background
 * ----------
 * After `cyberfox update` + `cyberfox desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything CYBERFOX_DESKTOP_* is preserved
// plus CYBERFOX_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['CYBERFOX_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['CYBERFOX_DESKTOP_']
