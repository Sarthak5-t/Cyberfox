...
// Regression guards for Windows `cyberfox` resolution in main.cjs.
//
...
//      Git-Bash `cyberfox` shim shadowed the real cyberfox.cmd/cyberfox.exe; the
...
//      --repair by checking ONLY venv\Scripts\cyberfox.exe (the console-script
...
//      installs.
//   3. unwrapWindowsVenvCyberfoxCommand() returned the venv python with NO
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow cyberfox.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  // The old too-narrow check (only venv\Scripts\cyberfox.exe) must not return.
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvCyberfox\) \?/,
    'recovery regressed to gating only on the cyberfox.exe shim, which forces destructive --repair'
  )
})

test('unwrapWindowsVenvCyberfoxCommand smoke-tests the venv python before trusting it', () => {
  const source = readMain()
  const fnStart = source.indexOf('function unwrapWindowsVenvCyberfoxCommand(')
  assert.notEqual(fnStart, -1, 'unwrapWindowsVenvCyberfoxCommand must exist in main.cjs')
