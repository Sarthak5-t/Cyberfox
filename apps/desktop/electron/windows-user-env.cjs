...
// even though a fresh shell — and the Cyberfox CLI — sees it immediately. The
// desktop's CYBERFOX_HOME resolution relies on process.env, so that stale-snapshot
// gap silently sends the backend to the default %LOCALAPPDATA%\cyberfox. Reading
...
//       CYBERFOX_HOME    REG_SZ    F:\Cyberfox\data
//
