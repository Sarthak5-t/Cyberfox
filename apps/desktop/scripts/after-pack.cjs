 ...
 *
 * Stamps the Cyberfox icon + identity onto the packed Windows Cyberfox.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `cyberfox desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Cyberfox')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Cyberfox'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Cyberfox.exe keeps the stock Electron icon`)
  }
}
