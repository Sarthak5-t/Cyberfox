 ...
 * product name (`Cyberfox`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Cyberfox'
 *
 * This is a hard failure with no obvious cause for the user — `cyberfox desktop`
 ...
 * (the mac-unpacked Cyberfox.app bundle) and Windows (win-unpacked), so we
