...
// no longer bundles the Cyberfox Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Cyberfox.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Cyberfox'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Cyberfox.exe'),
...
// Default CYBERFOX_HOME for non-sandboxed runs -- matches main.cjs's
// resolveCyberfoxHome(). On Windows it's %LOCALAPPDATA%\hermes; elsewhere
// it's ~/.cyberfox. The fresh-install sandbox launchFresh() sets its own
// CYBERFOX_HOME and never touches this.
const DEFAULT_CYBERFOX_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'hermes')
  }
  return path.join(os.homedir(), '.hermes')
})()
const VENV_ROOT = path.join(DEFAULT_CYBERFOX_HOME, 'cyberfox-agent', 'venv')
...
function ensurePackagedApp() {
  if (process.env.CYBERFOX_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Cyberfox-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Cyberfox-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Cyberfox-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Cyberfox-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.CYBERFOX_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.CYBERFOX_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  env.CYBERFOX_DESKTOP_CWD = cwd
  env.CYBERFOX_DESKTOP_IGNORE_EXISTING = '1'
  env.CYBERFOX_DESKTOP_TEST_MODE = 'fresh-install'
  env.CYBERFOX_DESKTOP_USER_DATA_DIR = userDataDir
  env.CYBERFOX_HOME = hermesHome
  delete env.CYBERFOX_DESKTOP_CYBERFOX
  delete env.CYBERFOX_DESKTOP_CYBERFOX_ROOT
  ...
  console.log(`  CYBERFOX_HOME: ${hermesHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(hermesHome, 'cyberfox-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Cyberfox Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of cyberfox_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'cyberfox-agent', 'cyberfox_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Cyberfox
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + CYBERFOX_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  CYBERFOX_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
