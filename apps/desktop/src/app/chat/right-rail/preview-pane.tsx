        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.cyberfoxDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Cyberfox to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.cyberfoxDesktop?.watchPreviewFile ||
      !window.cyberfoxDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.cyberfoxDesktop.onPreviewFileChanged(payload => {
    ...
    void window.cyberfoxDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.cyberfoxDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.cyberfoxDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:cyberfox-preview')
