...
const branchActionLabel = (branch: CyberfoxGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `cyberfox/work-<ts>` trees.
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<CyberfoxGitBranch[]>([])
  ...
  const convert = async (branch: CyberfoxGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
