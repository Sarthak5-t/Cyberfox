...
import { getSessionMessages, type SessionMessage, triggerCronJob } from '../cyberfox'
...
import { useCyberfoxConfig } from './session/hooks/use-cyberfox-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.cyberfoxDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.cyberfoxDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.cyberfoxDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.cyberfoxDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // cyberfox:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.cyberfoxDesktop?.onDeepLink?.(payload => {
    ...
    void window.cyberfoxDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.cyberfoxDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshCyberfoxConfig, sttEnabled, voiceMaxRecordingSeconds } = useCyberfoxConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshCyberfoxConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshCyberfoxConfig])
