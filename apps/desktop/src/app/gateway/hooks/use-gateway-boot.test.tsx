...
// bug that drives the REAL useGatewayBoot hook + REAL CyberfoxGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshCyberfoxConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { cyberfoxDesktop?: unknown }).cyberfoxDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { cyberfoxDesktop?: unknown }).cyberfoxDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForCyberfox) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startCyberfox()'s remote branch awaits waitForCyberfox() for 45s before it
    ...
    ;(window as { cyberfoxDesktop?: unknown }).cyberfoxDesktop = desktop
    ...
    // After ~45s waitForCyberfox gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Cyberfox backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
