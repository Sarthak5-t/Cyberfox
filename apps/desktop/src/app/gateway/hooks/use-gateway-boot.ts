import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@cyberfox/shared'
import { useEffect, useRef } from 'react'

import type { CyberfoxConnection } from '@/global'
import { CyberfoxGateway } from '@/cyberfox'
...
import type { RpcEvent } from '@/types/cyberfox'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.cyberfoxDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: CyberfoxGateway | null) => void
  refreshCyberfoxConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.cyberfoxDesktop

    const publish = (next: CyberfoxConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Cyberfox...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Cyberfox…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Cyberfox gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshCyberfoxConfig().catch(() => undefined)
    ...
    const gateway = new CyberfoxGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshCyberfoxConfig()
