...
import type { CyberfoxReadDirEntry, CyberfoxReadDirResult } from '@/global'
...
export type ProjectTreeEntry = CyberfoxReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: CyberfoxReadDirEntry) {
...
async function filterIgnored(entries: CyberfoxReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<CyberfoxReadDirResult> {
  if (!window.cyberfoxDesktop) {
