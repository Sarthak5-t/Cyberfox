...
import type { CyberfoxReadDirResult } from '@/global'
import { $connection } from '@/store/session'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<CyberfoxReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { cyberfoxDesktop: { readDir: typeof readDir } }).cyberfoxDesktop = { readDir }
})
...
afterEach(() => {
  ...
  $connection.set(null)
  resetProjectTreeState()
  delete (window as unknown as { cyberfoxDesktop?: unknown }).cyberfoxDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): CyberfoxReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { cyberfoxDesktop: unknown }).cyberfoxDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: CyberfoxReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<CyberfoxReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { cyberfoxDesktop: unknown }).cyberfoxDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.cyberfoxDesktop is missing', async () => {
    delete (window as unknown as { cyberfoxDesktop?: unknown }).cyberfoxDesktop
