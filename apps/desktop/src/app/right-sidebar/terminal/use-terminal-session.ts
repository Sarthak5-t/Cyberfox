...
const CYBERFOX_PATHS_MIME = 'application/x-cyberfox-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(CYBERFOX_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(CYBERFOX_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.cyberfoxDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      letterSpacing: 0,
      lineHeight: 1.12,
      // Full-screen TUIs (cyberfox --tui, vim) grab the mouse, so a plain drag
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[cyberfox-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      void window.cyberfoxDesktop?.terminal?.write(sessionId, `${command}\r`)
