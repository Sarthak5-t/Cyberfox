...
import { getCyberfoxConfig } from '@/cyberfox'
import { persistString } from '@/lib/storage'
import { $currentCwd, setCurrentCwd } from '@/store/session'

import { useCyberfoxConfig } from './use-cyberfox-config'

vi.mock('@/cyberfox', () => ({
  getCyberfoxConfig: vi.fn(),
  getCyberfoxConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'cyberfox.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getCyberfoxConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getCyberfoxConfig>>)

describe('useCyberfoxConfig refreshCyberfoxConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshCyberfoxConfig()
    })
