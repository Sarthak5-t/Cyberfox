...
import { getCyberfoxConfig, getCyberfoxConfigDefaults } from '@/cyberfox'
...
interface CyberfoxConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useCyberfoxConfig({ activeSessionIdRef, refreshProjectBranch }: CyberfoxConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshCyberfoxConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getCyberfoxConfig(), getCyberfoxConfigDefaults().catch(() => ({}))])
  ...
  return { refreshCyberfoxConfig, sttEnabled, voiceMaxRecordingSeconds }
}
