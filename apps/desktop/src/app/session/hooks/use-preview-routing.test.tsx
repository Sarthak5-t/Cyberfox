...
describe('usePreviewRouting', () => {
  ...
  it('does not infer previews from assistant prose', async () => {
    ...
    expect(window.cyberfoxDesktop.normalizePreviewTarget).not.toHaveBeenCalled()
  })
  ...
  it('does not auto-open a preview from tool results', async () => {
    ...
    expect($previewTarget.get()).toBeNull()
    expect(window.localStorage.getItem('cyberfox.desktop.sessionPreviews.v1')).toBeNull()
  })
})
