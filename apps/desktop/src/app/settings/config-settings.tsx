...
import { getElevenLabsVoices, getCyberfoxConfigSchema, saveCyberfoxConfig } from '@/cyberfox'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, CyberfoxConfigRecord } from '@/types/cyberfox'

import { setCyberfoxConfigCache, useCyberfoxConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: CyberfoxConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<CyberfoxConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useCyberfoxConfigRecord()
  ...
  const {
    ...
    queryKey: ['cyberfox-config-schema'],
    queryFn: getCyberfoxConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  const updateConfig = (next: CyberfoxConfigRecord) => {
