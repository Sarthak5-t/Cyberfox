...
import { getCyberfoxConfigDefaults, getCyberfoxConfigRecord, saveCyberfoxConfig } from '@/cyberfox'
...
export function SettingsView({ onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getCyberfoxConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'cyberfox-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveCyberfoxConfig(await getCyberfoxConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
