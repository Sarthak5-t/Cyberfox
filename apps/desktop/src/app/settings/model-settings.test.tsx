...
const getCyberfoxConfigRecord = vi.fn()
const saveCyberfoxConfig = vi.fn()
...
vi.mock('@/cyberfox', () => ({
  ...
  getCyberfoxConfigRecord: () => getCyberfoxConfigRecord(),
  saveCyberfoxConfig: (config: unknown) => saveCyberfoxConfig(config)
}))
...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nous', model: 'cyberfox-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'Nous',
        slug: 'nous',
        models: ['cyberfox-4', 'cyberfox-4-mini'],
        authenticated: true,
        capabilities: { 'cyberfox-4': { reasoning: true, fast: true } }
      },
  ...
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nous', model: 'cyberfox-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  setModelAssignment.mockResolvedValue({ provider: 'nous', model: 'cyberfox-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'deepseek', model: 'deepseek-chat', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getCyberfoxConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveCyberfoxConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getCyberfoxConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveCyberfoxConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['cyberfox-4'],
          authenticated: true,
          capabilities: { 'cyberfox-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nous', model: 'cyberfox-4' }]
    })
