...
import { invalidateCyberfoxConfig, setCyberfoxConfigCache, useCyberfoxConfigRecord } from '../hooks/use-config-record'
...
// Cyberfox' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Cyberfox default (medium), shown as Medium.
...
// backend now surfaces the full `cyberfox model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in cyberfox_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useCyberfoxConfigRecord()
  const setConfig = setCyberfoxConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `cyberfox model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Cyberfox runs the flow for you.`}
          </p>
        )}
