...
import { disconnectOAuthProvider, listOAuthProviders } from '@/cyberfox'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/cyberfox'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.cyberfoxDesktop?.terminal)
...
//      in cyberfox_cli/provider_catalog.py) — the SAME provider identity
//      `cyberfox model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Cyberfox can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Cyberfox can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Cyberfox never deletes creds another tool owns behind a silent API call.
