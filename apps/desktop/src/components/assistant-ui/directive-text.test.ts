...
import { formatRefValue, cyberfoxDirectiveFormatter } from './directive-text'
...
describe('cyberfoxDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = cyberfoxDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = cyberfoxDirectiveFormatter.parse('@file:src/main.tsx the entry point')
