...
const CYBERFOX_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type CyberfoxRefType = (typeof CYBERFOX_REF_TYPES)[number]
...
const ICON_PATHS: Record<CyberfoxRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as CyberfoxRefType] ?? ICON_FALLBACK
...
const CYBERFOX_DIRECTIVE_RE = new RegExp(
...
export const cyberfoxDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(CYBERFOX_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as CyberfoxRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: CyberfoxRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...cyberfoxDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Cyberfox directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.cyberfoxDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.cyberfoxDesktop?.readFileDataUrl(id)
