...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.cyberfoxDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.cyberfoxDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.cyberfoxDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.cyberfoxDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.cyberfoxDesktop && text && normalizeExternalUrl(text) === target) {
