...
declare global {
  interface Window {
    cyberfoxDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<CyberfoxConnection>
      ...
      profile: {
        ...
        // backend under the new CYBERFOX_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: CyberfoxApiRequest) => Promise<T>
      notify: (payload: CyberfoxNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<CyberfoxReadFileTextResult>
      selectPaths: (options?: CyberfoxSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<CyberfoxPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<CyberfoxPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: CyberfoxTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<CyberfoxReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<CyberfoxGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<CyberfoxGitBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<CyberfoxRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: CyberfoxReviewScope, baseRef?: null | string) => Promise<CyberfoxReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: CyberfoxReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<CyberfoxReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: CyberfoxTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<CyberfoxTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: CyberfoxWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: CyberfoxPreviewFileChanged) => void) => () => void
...
export interface CyberfoxTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface CyberfoxTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `cyberfox update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  cyberfoxRoot?: string
...
export interface CyberfoxConnection {
  baseUrl: string
...
export interface CyberfoxTitleBarTheme {
  background: string
  foreground: string
}

export interface CyberfoxWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface CyberfoxApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface CyberfoxNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface CyberfoxPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface CyberfoxReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface CyberfoxPreviewWatch {
  id: string
  path: string
}
...
export interface CyberfoxGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface CyberfoxGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
export interface CyberfoxRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface CyberfoxRepoStatus {
  branch: null | string
...
// turn began.
export type CyberfoxReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface CyberfoxReviewFile {
  path: string
  added: number
  removed: number
...
export interface CyberfoxReviewList {
  files: CyberfoxReviewFile[]
...
export interface CyberfoxReviewPr {
  url: string
  state: string
  number: number
}
...
export interface CyberfoxReviewShipInfo {
  ghReady: boolean
  pr: CyberfoxReviewPr | null
}

export interface CyberfoxReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface CyberfoxReadDirResult {
  entries: CyberfoxReadDirEntry[]
  error?: string
}

export interface CyberfoxPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface CyberfoxSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
