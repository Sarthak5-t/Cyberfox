...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'cyberfox:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.cyberfoxDesktop?.saveImageFromUrl) {
        if (await window.cyberfoxDesktop.saveImageFromUrl(src)) {
