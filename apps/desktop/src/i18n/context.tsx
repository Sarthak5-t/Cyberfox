...
import { getCyberfoxConfigRecord, type CyberfoxConfigRecord, saveCyberfoxConfig } from '@/cyberfox'
...
export interface I18nConfigClient {
  getConfig: () => Promise<CyberfoxConfigRecord>
  saveConfig: (config: CyberfoxConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.cyberfoxDesktop?.api) {
      return Promise.resolve({})
    }

    return getCyberfoxConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.cyberfoxDesktop?.api) {
    ...
    return saveCyberfoxConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: CyberfoxConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: CyberfoxConfigRecord, locale: Locale): CyberfoxConfigRecord {
