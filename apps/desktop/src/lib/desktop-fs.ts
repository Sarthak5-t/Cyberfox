import type {
  CyberfoxConnection,
  CyberfoxReadDirResult,
  CyberfoxReadFileTextResult,
  CyberfoxSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: CyberfoxConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<CyberfoxReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<CyberfoxReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<CyberfoxReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<CyberfoxReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: CyberfoxSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
