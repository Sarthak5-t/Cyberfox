...
/** Why a known Cyberfox command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Cyberfox profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Cyberfox Agent version', surface: exec() },
...
function isKnownCyberfoxSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Cyberfox' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownCyberfoxSlashCommand(normalized)
}
