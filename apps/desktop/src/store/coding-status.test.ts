...
import type { CyberfoxRepoStatus } from '@/global'

import { $repoStatus, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: CyberfoxRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<CyberfoxRepoStatus | null>) {
  ;(window as unknown as { cyberfoxDesktop?: unknown }).cyberfoxDesktop = { git: { repoStatus: impl } }
}
