import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@cyberfox/shared'
...
const isOpen = (gateway: CyberfoxGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<CyberfoxGateway | null>(null)
...
let primaryGateway: CyberfoxGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: CyberfoxGateway | null, profile = 'default'): void {
...
export function activeGateway(): CyberfoxGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<CyberfoxGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
