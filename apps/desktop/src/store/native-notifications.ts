...
const STORAGE_KEY = 'cyberfox:native-notifications'
...
// "Backgrounded" = the user isn't on Cyberfox. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.cyberfoxDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.cyberfoxDesktop
