...
import type { OAuthProvider } from '@/types/cyberfox'
...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('cyberfox-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('cyberfox-desktop-onboarded-v1')).toBe('1')
  })
