 ...
 * app's bounds and stays visible while Cyberfox is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'cyberfox.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'cyberfox.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.cyberfoxDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.cyberfoxDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.cyberfoxDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.cyberfoxDesktop?.petOverlay?.close()
}
