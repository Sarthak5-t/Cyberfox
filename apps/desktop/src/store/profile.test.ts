...
const { getProfiles } = await import('@/cyberfox')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/cyberfox/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<CyberfoxConnection> = {}): CyberfoxConnection =>
  ({ baseUrl: 'https://cyberfox-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as CyberfoxConnection

const localConn = (over: Partial<CyberfoxConnection> = {}): CyberfoxConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as CyberfoxConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<CyberfoxConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { cyberfoxDesktop: { getConnection } })
