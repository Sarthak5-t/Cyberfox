...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/cyberfox'
...
import type { ProfileInfo } from '@/types/cyberfox'
...
// /api/profiles/active `current`). "default" is the root ~/.cyberfox. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'cyberfox.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'cyberfox.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.cyberfoxDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new CYBERFOX_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.cyberfoxDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.cyberfoxDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'cyberfox.desktop.showAllProfiles'
...
// Switch to the default (root ~/.cyberfox) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.cyberfoxDesktop?.touchBackend?.(target).catch(() => undefined)
}
