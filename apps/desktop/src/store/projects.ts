...
import type { CyberfoxGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/cyberfox'
...
const PROJECT_SCOPE_KEY = 'cyberfox.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Cyberfox gateway is not connected')
  }
...
// repo, returning where Cyberfox should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<CyberfoxGitBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.cyberfoxDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.cyberfoxDesktop?.writeClipboard?.(path)
  }
}
