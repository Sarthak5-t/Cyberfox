...
import type { CyberfoxReviewFile, CyberfoxReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Cyberfox' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'cyberfox.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'cyberfox.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'cyberfox.desktop.reviewTreeMode'
const SELECTED_KEY = 'cyberfox.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<CyberfoxReviewFile[]>([])
...
export const $reviewShipInfo = atom<CyberfoxReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['cyberfoxDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: CyberfoxReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.cyberfoxDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.cyberfoxDesktop?.openExternal?.(url)
    }
