...
import type { SessionInfo, UsageStats } from '@/types/cyberfox'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'cyberfox.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'cyberfox.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'cyberfox.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'cyberfox.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'cyberfox.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'cyberfox.desktop.lastSessionId'
...
function workspaceCwdKey(connection: CyberfoxConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.cyberfoxDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.cyberfoxDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<CyberfoxConnection | null>(null)
...
export const setConnection = (next: Updater<CyberfoxConnection | null>) => updateAtom($connection, next)
