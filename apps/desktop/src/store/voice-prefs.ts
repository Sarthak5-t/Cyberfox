...
import { getCyberfoxConfigRecord, saveCyberfoxConfig } from '@/cyberfox'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getCyberfoxConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveCyberfoxConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
