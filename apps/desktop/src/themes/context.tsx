...
const SKIN_KEY = 'cyberfox-desktop-theme-v2'
const MODE_KEY = 'cyberfox-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'cyberfox-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'cyberfox-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'cyberfox-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.cyberfoxTheme = skinName
  root.dataset.cyberfoxMode = rendered
  ...
  window.cyberfoxDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('cyberfox-boot-background', chromeBg)
    window.localStorage.setItem('cyberfox-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.cyberfoxThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.cyberfoxDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Cyberfox backend theme on connect. */
