...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Cyberfox doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in cyberfox_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `cyberfox model` uses — instead of
...
export interface CyberfoxConfig {
  agent?: {
...
export type CyberfoxConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `cyberfox model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors cyberfox_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Cyberfox, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `cyberfox tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Cyberfox); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/cyberfox/update/check` — the backend's own update state.
