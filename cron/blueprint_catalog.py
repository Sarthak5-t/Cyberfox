...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``cyberfox://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``cyberfox://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"cyberfox://blueprint/{quote(blueprint.key)}{qs}"
