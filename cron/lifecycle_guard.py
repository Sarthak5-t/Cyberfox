...
``cyberfox gateway restart`` (or ``launchctl kickstart ai.cyberfox.gateway``
or ``systemctl restart cyberfox-gateway``).  When the cron fires, the
...
``cyberfox cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``cyberfox gateway``, ``launchctl ... cyberfox-gateway``,
``systemctl ... cyberfox-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_CYBERFOX_GATEWAY=1``, and
``cyberfox gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `cyberfox gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:cyberfox\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a cyberfox-gateway label. macOS launchd
    # labels look like `ai.cyberfox.gateway` / `cyberfox-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated cyberfox services (e.g.
    # `launchctl unload ai.cyberfox.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bcyberfox[.\-]?gateway)"
    # Branch C: systemctl ops on a cyberfox-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bcyberfox[.\-]?gateway)"
    # Branch D: pkill / kill targeting the cyberfox gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<CYBERFOX_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.cyberfox/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_cyberfox_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `cyberfox gateway restart` from a shell outside "
