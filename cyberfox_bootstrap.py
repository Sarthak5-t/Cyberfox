"""Windows UTF-8 bootstrap for Cyberfox entry points.
...
should be imported at the very top of every Cyberfox entry point
(``cyberfox``, ``cyberfox-agent``, ``cyberfox-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Cyberfox modules.

    Cyberfox ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Cyberfox source root
    ...
    """
    root = src_root or os.environ.get("CYBERFOX_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``CYBERFOX_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("CYBERFOX_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import cyberfox_bootstrap``
# (or ``from cyberfox_bootstrap import apply_windows_utf8_bootstrap``) at
