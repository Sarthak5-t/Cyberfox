...
Supports OAuth device code flows (OpenAI Codex, future: xAI) and
...
SPOTIFY_DOCS_URL = "https://github.com/Sarthak5-t/Cyberfox/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://github.com/Sarthak5-t/Cyberfox/guides/oauth-over-ssh"
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared token store) don't share reentrancy
...
def _auth_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    the shared lock, acquire ``_auth_store_lock`` FIRST
    (outer) and the shared lock SECOND (inner). All runtime
...
def _decode_jwt_claims(token: Any) -> Dict[str, Any]:
    ...
    Returns an empty dict if the token is not a well-formed JWT (e.g. an opaque
    bearer token). Used to inspect exp/iss/sub claims on provider OAuth tokens.
    """
