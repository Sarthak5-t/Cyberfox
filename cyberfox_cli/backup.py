"""
Backup and import commands for cyberfox CLI.

`cyberfox backup` creates a zip archive of the entire ~/.cyberfox/ directory
(excluding the cyberfox-agent repo and transient files).

`cyberfox import` restores from a backup zip, overlaying onto the current
CYBERFOX_HOME root.
"""
...
from cyberfox_constants import get_default_cyberfox_root, get_cyberfox_home, display_cyberfox_home
...
# ``cyberfox-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/cyberfox-agent/``
...
# CYBERFOX_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "cyberfox-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under CYBERFOX_HOME) —
...
# File names that ``cyberfox import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking", disconnecting it from the hosted gateway
...
# Reserved archive subtree for provider state that lives OUTSIDE CYBERFOX_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside CYBERFOX_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to cyberfox root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``cyberfox-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/cyberfox-agent/`` — must be preserved.
        if part == "cyberfox-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside CYBERFOX_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Cyberfox home directory."""
    cyberfox_root = get_default_cyberfox_root()

    if not cyberfox_root.is_dir():
        print(f"Error: Cyberfox home directory not found at {cyberfox_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"cyberfox-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"cyberfox-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_cyberfox_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(cyberfox_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(cyberfox_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``cyberfox-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "cyberfox-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(cyberfox_root)
    ...
    # outside CYBERFOX_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_cyberfox_home()}."
        )
    ...
    print(f"\nRestore with: cyberfox import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Cyberfox backup.
    ...
    # Look for telltale files that a cyberfox home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Cyberfox backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.cyberfox/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a cyberfox dir name
        if prefix in {".cyberfox", "cyberfox"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_cyberfox_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (cyberfox_root / "config.yaml").exists()
        has_env = (cyberfox_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Cyberfox configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under CYBERFOX_HOME.
...
#     managed gateway. Matched by basename so both the root profile and
            # named profiles (profiles/<name>/gateway_state.json) are covered.
            if Path(rel).name in _IMPORT_SKIP_NAMES:
                skipped_runtime.append(rel)
                continue

            target = cyberfox_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(cyberfox_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_cyberfox_home()}."
            )
        ...
        profiles_dir = cyberfox_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # cyberfox_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: cyberfox profile list  (after installing cyberfox)")

        # Guidance
        print()
        if not (cyberfox_root / "cyberfox-agent").is_dir():
            print("Note: The cyberfox-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: cyberfox update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  cyberfox -p {pname} gateway install")

        print("Done. Your Cyberfox configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and cyberfox backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to CYBERFOX_HOME).
...
# — `cyberfox update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $CYBERFOX_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside CYBERFOX_HOME and the entry is silently
...
def _quick_snapshot_root(cyberfox_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(cyberfox_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside CYBERFOX_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``cyberfox update``.
    ...
    Args:
        ...
        cyberfox_home: Override for the Cyberfox home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(cyberfox_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for cyberfox backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_cyberfox_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, cyberfox_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``cyberfox_root`` to ``out_path``.
...
def _pre_update_backup_dir(cyberfox_home: Optional[Path] = None) -> Path:
    home = cyberfox_home or get_cyberfox_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of CYBERFOX_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<CYBERFOX_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``cyberfox update``) should continue even if the backup fails.
    """
    cyberfox_root = cyberfox_home or get_default_cyberfox_root()
    if not cyberfox_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(cyberfox_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, cyberfox_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `cyberfox claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of CYBERFOX_HOME under ``backups/`` before a
    ``cyberfox claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``cyberfox import <archive>``.  Writes to
    ``<CYBERFOX_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `cyberfox import` and the
