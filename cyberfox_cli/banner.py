...
Pure display functions with no CyberfoxCLI state dependency.
"""
...
from cyberfox_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built cyberfox — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/cyberfox-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/cyberfox-agent"
...
def _fetch_pypi_latest(package: str = "cyberfox-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Cyberfox update is available.

    Two paths: if ``CYBERFOX_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = cyberfox_home / ".update_check"
    embedded_rev = os.environ.get("CYBERFOX_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # CYBERFOX_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `cyberfox update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/cyberfox/update/check`
    ...
    if embedded_rev:
        ...
        # $CYBERFOX_HOME/cyberfox-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = cyberfox_home / "cyberfox-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Cyberfox git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$CYBERFOX_HOME/cyberfox-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``cyberfox_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from cyberfox_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/cyberfox-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Cyberfox checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/cyberfox-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Cyberfox Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Sarthak Khatal[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Sarthak Khatal[/]")

    if os.getenv("CYBERFOX_YOLO_MODE"):
    ...
    try:
        from cyberfox_cli.codex_runtime_switch import get_current_runtime
    ...
    # Update check — disabled for local development

    # Pip-install warning — `pip install cyberfox-agent` is not the supported
