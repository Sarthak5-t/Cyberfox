"""`cyberfox checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.cyberfox/checkpoints/``.  Actions:

    cyberfox checkpoints               # same as `status`
    cyberfox checkpoints status        # total size, project count, breakdown
    cyberfox checkpoints list          # per-project checkpoint counts + workdir
    cyberfox checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    cyberfox checkpoints clear [-f]    # nuke the entire base (asks first)
    cyberfox checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    cyberfox checkpoints
    cyberfox checkpoints prune --retention-days 3 --max-size-mb 200
    cyberfox checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: cyberfox checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``cyberfox checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `cyberfox checkpoints` → status
