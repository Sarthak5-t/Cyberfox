"""Shell completion script generation for cyberfox CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_cyberfox_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Cyberfox Agent bash completion
# Add to ~/.bashrc:
#   eval "$(cyberfox completion bash)"

_cyberfox_profiles() {{
    local profiles_dir="$HOME/.cyberfox/profiles"
...
_cyberfox_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_cyberfox_profiles)" -- "$cur"))
        return
    fi
...
complete -F _cyberfox_completion cyberfox
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef cyberfox
# Cyberfox Agent zsh completion
# Add to ~/.zshrc:
...
_cyberfox_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.cyberfox/profiles" ]]; then
        profiles+=($HOME/.cyberfox/profiles/*(N/:t))
    fi
...
_cyberfox() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_cyberfox_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'cyberfox command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Cyberfox Agent fish completion",
        "# Add to your config:",
        "#   cyberfox completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __cyberfox_profiles",
        "    echo default",
        "    if test -d $HOME/.cyberfox/profiles",
        "        for d in $HOME/.cyberfox/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c cyberfox -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c cyberfox -f -s p -l profile"
        " -d 'Profile name' -xa '(__cyberfox_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__cyberfox_profiles)' -d 'Profile name'"
                )
