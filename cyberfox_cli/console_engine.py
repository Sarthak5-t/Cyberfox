"""Safe Cyberfox Console command engine.

This module backs ``cyberfox console`` and is intentionally narrower than the
full Cyberfox CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["CyberfoxConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'cyberfox doctor'")
        and last.startswith("Run 'cyberfox setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="cyberfox", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `cyberfox "):
        return ""
    return summary
...
# dashboard opens a fresh CyberfoxConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("cyberfox_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace, ConsoleContext], None] | None = None,
) -> Callable[["CyberfoxConsoleEngine", list[str]], str]:
    def handler(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    ...
    *,
    ...
    handler_factory: Callable[[Sequence[str]], Callable[["CyberfoxConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `cyberfox {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `cyberfox {usage}`?",
            contexts=ALL_CONTEXTS if child_key in hosted_paths else LOCAL_CONTEXTS,
        )


class CyberfoxConsoleEngine:
    """Curated line-command executor for Cyberfox Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "cyberfox":
                tokens = tokens[1:]
            if not tokens:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Cyberfox Console does not run shell syntax. Use one supported "
                    "Cyberfox command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Cyberfox component status.", _status, contexts=ALL_CONTEXTS)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor, contexts=ALL_CONTEXTS)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Cyberfox logs.", _logs, contexts=ALL_CONTEXTS)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Cyberfox configuration?",
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Cyberfox Console."""

        extracted = {
            "version": (
                "cyberfox_cli.subcommands.version",
            ...
            "debug": (
                "cyberfox_cli.subcommands.debug",
                "build_debug_parser",
                "cmd_debug",
                [("share",), ("delete",)],
                {("share",), ("delete",)},
            ...
            "prompt-size": (
                "cyberfox_cli.subcommands.prompt_size",
            ...
            "insights": (
                "cyberfox_cli.subcommands.insights",
            ...
            "security": (
                "cyberfox_cli.subcommands.security",
            ...
            "backup": (
                "cyberfox_cli.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "cyberfox_cli.subcommands.import_cmd",
            ...
            "config": (
                "cyberfox_cli.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "tools": (
                "cyberfox_cli.subcommands.tools",
                "build_tools_parser",
                "cmd_tools",
            ...
            "plugins": (
                "cyberfox_cli.subcommands.plugins",
            ...
            "skills": (
                "cyberfox_cli.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "memory": (
                "cyberfox_cli.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "pairing": (
                "cyberfox_cli.subcommands.pairing",
            ...
            "webhook": (
                "cyberfox_cli.subcommands.webhook",
            ...
            "hooks": (
                "cyberfox_cli.subcommands.hooks",
                "build_hooks_parser",
                "cmd_hooks",
            ...
            "slack": (
                "cyberfox_cli.subcommands.slack",
                "build_slack_parser",
                "cmd_slack",
                [("manifest",)],
                set(),
            ...
            "profile": (
                "cyberfox_cli.subcommands.profile",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Cyberfox configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "cyberfox_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("cyberfox_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "cyberfox_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("cyberfox_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("cyberfox_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        for size in range(min(len(tokens), 3), 0, -1):
            ...
            if command:
                if self.context not in command.contexts:
                    raise ConsoleCommandError(
                        f"`cyberfox {command.usage}` is not available in "
                        f"{self.context} Cyberfox Console."
                    )
        ...
        raise ConsoleCommandError(f"Unsupported Cyberfox Console command: {probe}.{suffix}")
    ...
    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Cyberfox Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`cyberfox {first}` is not available in Cyberfox Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Cyberfox Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Cyberfox Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Cyberfox Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Cyberfox Console.",
            ("skills", "publish"): "`skills publish` is not available in Cyberfox Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Cyberfox Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Cyberfox Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Cyberfox Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Cyberfox Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Cyberfox Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Cyberfox Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Cyberfox Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Cyberfox Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Cyberfox Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Cyberfox Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Cyberfox Console."
        return ""
...
def _enforce_hosted_line_policy(path: tuple[str, ...], args: Sequence[str]) -> None:
    if path == ("config", "set"):
        ...
        if key and not _hosted_config_key_allowed(key):
            raise ConsoleCommandError(
                f"`config set {key}` is not available in hosted Cyberfox Console. "
    ...
    if path == ("mcp", "add"):
        if _flag_present(args, "--command") or _flag_present(args, "--args"):
            raise ConsoleCommandError(
                "Hosted Cyberfox Console does not add stdio MCP servers. "
                "Use catalog install or an HTTP/SSE URL."
            )
        if _flag_present(args, "--preset"):
            raise ConsoleCommandError(
                "Hosted Cyberfox Console does not add MCP presets directly. "
                "Use `mcp install <catalog-name>`."
            )
        url = _flag_value(args, "--url")
        if not url:
            raise ConsoleCommandError(
                "Hosted Cyberfox Console requires `mcp add` to use --url with "
                "an HTTP/SSE endpoint."
            )
        scheme = urlparse(url).scheme.lower()
        if scheme not in {"http", "https"}:
            raise ConsoleCommandError(
                "Hosted Cyberfox Console only accepts http:// or https:// MCP URLs."
            )
        return
...
def _apply_confirmed_defaults(args: argparse.Namespace, context: ConsoleContext) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Cyberfox Console.")
...
def _status(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Cyberfox Console.")
    ...
    from cyberfox_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from cyberfox_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config path")
    from cyberfox_cli.config import get_config_path

    return str(get_config_path())


def _config_set(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
        raise ConsoleCommandError("Usage: config set <key> <value>")
    key = args[0]
    value = " ".join(args[1:])
    from cyberfox_cli.config import set_config_value

    return _capture_output(lambda: set_config_value(key, value))


def _config_migrate(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config migrate")

    def _run() -> None:
        from cyberfox_cli.config import migrate_config
...
def _sessions_export(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from cyberfox_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="cyberfox_cli.subcommands.profile",
...
def _cron_list(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from cyberfox_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from cyberfox_cli.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from cyberfox console")
...
def _cron_resume(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def _cron_run(_engine: CyberfoxConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``cyberfox console`` REPL."""
    ...
    engine = CyberfoxConsoleEngine()
    if interactive:
        print("Cyberfox Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("cyberfox> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
