...
from ``cyberfox_constants`` or other cyberfox_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$CYBERFOX_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``cyberfox_constants.get_cyberfox_home`` semantics: env var wins,
    else ``~/.cyberfox``. A local copy avoids an import cycle with the
    middleware which lives below ``cyberfox_cli``.
    """
    home = os.environ.get("CYBERFOX_HOME") or str(Path.home() / ".cyberfox")
