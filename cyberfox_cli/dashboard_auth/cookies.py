...
Three cookies in play:
  - cyberfox_session_at:   the OAuth access token
  ...
  - cyberfox_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/cyberfox) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/cyberfox`` covers
...
setter wrote ``__Secure-cyberfox_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "cyberfox_session_at"
SESSION_RT_COOKIE = "cyberfox_session_rt"
PKCE_COOKIE = "cyberfox_session_pkce"
...
SSO_ATTEMPT_COOKIE = "cyberfox_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /cyberfox`` we want ``Path=/cyberfox`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/cyberfox``)
