...
from cyberfox_cli.dashboard_auth import list_session_providers
from cyberfox_cli.dashboard_auth.audit import AuditEvent, audit_log
from cyberfox_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
...
from cyberfox_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /cyberfox``, the
    ``login_url`` is prefixed (``/cyberfox/login?next=...``) so the
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from cyberfox_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["cyberfox.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``cyberfox_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # a session hits one provider first, which
        # tries to reach its IDP's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        from cyberfox_cli.dashboard_auth.cookies import clear_session_cookies
        from cyberfox_cli.dashboard_auth.prefix import prefix_from_request
