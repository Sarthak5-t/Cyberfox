"""``cyberfox debug`` debug tools for Cyberfox Agent.

Currently supports:
    cyberfox debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.cyberfox/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[cyberfox debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.cyberfox/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``cyberfox debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``cyberfox debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `cyberfox debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "cyberfox-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``cyberfox debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.cyberfox/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``cyberfox debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "cyberfox-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----CyberfoxDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from cyberfox_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_cyberfox_home() / "logs" / filename) if filename else None
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``cyberfox debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``cyberfox dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``cyberfox dump`` is run
        internally.
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by the paste.rs path)
# ---------------------------------------------------------------------------
...
def collect_share_bundle(
    ...
    This is the single source of collection + redaction used by the paste.rs
    path (:func:`build_debug_share`).  Centralising it guarantees the bundle is
    built from force-redacted snapshots — redaction is the safety boundary.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``cyberfox debug share`` (CLI) and the
    ...
    # shared collector so the paste.rs path builds the bundle. The dump header
    # + redaction banner are applied inside collect_share_bundle.
    bundle = collect_share_bundle(log_lines=log_lines, redact=redact)

    if redact:
        logger.info(
            "cyberfox debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `cyberfox debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  cyberfox debug delete <url>")

    print("\nShare these links with the Cyberfox team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: cyberfox debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'cyberfox debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``cyberfox debug`` call.
    ...
    # best-effort — any failure is swallowed so ``cyberfox debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: cyberfox debug <command>")
        print()
        print("Commands:")
