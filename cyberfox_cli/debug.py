...
_NOUS_BUNDLE_FORMAT = "cyberfox-debug-share/1"
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Cyberfox version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``cyberfox debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from cyberfox_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "cyberfox debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `cyberfox debug share --local` to print the report instead, "
            "or `cyberfox debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
