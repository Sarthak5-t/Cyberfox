"""
Doctor command for cyberfox CLI.

Diagnoses issues with Cyberfox Agent setup.
"""
...
from cyberfox_cli.config import get_project_root, get_cyberfox_home, get_env_path
from cyberfox_cli.env_loader import load_cyberfox_dotenv
from cyberfox_constants import display_cyberfox_home
from cyberfox_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
CYBERFOX_HOME = get_cyberfox_home()
_DHH = display_cyberfox_home()  # user-facing display path (e.g. ~/.cyberfox or ~/.cyberfox/profiles/coder)

# Load environment variables from ~/.cyberfox/.env so API key checks work
_env_path = get_env_path()
load_cyberfox_dotenv(cyberfox_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from cyberfox_cli.colors import Colors, color
from cyberfox_cli.models import _CYBERFOX_USER_AGENT
from cyberfox_constants import OPENROUTER_MODELS_URL
...
from cyberfox_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.cyberfox/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("CYBERFOX_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from cyberfox_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches cyberfox_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``cyberfox --version`` reporting a stale version while
    ...
    """
    try:
        from cyberfox_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != cyberfox_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'cyberfox update', or set "
            "cyberfox_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-cyberfox and dashboard static services are up
    ...
    for static in ("main-cyberfox", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `cyberfox profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from cyberfox_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    if os.environ.get("CYBERFOX_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via CYBERFOX_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `cyberfox`.
    os.environ.setdefault("CYBERFOX_INTERACTIVE", "1")

    # Handle `cyberfox doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.cyberfox/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Cyberfox Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`cyberfox doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and cyberfox_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.cyberfox/.env (primary location for user config)
    env_path = CYBERFOX_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'cyberfox setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'cyberfox setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'cyberfox setup' to create one")
                issues.append("Run 'cyberfox setup' to create .env")
    
    # Check ~/.cyberfox/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = CYBERFOX_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from cyberfox_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from cyberfox_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'cyberfox config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from cyberfox_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.cyberfox/.env or run 'cyberfox setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'cyberfox setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'cyberfox config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from cyberfox_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'cyberfox setup' to migrate config")
                else:
                    issues.append("Run 'cyberfox doctor --fix' or 'cyberfox setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'cyberfox doctor --fix'")
        except Exception:
            pass

        # Detect stale CYBERFOX_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives CYBERFOX_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from cyberfox_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("CYBERFOX_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"CYBERFOX_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `cyberfox setup` run)",
                )
                if should_fix:
                    if remove_env_value("CYBERFOX_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale CYBERFOX_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove CYBERFOX_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the CYBERFOX_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale CYBERFOX_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'cyberfox doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Cyberfox' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = cyberfox_home / subdir_name
    ...
    soul_path = cyberfox_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Cyberfox a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Cyberfox Agent Persona\n\n"
                "<!-- Edit this file to customize how Cyberfox communicates. -->\n\n"
                "You are Cyberfox, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = cyberfox_home / "memories"
    ...
    state_db_path = cyberfox_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from cyberfox_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'cyberfox doctor --fix' "
                        "(or 'cyberfox sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from cyberfox_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'cyberfox doctor --fix' "
                        "(or 'cyberfox sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = cyberfox_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'cyberfox doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "cyberfox"
        ...
        _cmd_link = _cmd_link_dir / "cyberfox"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(cyberfox not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/cyberfox → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/cyberfox points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/cyberfox → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/cyberfox — run 'cyberfox doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/cyberfox exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/cyberfox not found",
                    "(cyberfox command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/cyberfox → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/cyberfox symlink — run 'cyberfox doctor --fix'")
    ...
    try:
        from cyberfox_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_path.exists():
            ...
            # `cyberfox update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `cyberfox doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable CYBERFOX_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Cyberfox one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'cyberfox config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _CYBERFOX_USER_AGENT,
            }
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``cyberfox doctor``.
    ...
    # never the legitimate source for `cyberfox doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'cyberfox setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = CYBERFOX_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: cyberfox skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = CYBERFOX_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: cyberfox memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: cyberfox memory setup",
                    "No Honcho API key — run 'cyberfox memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run cyberfox memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: cyberfox memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: cyberfox memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from cyberfox_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "cyberfox -p" in content:
                            _m = _re.search(r"cyberfox -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'cyberfox doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
