"""
Dump command for cyberfox CLI.

Outputs a compact, plain-text summary of the user's Cyberfox setup
...
from cyberfox_cli.config import get_cyberfox_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.cyberfox/.env.
    ...
    exports. ``cyberfox debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.cyberfox_build_sha`` by the Dockerfile's
    ``CYBERFOX_GIT_SHA`` build-arg (see ``cyberfox_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``cyberfox dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from cyberfox_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(cyberfox_home: Path) -> int:
...
def _cron_summary(cyberfox_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = cyberfox_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"cyberfox_home:      {display_cyberfox_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.cyberfox/.env: a managed
        ...
        # A credential added via `cyberfox auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `cyberfox auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["cyberfox-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(cyberfox_home)}")
    lines.append(f"  skills:             {_count_skills(cyberfox_home)}")
