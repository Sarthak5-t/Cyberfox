"""
cyberfox fallback — manage the fallback provider chain.
...
Subcommands:
  ...
  cyberfox fallback add      Pick provider + model via the same picker as `cyberfox model`,
                           then append the selection to the chain
  cyberfox fallback remove   Pick an entry to delete from the chain
  cyberfox fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.cyberfox/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from cyberfox_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `cyberfox model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  cyberfox fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `cyberfox model`, then append the selection to the chain."""
    from cyberfox_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `cyberfox model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `cyberfox fallback list` to view, or `cyberfox fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``cyberfox fallback [subcommand]``."""
