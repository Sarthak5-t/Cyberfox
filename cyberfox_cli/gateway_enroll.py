"""``cyberfox gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `cyberfox gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    save_env_value writes them to ~/.cyberfox/.env (0600 dir) and never logs.
    to_write = {
