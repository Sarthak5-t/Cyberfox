"""
Cyberfox Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``cyberfox desktop`` / ``cyberfox gui``)
     Built inside the agent checkout under ``$CYBERFOX_HOME/cyberfox-agent/``:
       ...
       - ``$CYBERFOX_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Cyberfox.app`` or ``~/Applications/Cyberfox.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Cyberfox`` (NSIS per-user)
...
the app name ("Cyberfox"), separate from ``$CYBERFOX_HOME``:
  - macOS:   ``~/Library/Application Support/Cyberfox``
  - Windows: ``%APPDATA%\\Cyberfox``
  - Linux:   ``$XDG_CONFIG_HOME/Cyberfox`` (default ``~/.config/Cyberfox``)
...
``cyberfox uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(cyberfox_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return cyberfox_home / "cyberfox-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Cyberfox"
...
def source_built_gui_artifacts(cyberfox_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``cyberfox desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``cyberfox-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(cyberfox_home)
    ...
    return [
        ...
        cyberfox_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Cyberfox" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
...
def agent_is_installed(cyberfox_home: Path) -> bool:
    """Return True when a usable Python agent install exists under CYBERFOX_HOME.
    ...
    if (agent_root / "cyberfox_cli").is_dir():
        return True
...
def gui_is_installed(cyberfox_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(cyberfox_home):
        if p.exists():
            return True
...
def gui_install_summary(cyberfox_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = cyberfox_home if cyberfox_home is not None else get_cyberfox_home()
...
def uninstall_gui(cyberfox_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``cyberfox-agent/cyberfox_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$CYBERFOX_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove cyberfox' or "
            "'sudo dnf remove cyberfox'). AppImage builds are a single file you "
