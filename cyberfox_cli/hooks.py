"""cyberfox hooks — inspect and manage shell-script hooks.

Usage::

    cyberfox hooks list
    cyberfox hooks test <event> [--for-tool X] [--payload-file F]
    cyberfox hooks revoke <command>
    cyberfox hooks doctor

Consent records live under ``~/.cyberfox/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.cyberfox/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``cyberfox hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: cyberfox hooks {list|test|revoke|doctor}")
        print("Run 'cyberfox hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.cyberfox/config.yaml.")
        print("See `cyberfox hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `cyberfox hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `cyberfox hooks test` and `cyberfox hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Cyberfox wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `cyberfox hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `cyberfox hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `cyberfox hooks doctor`.")
