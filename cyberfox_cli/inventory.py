...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (some provider /models
  endpoints return hundreds of IDs including TTS, embeddings, rerankers,
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing so the
      GUI picker can show $/Mtok columns — mirroring the ``cyberfox model``
      CLI picker. Adds network calls (pricing fetch); only set for
      interactive pickers.
    ...
    try:
        from cyberfox_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def _append_unconfigured_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    from cyberfox_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            else f"run `cyberfox model` to configure ({auth_type})"
        )
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`cyberfox_cli.model_switch.list_picker_providers`) so the
