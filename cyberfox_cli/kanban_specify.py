...
Used by ``cyberfox kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``cyberfox_cli/goals.py`` — same aux
...
CYBERFOX_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("CYBERFOX_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Cyberfox Agent board.
...
def _profile_author() -> str:
    """Mirror of ``cyberfox_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("CYBERFOX_PROFILE")
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=CYBERFOX_KANBAN_SPECIFY_MAX_TOKENS,
