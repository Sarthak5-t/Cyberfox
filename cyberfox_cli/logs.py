"""``cyberfox logs`` — view and filter Cyberfox log files.
...
Usage examples::
    ...
    cyberfox logs                    # last 50 lines of agent.log
    cyberfox logs -f                 # follow agent.log in real time
    cyberfox logs errors             # last 50 lines of errors.log
    cyberfox logs gateway -n 100    # last 100 lines of gateway.log
    cyberfox logs gui -f            # follow gui.log (dashboard/pty/ws)
    cyberfox logs desktop -f        # follow desktop.log (Electron app boot/backend)
    cyberfox logs --level WARNING    # only WARNING+ lines
    cyberfox logs --session abc123   # filter by session ID substring
    cyberfox logs --component tools  # only tool-related lines
    cyberfox logs --since 1h         # lines from the last hour
    cyberfox logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_cyberfox_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Cyberfox runs — try 'cyberfox chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_cyberfox_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_cyberfox_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_cyberfox_home()}/logs/")
        return

    print(f"Log files in {display_cyberfox_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'cyberfox chat' to generate logs)")
