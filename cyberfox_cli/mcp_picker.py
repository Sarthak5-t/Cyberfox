"""MCP picker — interactive `cyberfox mcp picker` (also the default `cyberfox mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``cyberfox mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `cyberfox plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from cyberfox_cli.cli_output import prompt_yes_no
from cyberfox_cli.curses_ui import curses_single_select
from cyberfox_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Cyberfox session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from cyberfox_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `cyberfox mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: cyberfox mcp install <name>    Picker: cyberfox mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Cyberfox is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Cyberfox — run `cyberfox update` "
...
def show_catalog() -> None:
    """`cyberfox mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`cyberfox mcp picker` (and default `cyberfox mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`cyberfox mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `cyberfox mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
