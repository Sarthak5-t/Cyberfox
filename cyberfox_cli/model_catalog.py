...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.cyberfox/cache/model_catalog.json``.
...
def _cache_path() -> Path:
    ...
    return get_cyberfox_home() / "cache" / "model_catalog.json"
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``cyberfox update`` pulls the latest repo, so the freshly-pulled
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``cyberfox model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
