...
- ``cyberfox_cli.providers``        -- canonical provider identity + overlays
- ``cyberfox_cli.model_normalize``  -- per-provider name formatting
...
_CYBERFOX_MODEL_WARNING = (
    "Sarthak Khatal Cyberfox 3 & 4 models are NOT agentic and are not designed "
    "for use with Cyberfox Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Sarthak Khatal Cyberfox 3 / Cyberfox 4 chat families.
# The previous substring check (`"cyberfox" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``cyberfox-brain:qwen3-14b-ctx16k`` that just
# happen to carry "cyberfox" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Cyberfox-3-Llama-3.1-70B, cyberfox-4-405b, openrouter/cyberfox3:70b
# Negative examples it must NOT match:
#   cyberfox-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_CYBERFOX_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])cyberfox[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_cyberfox_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Cyberfox 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_CYBERFOX_NON_AGENTIC_RE.search(model_name))


def _check_cyberfox_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Cyberfox 3/4 chat model."""
    if is_nous_cyberfox_non_agentic(model_name):
        return _CYBERFOX_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``cyberfox config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from cyberfox_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'cyberfox model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'cyberfox doctor' — config issues detected:"
        ...
        from cyberfox_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from cyberfox_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from cyberfox_cli.config import load_config
            from cyberfox_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from cyberfox_cli.providers import resolve_user_provider as _ruser
    ...
    # cyberfox_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from cyberfox_cli.models import normalize_opencode_base_url
    ...
    cyberfox_warn = _check_cyberfox_model_warning(new_model)
    if cyberfox_warn:
        warnings.append(cyberfox_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from cyberfox_cli.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from cyberfox_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from cyberfox_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from cyberfox_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by cyberfox provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://cyberfox-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Cyberfox release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from cyberfox_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from cyberfox_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Cyberfox-mapped providers ---
    from cyberfox_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from cyberfox_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for cyberfox_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(cyberfox_id)
        if (
            _alias_target
            and _alias_target != cyberfox_id
        ...
        pconfig = PROVIDER_REGISTRY.get(cyberfox_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (CYBERFOX_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from cyberfox_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(cyberfox_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `cyberfox model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(cyberfox_id)
        if not model_ids:
            model_ids = curated.get(cyberfox_id, [])
            if cyberfox_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(cyberfox_id, model_ids)
        total = len(model_ids)
        if cyberfox_id in _UNCAPPED_PICKER_PROVIDERS:
    ...
    # --- 2. Check Cyberfox-only providers (nous, openai-codex, copilot, opencode-go) ---
    from cyberfox_cli.providers import CYBERFOX_OVERLAYS
    from cyberfox_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Cyberfox provider ID.
    # CYBERFOX_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Cyberfox IDs ("copilot").
    _mdev_to_cyberfox = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in CYBERFOX_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Cyberfox slug — e.g. "github-copilot" → "copilot"
        cyberfox_slug = _mdev_to_cyberfox.get(pid, pid)
        if cyberfox_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(cyberfox_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, cyberfox_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or cyberfox_slug in providers_store):
                    has_creds = True
        ...
        # configured.
        if not has_creds and cyberfox_slug == "anthropic":
            try:
                ...
                if (cyberfox_creds and cyberfox_creds.get("accessToken")) or \
        ...
        if cyberfox_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(cyberfox_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(cyberfox_slug)
                model_ids = _ids if _ids else (curated.get(cyberfox_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(cyberfox_slug, []) or curated.get(pid, [])
        elif cyberfox_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `cyberfox model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from cyberfox_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(cyberfox_slug, []) or curated.get(pid, [])
                if cyberfox_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(cyberfox_slug, model_ids)
        total = len(model_ids)
        if cyberfox_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": cyberfox_slug,
            "name": get_label(cyberfox_slug),
            "is_current": cyberfox_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "cyberfox",
        ...
        seen_slugs.add(cyberfox_slug.lower())
        _record_builtin_endpoint(cyberfox_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or CYBERFOX_OVERLAYS (keeps /model in sync
    # with `cyberfox model`).
    try:
        from cyberfox_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Cyberfox's canonical write key (matches
            ...
            # Cyberfox writes ``models:`` as a dict keyed by model id, but older
            ...
            if should_probe:
                try:
                    from cyberfox_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Cyberfox's own writer uses; a hyphen variant is accepted
            ...
            # active model. Cyberfox's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, cyberfox_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from cyberfox_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`cyberfox_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from cyberfox_cli.models import fetch_openrouter_models
