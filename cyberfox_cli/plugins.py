"""
Cyberfox Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with cyberfox-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.cyberfox/plugins/<name>/``
3. **Project plugins** – ``./.cyberfox/plugins/<name>/`` (opt-in via
   ``CYBERFOX_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``cyberfox_agent.plugins``
   entry-point group.
...
from cyberfox_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``CYBERFOX_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("CYBERFOX_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``CYBERFOX_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.cyberfox/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("CYBERFOX_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When CYBERFOX_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Cyberfox loggers.
    """
    ...
    logger.debug(
        "CYBERFOX_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Cyberfox' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by cyberfox_cli.kanban_db when a task
    ...
    # separate `cyberfox -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `cyberfox kanban dispatch`),
...
ENTRY_POINTS_GROUP = "cyberfox_agent.plugins"

_NS_PARENT = "cyberfox_plugins"
...
class PluginManifest:
    ...
    #              in ~/.cyberfox/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``cyberfox plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Cyberfox profile name (e.g. ``"default"``).

        Derived from ``CYBERFOX_HOME`` via
        :func:`cyberfox_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.cyberfox/profiles/<name>``, or ``"custom"`` when
        ``CYBERFOX_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Cyberfox core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``cyberfox honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``cyberfox <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`cyberfox_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from cyberfox_cli.dashboard_auth import (
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_cyberfox_dotenv()`` startup — after
        ``~/.cyberfox/.env`` loads, before Cyberfox reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Cyberfox' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``cyberfox model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: cyberfox_cli.main imports plugins indirectly
        from cyberfox_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.cyberfox/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        if env_var_enabled("CYBERFOX_SAFE_MODE"):
            logger.info("CYBERFOX_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to cyberfox_cli/. Two layouts are
        ...
        # 2. User plugins (~/.cyberfox/plugins/)
        user_dir = get_cyberfox_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.cyberfox/plugins/)
        if _env_enabled("CYBERFOX_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".cyberfox" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set CYBERFOX_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with cyberfox and must
            ...
            # all ~20 of them added several seconds to every `cyberfox`
            # invocation — including plain `cyberfox chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Cyberfox ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `cyberfox plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``cyberfox plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`cyberfox plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `cyberfox plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``cyberfox_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``cyberfox_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="cyberfox-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``cyberfox tools`` TUI so plugin-provided toolsets appear
