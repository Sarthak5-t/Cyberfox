"""Profile distributions — shareable, packaged Cyberfox profiles via git.

A distribution is a Cyberfox profile published as a git repository (or
...
* ``cyberfox profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``cyberfox skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``cyberfox profile``, not a parallel tree):

    cyberfox profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "cyberfox_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".cyberfox_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "cyberfox-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            cyberfox_requires=str(data.get("cyberfox_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.cyberfox_requires:
            out["cyberfox_requires"] = self.cyberfox_requires
        if self.author:
...
def check_cyberfox_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Cyberfox {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``cyberfox profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Cyberfox profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Cyberfox distribution."
        )

    # Version check up-front so we fail fast
    check_cyberfox_requires(manifest.cyberfox_requires, cyberfox_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.cyberfox).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="cyberfox_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `cyberfox profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`cyberfox profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="cyberfox_dist_update_") as tmp:
        plan = plan_install(
