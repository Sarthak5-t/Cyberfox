...
# xAI's public API is OpenAI-compatible for the endpoints Cyberfox commonly
# uses. The Responses endpoint is included because Cyberfox' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Cyberfox-managed OAuth credentials."""

    auth_hint = "cyberfox auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`cyberfox auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`cyberfox auth reset xai-oauth` or re-authenticate with "
                    "`cyberfox auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `cyberfox auth add xai-oauth --type oauth`."
            )
