"""
Unified self-relaunch for Cyberfox CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``cyberfox sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``cyberfox`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from cyberfox_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched cyberfox."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_cyberfox_bin() -> Optional[str]:
    """Find the cyberfox entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("cyberfox")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m cyberfox_cli.main``.
    ...
    ``cyberfox.exe`` on PATH or the ``sys.executable -m cyberfox_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with cyberfox.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_cyberfox_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "cyberfox_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh cyberfox invocation.
    ...
    relaunch contract wants: "run cyberfox again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``cyberfox.exe`` (a Python console-script shim that wraps
    ``python -m cyberfox_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "cyberfox exited, then
    new cyberfox started" — just with two PIDs in play instead of one.
    """
