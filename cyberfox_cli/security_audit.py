"""On-demand supply-chain audit for Cyberfox Agent installs.

Scans three surfaces a Cyberfox user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Cyberfox venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.cyberfox/plugins``
...
def _discover_plugins(cyberfox_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.cyberfox/plugins``.
...
def run_audit(
    *,
    ...
    home = cyberfox_home or Path(get_cyberfox_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, cyberfox_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(cyberfox_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `cyberfox security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, cyberfox_home=home
    )
    if total == 0:
