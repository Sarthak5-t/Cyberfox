...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``cyberfox gateway start/stop/restart``).
    """
    ...
    # `cyberfox gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged cyberfox
    ...
    detection always fails. Since every Cyberfox runtime call inside the
    container drops to cyberfox via ``s6-setuidgid``, that silent failure
...
# in ``cyberfox_cli.gateway`` (systemd/launchd) and ``cyberfox_cli.gateway_windows``
...
# active (set via the ``cyberfox -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in cyberfox_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from cyberfox_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from cyberfox_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from cyberfox_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from cyberfox_cli.gateway import _probe_systemd_service_running
...
#
# Per-profile gateways are registered dynamically when `cyberfox profile create`
# runs inside the container (Phase 4). Static services (main-cyberfox, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's CYBERFOX_HOME blindly;
    derive the shared profile root from the current CYBERFOX_HOME and map the
    ...
    cyberfox_home = Path(os.environ.get("CYBERFOX_HOME", "/opt/data"))
    if cyberfox_home.parent.name == "profiles":
        root = cyberfox_home.parent.parent
    else:
        root = cyberfox_home
...
# UID/GID of the in-image ``cyberfox`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the cyberfox user.
    ...
    (the ``cyberfox`` user — UID 10000 — running every Cyberfox runtime
    ...
    is dispatched as the cyberfox user.
    ...
    with cyberfox ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           cyberfox:cyberfox, 0755 (parent must already exist)
    ``svc_dir/event/``                     cyberfox:cyberfox, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 cyberfox:cyberfox, 0755
    ``svc_dir/supervise/event/``           cyberfox:cyberfox, 03730
    ``svc_dir/supervise/control``          cyberfox:cyberfox, 0660    (FIFO)
    ...
    slot's supervise/ was cyberfox-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _CYBERFOX_UID, _CYBERFOX_GID)
        except PermissionError:
            # Running as the cyberfox user already — directory is cyberfox-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _CYBERFOX_UID, _CYBERFOX_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is cyberfox-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _CYBERFOX_UID, _CYBERFOX_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``cyberfox -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`cyberfox profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-cyberfox, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources CYBERFOX_HOME (and any extra env) via with-contenv —
             so e.g. ``-e CYBERFOX_HOME=/data/cyberfox`` is honored at run
          ...
          4. Drops to the cyberfox user and exec's
             ``cyberfox -p <profile> gateway run`` (or just ``cyberfox
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``cyberfox gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root CYBERFOX_HOME profile" (the implicit profile that exists at
        the top of $CYBERFOX_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``cyberfox_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$CYBERFOX_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``CYBERFOX_HOME``,
        ...
        allocator in ``cyberfox_cli.profiles._allocate_gateway_port``).
        ...
        lines = [
            ...
            "cd /opt/data",
            ". /opt/cyberfox/.venv/bin/activate",
        ]
        ...
        # guard.
        lines.append("export CYBERFOX_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's CYBERFOX_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``cyberfox gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-CYBERFOX_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The CYBERFOX_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own CYBERFOX_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "cyberfox gateway run --replace"
        else:
            gateway_cmd = f"cyberfox -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid cyberfox {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${CYBERFOX_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the CYBERFOX_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e CYBERFOX_HOME=/data/cyberfox``
        gets its logs under /data/cyberfox/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``cyberfox logs`` reads and what
        ...
        return (
            ...
            f': "${{CYBERFOX_HOME:=/opt/data}}"\n'
            f'log_dir="$CYBERFOX_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as cyberfox and crash-loops on
            ...
            f'chown cyberfox:cyberfox "$CYBERFOX_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R cyberfox:cyberfox "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid cyberfox s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``cyberfox gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``cyberfox -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with cyberfox ownership
            ...
            # the cyberfox user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is cyberfox-owned (see ``_seed_supervise_skeleton``), and on
