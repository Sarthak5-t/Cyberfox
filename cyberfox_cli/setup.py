"""
Interactive setup wizard for Cyberfox Agent.
...
_DOCS_BASE = "https://cyberfox-agent.nousresearch.com/docs"
...
# display_cyberfox_home imported lazily at call sites (stale-module safety during cyberfox update)
...
from cyberfox_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from cyberfox_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Cyberfox Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Cyberfox using environment variables or config commands:")
    print_info("  cyberfox config set model.provider custom")
    print_info("  cyberfox config set model.base_url http://localhost:8080/v1")
    print_info("  cyberfox config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'cyberfox setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``CYBERFOX_NONINTERACTIVE=1`` (see ``cyberfox_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("CYBERFOX_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``CYBERFOX_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from cyberfox_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'cyberfox setup')")


def _print_setup_summary(config: dict, cyberfox_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'cyberfox setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from cyberfox_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `cyberfox tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from cyberfox_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'cyberfox setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'cyberfox setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'cyberfox setup terminal'"))
    ...
    # Spotify (OAuth via cyberfox auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'cyberfox setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {cyberfox_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('cyberfox setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('cyberfox setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('cyberfox setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('cyberfox setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('cyberfox setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('cyberfox config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('cyberfox config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('cyberfox config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('cyberfox', Colors.GREEN)}              Start chatting")
    print(f"   {color('cyberfox gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('cyberfox doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `cyberfox tools` and `cyberfox setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``cyberfox model``)
    ...
    provider added to ``cyberfox model`` is automatically available here.
    ...
    # Delegate to the shared cyberfox model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from cyberfox_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: cyberfox model")
    ...
    # on demand via `cyberfox auth add`, `cyberfox setup` vision, and
    # `cyberfox setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from cyberfox_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``cyberfox model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from cyberfox_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.cyberfox/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Cyberfox auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via cyberfox setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'cyberfox setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Cyberfox runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `cyberfox setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `cyberfox setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into CYBERFOX_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("CYBERFOX_MAX_ITERATIONS")
    ...
    print_info("  Run `cyberfox setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # CYBERFOX_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.cyberfox/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from cyberfox_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_cyberfox_home(Path(get_cyberfox_home()))
    except Exception:
        pass
...
def _profile_name_from_cyberfox_home(cyberfox_home) -> str | None:
    """Return the active profile name when CYBERFOX_HOME is a profile dir."""
    if cyberfox_home.parent.name == "profiles":
        return cyberfox_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Cyberfox delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Cyberfox to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from cyberfox_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://cyberfox-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://cyberfox-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  cyberfox config edit")
    print_info("   Open config in your editor:  cyberfox config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from cyberfox_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Cyberfox from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'cyberfox setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     cyberfox config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )

        # Offer to install the gateway as a system service
        import platform as _platform

        _is_linux = True  # Cyberfox is Linux-only
        ...
        supports_service_manager = supports_systemd
        ...
        if supports_systemd and has_legacy_cyberfox_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:
                    print_info("  Or as a boot-time service: cyberfox gateway install --system")
                print_info("  Or run in foreground:  cyberfox gateway")
        else:
            from cyberfox_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   cyberfox gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   cyberfox gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `cyberfox setup tools` and `cyberfox tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``cyberfox_cli.auth`` — lists every supported
    ...
    """
    try:
        from cyberfox_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from cyberfox_cli.config
    ...
    if section_key == "model":
        ...
        from cyberfox_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_cyberfox.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_cyberfox migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_cyberfox", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Cyberfox.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Cyberfox to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Cyberfox at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Cyberfox at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Cyberfox at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Cyberfox at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Cyberfox equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Cyberfox config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Cyberfox.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Cyberfox's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(cyberfox_home: Path) -> bool:
    ...
    print_info("Cyberfox can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: cyberfox claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=cyberfox_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: cyberfox claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Cyberfox configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Cyberfox config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Cyberfox (use cyberfox claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``cyberfox setup --portal`` and ``cyberfox portal``. This is the
    ...
    Cyberfox session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``cyberfox setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``cyberfox model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``cyberfox portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Cyberfox Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `cyberfox portal` == quick setup's Nous step.
    try:
        from cyberfox_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `cyberfox portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `cyberfox portal`: %s", exc)
        print()
    ...
    print_info("  Run `cyberfox portal info` to inspect routing.")
    print_info("  Run `cyberfox` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      cyberfox setup           — full or quick (auto-detected)
      cyberfox setup model     — just model/provider
      cyberfox setup tts       — just text-to-speech
      cyberfox setup terminal  — just terminal backend
      cyberfox setup gateway   — just messaging platforms
      cyberfox setup tools     — just tool configuration
      cyberfox setup agent     — just agent settings
    """
    from cyberfox_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Cyberfox Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Cyberfox Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Cyberfox Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, cyberfox_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Cyberfox configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'cyberfox setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(cyberfox_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Cyberfox?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, cyberfox_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, cyberfox_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {cyberfox_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'cyberfox config edit'")
    ...
    # Tune later with `cyberfox setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, cyberfox_home)


def _run_first_time_quick_setup(config: dict, cyberfox_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``cyberfox setup <section>``
    or switch providers with ``cyberfox model``.
    """
    from cyberfox_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'cyberfox setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    cyberfox setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  cyberfox setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``cyberfox tools`` (which
    ...
    try:
        ...
        from cyberfox_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``cyberfox setup agent`` /
    ``cyberfox config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, cyberfox_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `cyberfox update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        cyberfox tools")
        print_info("  Seed skills:         cyberfox skills opt-in --sync")
        print_info("  Add MCP servers:     cyberfox mcp add")
        print_info("  Enable plugins:      cyberfox plugins")
        print_info("  Tune agent settings: cyberfox setup agent")
        print()
        _print_setup_summary(config, cyberfox_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, cyberfox_home)


def _blank_slate_walkthrough(config: dict, cyberfox_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`cyberfox update` runs from re-injecting them. Opt back in any")
            print_info("time with `cyberfox skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `cyberfox tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `cyberfox plugins list` / `cyberfox plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `cyberfox plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `cyberfox mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `cyberfox mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   cyberfox tools")
    print_info("  Seed skills:         cyberfox skills opt-in --sync")
    print_info("  Add MCP servers:     cyberfox mcp add")
    print_info("  Tune agent settings: cyberfox setup agent")
    print()
...
def _run_quick_setup(config: dict, cyberfox_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'cyberfox setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Cyberfox to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'cyberfox setup gateway'.")
