"""``cyberfox slack ...`` CLI subcommands.

Today only ``cyberfox slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ cyberfox slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Cyberfox. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Cyberfox deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from cyberfox_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Cyberfox in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Cyberfox agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``cyberfox_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$CYBERFOX_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Cyberfox")
    ...
    """
    name = getattr(args, "name", None) or "Cyberfox"
    description = getattr(args, "description", None) or "Your Cyberfox agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("CYBERFOX_HOME") or str(Path.home() / ".cyberfox")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Cyberfox app\n"
