...
on Japanese locales, etc.).  Cyberfox's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``cyberfox_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Cyberfox is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``CYBERFOX_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("CYBERFOX_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Cyberfox-managed Git install directories so
    ...
    # already-running shells don't see the change — which means cyberfox
    ...
    # install without Cyberfox-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Cyberfox's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Cyberfox's
    ``cyberfox config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Cyberfox (User env var in Windows Settings, or
    export in a PowerShell profile) and Cyberfox picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Cyberfox-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\cyberfox\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``cyberfox`` from right after install) retain their old PATH.

    Any subprocess Cyberfox spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Cyberfox-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "cyberfox", "git", "cmd"),
        os.path.join(local_appdata, "cyberfox", "git", "bin"),
        os.path.join(local_appdata, "cyberfox", "git", "usr", "bin"),
        # Cyberfox venv Scripts directory — host of the cyberfox.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes cyberfox, but harmless to include.
        os.path.join(local_appdata, "cyberfox", "cyberfox-agent", "venv", "Scripts"),
