"""``cyberfox dashboard`` / ``cyberfox serve`` subcommand parsers.
...
``cyberfox_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `cyberfox dashboard` / `cyberfox serve` cmdlines and SIGTERM them directly —
    # the same path `cyberfox update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Cyberfox web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Cyberfox web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Cyberfox Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Cyberfox desktop app shells (<= 0.15.x) spawn the
    # backend as `cyberfox dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Cyberfox couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Cyberfox Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Cyberfox backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Cyberfox backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports CYBERFOX_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `cyberfox dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.cyberfox/.env.
    # Nested subparser so bare `cyberfox dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "CYBERFOX_DASHBOARD_OAUTH_CLIENT_ID into ~/.cyberfox/.env, and prints "
            "how to engage the login gate. Requires being logged in (cyberfox setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://cyberfox.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via CYBERFOX_DASHBOARD_PORTAL_URL. Mainly for "
