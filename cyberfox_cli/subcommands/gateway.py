"""``cyberfox gateway`` and ``cyberfox proxy`` subcommand parsers.

Extracted verbatim from ``cyberfox_cli/main.py:main()`` (god-file Phase 2).
...
from cyberfox_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if CYBERFOX_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy cyberfox.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Cyberfox gateway unit files "
            "(e.g. cyberfox.service) left over from older installs. Profile "
            "units (cyberfox-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Authenticates as your Cyberfox account (the connector derives the "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.cyberfox/.env. "
            "Requires being logged in (cyberfox setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.cyberfox/.env and forwarded at provision. "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. OpenRouter). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="xai",
        help="Upstream provider: xai (default: xai). See `cyberfox proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
