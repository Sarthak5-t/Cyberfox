...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or cyberfox -c.",
    ...
    "Ctrl+Z suspends Cyberfox to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "cyberfox -c resumes your most recent CLI session. cyberfox -c \"project name\" resumes by title.",
    "cyberfox -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "cyberfox -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "cyberfox chat -t web,terminal enables only specific toolsets for a focused session.",
    "cyberfox chat -s github-pr-workflow preloads a skill at launch.",
    "cyberfox chat -q \"query\" runs a single non-interactive query and exits.",
    "cyberfox chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "cyberfox chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "cyberfox --yolo bypasses all dangerous command approval prompts for the entire session.",
    "cyberfox chat --source telegram tags the session for filtering in cyberfox sessions list.",
    "cyberfox -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "cyberfox doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "cyberfox dump outputs a compact setup summary — great for bug reports.",
    "cyberfox config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "cyberfox config edit opens config.yaml in your default editor.",
    "cyberfox config check scans for missing or stale configuration options.",
    "cyberfox sessions browse opens an interactive session picker with search.",
    "cyberfox sessions stats shows session counts by platform and database size.",
    "cyberfox sessions prune --older-than 30 cleans up old sessions.",
    "cyberfox skills search react --source skills-sh searches the skills.sh public directory.",
    "cyberfox skills check scans installed hub skills for upstream updates.",
    "cyberfox skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "cyberfox skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "cyberfox mcp add github --command npx adds MCP servers from the command line.",
    "cyberfox mcp serve runs Cyberfox itself as an MCP server for other agents.",
    "cyberfox auth add lets you add multiple API keys for credential pool rotation.",
    "cyberfox completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "cyberfox logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "cyberfox backup creates a zip backup of your entire Cyberfox home directory.",
    "cyberfox profile create coder creates an isolated profile that becomes its own command.",
    "cyberfox profile create work --clone copies your current config and keys to a new profile.",
    "cyberfox update syncs new bundled skills to ALL profiles automatically.",
    "cyberfox gateway install sets up Cyberfox as a system service (systemd/launchd).",
    "cyberfox memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "cyberfox webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: cyberfox tools disables unused tools, cyberfox skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "cyberfox models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as cyberfox -w).",
    ...
    "CYBERFOX_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Cyberfox tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'cyberfox profile create coder' creates the 'coder' command.",
    "cyberfox profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Cyberfox prints a resume command with session ID and stats.",
    "cyberfox sessions export backup.jsonl exports all sessions for backup or analysis.",
    "cyberfox -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "cyberfox skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .cyberfox/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: cyberfox cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Cyberfox runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "cyberfox gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Cyberfox via a local macOS server.",
    ...
    "Context files (.cyberfox.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Cyberfox loads project context from .cyberfox.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — cyberfox tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.cyberfox/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "cyberfox chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "cyberfox plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "cyberfox acp runs Cyberfox as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "Cron scripts live in ~/.cyberfox/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Cyberfox your own.",
    ...
    "Before a gateway session reset, Cyberfox auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.cyberfox/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Cyberfox state via CYBERFOX_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "CYBERFOX_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in CYBERFOX_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.cyberfox/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'cyberfox profile create ls' would be rejected.",
    "cyberfox profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "cyberfox claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Cyberfox auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Cyberfox config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Cyberfox is working.',
    ...
    '/reload reloads ~/.cyberfox/.env into the running session — pick up new API keys without restarting.',
    ...
    "CYBERFOX_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.cyberfox/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.cyberfox/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'cyberfox curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "cyberfox curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'cyberfox curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    ...
    # --- TUI & Dashboard ---
    'CYBERFOX_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "CYBERFOX_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'cyberfox dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'cyberfox dashboard embeds the full Cyberfox TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.cyberfox/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.cyberfox/dashboard-plugins/ — no npm build required.',
    ...
    'CYBERFOX_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'CYBERFOX_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'CYBERFOX_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'CYBERFOX_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.cyberfox/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.cyberfox/skills/ so drop-in skills appear without restarting the session.',
    ...
    'cyberfox -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'cyberfox chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'cyberfox chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'cyberfox chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "cyberfox chat --source tool tags programmatic chats so they don't clutter cyberfox sessions list.",
    'cyberfox dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'cyberfox sessions rename <ID> "new title" renames any past session; cyberfox sessions delete <ID> removes one.',
    'cyberfox import restores a session export or profile archive produced by sessions export or profile export.',
    'cyberfox fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'cyberfox pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'cyberfox setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'cyberfox status --deep runs the full health sweep across every component; plain cyberfox status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'CYBERFOX_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'CYBERFOX_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.cyberfox/plugins/ — trust-gated by design.',
    "CYBERFOX_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'CYBERFOX_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'CYBERFOX_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'CYBERFOX_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'CYBERFOX_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'CYBERFOX_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'CYBERFOX_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'CYBERFOX_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'CYBERFOX_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'CYBERFOX_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'CYBERFOX_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Cyberfox block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from cyberfox sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.cyberfox/dashboard-plugins/.',
]
