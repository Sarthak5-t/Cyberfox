"""
Unified tool configuration for Cyberfox Agent.

`cyberfox tools` and `cyberfox setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.cyberfox/config.yaml under
...
from cyberfox_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _CYBERFOX_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `cyberfox tools` → Video Generation, which walks
...
# `cyberfox tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from cyberfox_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `cyberfox tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``cyberfox tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from cyberfox_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``cyberfox tools`` checklist actually offers
    for ``platform``.
    ...
    so ``cyberfox tools`` never claims to add or remove a toolset the user was
...
from cyberfox_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Cyberfox routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        # backend, never on the paid CloudBrowserProvider row:
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set CYBERFOX_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "CYBERFOX_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "CYBERFOX_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "CYBERFOX_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "CYBERFOX_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "CYBERFOX_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `cyberfox model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("CYBERFOX_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_driver_env() -> dict:
    """cua-driver child env with the Cyberfox telemetry policy applied.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``cyberfox update`` and
      by ``cyberfox computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `cyberfox update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `cyberfox computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True) -> bool:
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `cyberfox update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `cyberfox update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.cyberfox/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Cyberfox process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_cyberfox_home()}/cyberfox-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in cyberfox-agent directory)")
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.cyberfox/config.yaml")
        ...
        # Run the full `cyberfox auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.cyberfox/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from cyberfox_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: cyberfox auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `cyberfox auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: cyberfox auth spotify")
        except Exception as exc:
        ...
        try:
            from cyberfox_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: cyberfox plugins enable observability/langfuse")
        _print_info("    Restart Cyberfox for tracing to take effect.")
        _print_info("    Verify: cyberfox plugins list")
        ...
        try:
            ...
            _print_info("    Run later: cyberfox auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `cyberfox auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: cyberfox auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: cyberfox auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``cyberfox tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``cyberfox tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: cyberfox tools post-setup <key>")
        return 2
    ...
    *,
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"cyberfox-{platform}"
    ...
    # "cyberfox-cli" (which include all _CYBERFOX_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[cyberfox-cli, spotify]`` after enabling Spotify via ``cyberfox
        ...
        # (e.g. "cyberfox-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``cyberfox tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `cyberfox tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"cyberfox-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("cyberfox-")}
    ...
    # shipped as a bundled plugin but user must opt in via `cyberfox tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `cyberfox tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by cyberfox tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `cyberfox` instead of
    # `cyberfox-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `cyberfox update`/`cyberfox doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `cyberfox tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (cyberfox-cli, cyberfox-telegram, etc.)
    ...
    # Opening `cyberfox tools` is the user's opt-in to reconfigure tools, so treat
...
# ("Managed-gateway entry", "Firecrawl Self-Hosted")
...
# ("Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    # category — the per-provider hardcoded entries were deleted.
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `cyberfox tools` gets a silent no-op
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    NOT prompt for env vars, run post-setup hooks, or run
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, or run interactive model
    ...
    managed_feature = provider.get("managed_feature")
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``cyberfox model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    if not providers:
        _print_warning(
            ...
            "with `cyberfox model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `cyberfox model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `cyberfox tools` and `cyberfox setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Cyberfox Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://github.com/Sarthak5-t/Cyberfox/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from cyberfox_constants import display_cyberfox_home
    print(color(f"  Tool configuration saved to {display_cyberfox_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'cyberfox' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, cyberfox mcp
