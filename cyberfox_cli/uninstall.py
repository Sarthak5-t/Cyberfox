"""
Cyberfox Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.cyberfox/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Cyberfox PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing cyberfox-agent or cyberfox PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Cyberfox Agent" comment and following line
                if '# Cyberfox Agent' in line or '# cyberfox-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('cyberfox' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing cyberfox
                if 'cyberfox' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the cyberfox wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "cyberfox",
        Path("/usr/local/bin/cyberfox"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains cyberfox_cli reference)
                content = wrapper.read_text()
                if 'cyberfox_cli' in content or 'cyberfox-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(cyberfox_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``cyberfox`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Cyberfox home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (cyberfox_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``cyberfox gateway run`` processes
    ...
    try:
        from cyberfox_cli.gateway import kill_gateway_processes, find_gateway_pids
...
#
#   1. Sets User-scope env vars ``CYBERFOX_HOME`` and ``CYBERFOX_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\cyberfox\git\cmd``, ``%LOCALAPPDATA%\cyberfox\git\bin``,
#      ``%LOCALAPPDATA%\cyberfox\git\usr\bin``, ``%LOCALAPPDATA%\cyberfox\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\cyberfox\git\`` and Node to
#      ``%LOCALAPPDATA%\cyberfox\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``cyberfox dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\cyberfox\gateway-service\`` and sometimes
...
def _cyberfox_path_markers(cyberfox_home: Path) -> list[str]:
    """Path-entry substrings that identify Cyberfox-owned User-PATH entries."""
    root = str(cyberfox_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare cyberfox-agent install dir.
    markers = [root + "\\cyberfox-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if CYBERFOX_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "cyberfox".  We don't want to catch
    # unrelated entries like "ccyberfox-foo" or "ephermeral", so we look for
    # backslash-cyberfox as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(cyberfox_home: Path) -> list[str]:
    """Strip Cyberfox-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _cyberfox_path_markers(cyberfox_home)
            kept: list[str] = []
...
def remove_cyberfox_env_vars_windows() -> list[str]:
    """Delete CYBERFOX_HOME and CYBERFOX_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("CYBERFOX_HOME", "CYBERFOX_GIT_BASH_PATH"):
                try:
...
def _is_default_cyberfox_home(cyberfox_home: Path) -> bool:
    """Return True when ``cyberfox_home`` points at the default (non-profile) root."""
    try:
        from cyberfox_constants import get_default_cyberfox_root
        return cyberfox_home.resolve() == get_default_cyberfox_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its CYBERFOX_HOME directory.

    We shell out to ``cyberfox -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current CYBERFOX_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m cyberfox_cli.main` so we don't depend on a `cyberfox`
    #    wrapper that may be half-removed mid-uninstall.
    cyberfox_invocation = [_sys.executable, "-m", "cyberfox_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                cyberfox_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's CYBERFOX_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``cyberfox uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$CYBERFOX_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from cyberfox_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(cyberfox_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Cyberfox Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Cyberfox Chat GUI installation was found.")
        print(f"  Checked: {cyberfox_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Cyberfox agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(cyberfox_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Cyberfox agent at {cyberfox_home / 'cyberfox-agent'}")
        print(f"  • Your config, sessions, and secrets under {cyberfox_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Cyberfox agent is still installed. Run 'cyberfox' to use the CLI,")
    print("or 'cyberfox uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.cyberfox/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.cyberfox/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie CYBERFOX_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_cyberfox_home(cyberfox_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.cyberfox data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Cyberfox Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {cyberfox_home / 'config.yaml'}")
    print(f"  Secrets: {cyberfox_home / '.env'}")
    print(f"  Data:    {cyberfox_home / 'cron/'}, {cyberfox_home / 'sessions/'}, {cyberfox_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their CYBERFOX_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Cyberfox data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Cyberfox code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, cyberfox_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Cyberfox PATH entries from shell configs / Windows User PATH")
    print("  • Cyberfox wrapper scripts and Cyberfox-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Cyberfox config/data: {cyberfox_home}")
        if _is_default_cyberfox_home(cyberfox_home):
        ...
        print(f"  • Keep Cyberfox config/data: {cyberfox_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``cyberfox`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$CYBERFOX_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    #     (only when they still point into this Cyberfox home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Cyberfox-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(cyberfox_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Cyberfox-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(cyberfox_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE CYBERFOX_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from cyberfox_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(cyberfox_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.cyberfox/, just remove the cyberfox-agent subdir
            if cyberfox_home in project_root.parents or project_root.parent == cyberfox_home:
    ...
    #     under CYBERFOX_HOME but they're install tooling, not config — safe to
    ...
    if full_uninstall:
        ...
        #     alias wrapper. The profile CYBERFOX_HOME dirs live under
        ...
        try:
            if cyberfox_home.exists():
                shutil.rmtree(cyberfox_home)
                log_success(f"Removed {cyberfox_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {cyberfox_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {cyberfox_home}")
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m cyberfox_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import cyberfox_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``cyberfox_constants`` + ``cyberfox_cli.colors``
    (and lazily ``cyberfox_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m cyberfox_cli.uninstall")
    parser.add_argument(
        "--mode",
