...
def get_recommended_default_model(provider: str = ""):
    ...
    sensible default instead of blindly taking the first curated entry. For any
    provider it falls back to the first curated model (same as before).

    Response: {"provider": str, "model": str, "free_tier": bool | None}
    `model` may be empty if nothing could be resolved (caller degrades gracefully).
    """
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # a now-unpaid provider keeps paying 402s
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "email": {
        ...
        "docs_url": "https://github.com/Sarthak5-t/Cyberfox/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "google_chat": {
        ...
        "docs_url": "https://github.com/Sarthak5-t/Cyberfox/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://github.com/Sarthak5-t/Cyberfox/user-guide/messaging/weixin/",
    ...
    "teams": {
        "docs_url": "https://github.com/Sarthak5-t/Cyberfox/user-guide/messaging/teams",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
    ...
    "webhook": {
        ...
        "docs_url": "https://github.com/Sarthak5-t/Cyberfox/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://sarthak5t.github.io/cyberfox-agent"
...
# Anthropic, device-code for OpenAI Codex) still runs in the CLI for now;
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "docs_url": "https://github.com/Sarthak5-t/Cyberfox/guides/xai-grok-oauth",
...
#     1. POST /api/providers/oauth/{openai-codex|minimax|xai}/start
...
def _minimax_poller(session_id: str) -> None:
    ...
    Mirrors the other device-code pollers but calls the MiniMax-specific token
    endpoint, which uses a PKCE-style ``code_verifier`` + ``user_code`` rather
    than the standard ``device_code`` field. On success, builds the same
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    the managed subscription image row instead carries the legacy
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `cyberfox dashboard register` or "
