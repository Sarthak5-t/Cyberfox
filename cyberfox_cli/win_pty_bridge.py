"""Windows ConPTY bridge for the `cyberfox dashboard` chat tab.

Drop-in counterpart to ``cyberfox_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``cyberfox_cli.web_server`` consumes: ``spawn``, ``read``,
