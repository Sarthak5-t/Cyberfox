"""Centralized logging setup for Cyberfox Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.cyberfox/logs/`` (profile-aware via ``get_cyberfox_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``cyberfox_cli.web_server``,
    ``cyberfox_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Cyberfox (TUI, gateway, ``hy_memory`` server, MCP
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_cyberfox_session_injector", False):
    ...
    _session_record_factory._cyberfox_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``cyberfox logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``cyberfox logs --component gateway``.
    "gateway": ("gateway", "cyberfox_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("cyberfox_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "cyberfox_cli.web_server",
        "cyberfox_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    cyberfox_home: Optional[Path] = None,
    ...
    """Configure the Cyberfox logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    cyberfox_home
        Override for the Cyberfox home directory.  Falls back to
        ``get_cyberfox_home()`` (profile-aware).
    log_level
    ...
    home = cyberfox_home or get_cyberfox_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_cyberfox_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._cyberfox_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the cyberfox group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from cyberfox_cli.config import is_managed
...
# the module header): when several Cyberfox processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._cyberfox_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="cyberfox-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_cyberfox_queue", False):
