#!/usr/bin/env python3
"""
SQLite State Store for Cyberfox Agent.
...
DEFAULT_DB_PATH = get_cyberfox_home() / "state.db"
...
# cyberfox_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``cyberfox_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see cyberfox_cli/kanban_db.py) would
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    try:
        ...
        probe_session_id = f"_cyberfox_fts_health_probe_{time.time_ns()}"
        try:
...
class SessionDB:
    ...
    # With multiple cyberfox processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``cyberfox_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `cyberfox update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._cyberfox_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._cyberfox_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def _contains_cjk(text: str) -> bool:
        ...
        return any(SessionDB._is_cjk_codepoint(ord(ch)) for ch in text)
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Cyberfox surface and jump
    ...
    def delete_session_if_empty(
        ...
        and ``cyberfox sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Cyberfox, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Cyberfox session.

        A Cyberfox session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Cyberfox session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``CYBERFOX_DISABLE_FTS_TRIGRAM`` or not yet created), so
