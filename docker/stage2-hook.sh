...
#
# Wired into the image as /etc/cont-init.d/01-cyberfox-setup by the
...
CYBERFOX_HOME="${CYBERFOX_HOME:-/opt/data}"
INSTALL_DIR="/opt/cyberfox"

# Drop to cyberfox via s6-setuidgid, but skip it when already non-root.
as_cyberfox() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid cyberfox "$@"; }
...
# starts non-root. The baked install tree under /opt/cyberfox is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $CYBERFOX_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass CYBERFOX_UID/CYBERFOX_GID — or the PUID/PGID aliases — which the
...
# `id -u cyberfox` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# cyberfox UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u cyberfox)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-cyberfox UID).
...
and the baked /opt/cyberfox install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e CYBERFOX_UID=\$(id -u) -e CYBERFOX_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the cyberfox user to that UID/GID at boot and chowns the data
...
# --- Bootstrap CYBERFOX_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid cyberfox mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom CYBERFOX_HOME paths whose parents only
# root can create (e.g. `CYBERFOX_HOME=/home/cyberfox/.cyberfox` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for CYBERFOX_UID/CYBERFOX_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  CYBERFOX_UID/
# CYBERFOX_GID still win when both are set.  See #15290, salvages #25872.
CYBERFOX_UID="${CYBERFOX_UID:-${PUID:-}}"
CYBERFOX_GID="${CYBERFOX_GID:-${PGID:-}}"

if [ -n "${CYBERFOX_UID:-}" ] && validate_uid_gid "$CYBERFOX_UID" && [ "$CYBERFOX_UID" != "$(id -u cyberfox)" ]; then
    echo "[stage2] Changing cyberfox UID to $CYBERFOX_UID"
    usermod -u "$CYBERFOX_UID" cyberfox
fi
if [ -n "${CYBERFOX_GID:-}" ] && validate_uid_gid "$CYBERFOX_GID" && [ "$CYBERFOX_GID" != "$(id -g cyberfox)" ]; then
    echo "[stage2] Changing cyberfox GID to $CYBERFOX_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$CYBERFOX_GID" cyberfox 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised cyberfox user (UID 10000)
...
# cyberfox process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes cyberfox. Idempotent across container
...
#   - cyberfox is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G cyberfox 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] cyberfox already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" cyberfox 2>/dev/null; then
        echo "[stage2] Added cyberfox to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group cyberfox failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When CYBERFOX_UID is remapped or the top-level $CYBERFOX_HOME isn't owned by
# the runtime cyberfox UID, restore ownership to cyberfox — but ONLY for the
# directories cyberfox actually writes to. The full $CYBERFOX_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of cyberfox-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_cyberfox_uid=$(id -u cyberfox)

path_has_symlink_component() {
    path="$1"
    root="${2:-$CYBERFOX_HOME}"
...
chown_cyberfox_tree() {
    ...
    chown -R cyberfox:cyberfox "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$CYBERFOX_HOME" 2>/dev/null)" != "$actual_cyberfox_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $CYBERFOX_HOME (targeted) to cyberfox ($actual_cyberfox_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $CYBERFOX_HOME: chown the directory itself (not its contents)
    # so cyberfox can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$CYBERFOX_HOME"; then
        :
    else
        chown cyberfox:cyberfox "$CYBERFOX_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $CYBERFOX_HOME failed (rootless container?) — continuing"
    fi
    # Cyberfox-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by cyberfox (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$CYBERFOX_HOME/$sub" ]; then
            chown_cyberfox_tree "$CYBERFOX_HOME/$sub"
        fi
    done
fi
...
# cyberfox user. Hosted/container instances keep mutable state under
# $CYBERFOX_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# CYBERFOX_DISABLE_LAZY_INSTALLS=1. Keeping /opt/cyberfox root-owned and
...
# $CYBERFOX_HOME/lazy-packages dir on the data volume (Dockerfile sets
# CYBERFOX_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to cyberfox
...
# Always reset ownership of $CYBERFOX_HOME/profiles to cyberfox on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> cyberfox …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as cyberfox and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$CYBERFOX_HOME/profiles" ]; then
    chown_cyberfox_tree "$CYBERFOX_HOME/profiles"
fi

# Always reset ownership of $CYBERFOX_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged cyberfox runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$CYBERFOX_HOME/cron" ]; then
    chown_cyberfox_tree "$CYBERFOX_HOME/cron"
fi
...
# cyberfox pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged cyberfox gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $CYBERFOX_HOME is
...
# is negligible.
if [ -d "$CYBERFOX_HOME/platforms/pairing" ]; then
    chown_cyberfox_tree "$CYBERFOX_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$CYBERFOX_HOME/pairing" ]; then
    chown_cyberfox_tree "$CYBERFOX_HOME/pairing"
fi

# Reset ownership of cyberfox-owned top-level state files on every boot.
# The targeted data-volume chown above only covers cyberfox-owned
# *subdirectories*; loose state files living directly under $CYBERFOX_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> cyberfox …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged cyberfox runtime then hits
...
# sweep so host-owned files in a bind-mounted $CYBERFOX_HOME are never
...
# entries of cyberfox_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$CYBERFOX_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$CYBERFOX_HOME/$f"; then
            :
        else
            chown cyberfox:cyberfox "$CYBERFOX_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the cyberfox runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$CYBERFOX_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$CYBERFOX_HOME/config.yaml"; then
        :
    else
        chown cyberfox:cyberfox "$CYBERFOX_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$CYBERFOX_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as cyberfox user ---
# Run as cyberfox via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $CYBERFOX_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_cyberfox mkdir -p \
    "$CYBERFOX_HOME/backups" \
    "$CYBERFOX_HOME/cron" \
    "$CYBERFOX_HOME/sessions" \
    "$CYBERFOX_HOME/logs" \
    "$CYBERFOX_HOME/logs/gateways" \
    "$CYBERFOX_HOME/hooks" \
    "$CYBERFOX_HOME/memories" \
    "$CYBERFOX_HOME/skills" \
    "$CYBERFOX_HOME/skins" \
    "$CYBERFOX_HOME/plans" \
    "$CYBERFOX_HOME/workspace" \
    "$CYBERFOX_HOME/home" \
    "$CYBERFOX_HOME/pairing" \
    "$CYBERFOX_HOME/platforms/pairing" \
    "$CYBERFOX_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/cyberfox/.install_method (see Dockerfile), NOT written here into
# $CYBERFOX_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $CYBERFOX_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.cyberfox:/opt/data) and sometimes shared with a
...
# 'cyberfox update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $CYBERFOX_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/cyberfox stamp, so deleting the data-dir copy is safe).
if [ -f "$CYBERFOX_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$CYBERFOX_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$CYBERFOX_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$CYBERFOX_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$CYBERFOX_HOME/$dest"; then
            :
        else
            as_cyberfox cp "$INSTALL_DIR/$src" "$CYBERFOX_HOME/$dest"
        fi
    fi
}
...
if [ -f "$CYBERFOX_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$CYBERFOX_HOME/.env"; then
        :
    else
        chown cyberfox:cyberfox "$CYBERFOX_HOME/.env" 2>/dev/null || true
        chmod 600 "$CYBERFOX_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $CYBERFOX_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `cyberfox update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set CYBERFOX_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$CYBERFOX_HOME/config.yaml" ]; then
    s6-setuidgid cyberfox "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$CYBERFOX_HOME/auth.json" ] && [ -n "${CYBERFOX_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$CYBERFOX_HOME/auth.json"; then
        :
    else
        printf '%s' "$CYBERFOX_AUTH_JSON_BOOTSTRAP" > "$CYBERFOX_HOME/auth.json"
        chown cyberfox:cyberfox "$CYBERFOX_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$CYBERFOX_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via CYBERFOX_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$CYBERFOX_HOME/auth.json" ] && [ -n "${CYBERFOX_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$CYBERFOX_HOME/auth.json"; then
        :
    else
        s6-setuidgid cyberfox "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$CYBERFOX_HOME/auth.json" \
...
# can set CYBERFOX_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# CYBERFOX_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$CYBERFOX_HOME/gateway_state.json" ] && \
        [ "${CYBERFOX_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$CYBERFOX_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$CYBERFOX_HOME/gateway_state.json"
        chown cyberfox:cyberfox "$CYBERFOX_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$CYBERFOX_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_cyberfox "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/cyberfox/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Cyberfox spawns for the
...
# main-wrapper.sh propagates it into the supervised ``cyberfox`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-cyberfox, dashboard, etc.).
