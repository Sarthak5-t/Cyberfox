...
Cyberfox supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.cyberfox/config.yaml`:
...
Or set the env var: `CYBERFOX_KANBAN_DISPATCH_IN_GATEWAY=false`
