...
Cyberfox middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Cyberfox'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Cyberfox calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Cyberfox retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Cyberfox guardrails,
...
- `telemetry_schema_version`: currently `cyberfox.observer.v1`
- `middleware_schema_version`: currently `cyberfox.middleware.v1`
...
Cyberfox stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Cyberfox runs
them as a nested chain in registration order. Middleware failures are fail-open:
Cyberfox logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Cyberfox applies middleware in this order:
...
For each tool call, Cyberfox applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Cyberfox pre-execution path against the effective arguments:
...
```bash
cyberfox plugins enable <plugin-name>
```

For isolated local testing, use one `CYBERFOX_HOME` for plugin enablement and the
agent run:

```bash
export CYBERFOX_HOME=/tmp/cyberfox-middleware-test
mkdir -p "$CYBERFOX_HOME"
cyberfox plugins enable <plugin-name>
cyberfox chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run cyberfox plugins enable <plugin-name>
uv run cyberfox chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["cyberfox_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/cyberfox-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Cyberfox expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Cyberfox treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Cyberfox preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Cyberfox does not convert downstream
