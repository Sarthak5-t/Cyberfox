# Cyberfox Observer Hooks

Cyberfox observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Cyberfox' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Cyberfox catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Cyberfox only builds
