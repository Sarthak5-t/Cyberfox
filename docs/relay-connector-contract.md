...
This document is the formal interface between the **Cyberfox gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`Sarthak5-t/gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `cyberfox gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`cyberfox gateway enroll` → connector `/relay/enroll`, or
...
(`Sarthak5-t/gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `cyberfox` CLI.
