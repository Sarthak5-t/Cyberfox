...
When running Cyberfox inside Docker, the default `network_mode: host` gives the
...
The Cyberfox [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ cyberfox-agent │   │ cyberfox-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   CYBERFOX_UID=$(id -u) CYBERFOX_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=cyberfox,cyberfox-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://cyberfox-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Cyberfox trust model and vulnerability reporting
