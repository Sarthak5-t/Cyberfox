...
The canonical transcript store is SQLite via `SessionDB` (from `cyberfox_state`). The
...
Counts consecutive restarts via a JSON file (`{CYBERFOX_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `cyberfox update`, `cyberfox gateway restart`,
or `/restart`.
