...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``cyberfox setup`` display).
    ...
    # built-ins).  Used by ``cyberfox gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `cyberfox`
        # invocation -- including plain `cyberfox chat`, which never touches any
        ...
        # `cyberfox setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``cyberfox setup``/``gateway status``, channel
