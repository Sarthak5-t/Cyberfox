...
There are two ways to add a platform to the Cyberfox gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.cyberfox/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Cyberfox code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `cyberfox gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `cyberfox_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Cyberfox core.
...
And add it to the `cyberfox-gateway` composite:

```python
"cyberfox-gateway": {
    "includes": [..., "cyberfox-your-platform"]
}
```
...
## 12. Status Display (`cyberfox_cli/status.py`)
...
## 13. Gateway Setup Wizard (`cyberfox_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ cyberfox_cli/ toolsets.py \
  --include="*.py" -l | sort -u
