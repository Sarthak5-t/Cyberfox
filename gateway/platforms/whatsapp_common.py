...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Cyberfox Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to CYBERFOX_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/cyberfox), this function
    mirrors the bridge source to a writable CYBERFOX_HOME location and returns that
    ...
    # Try CYBERFOX_HOME location first
    cyberfox_home = get_cyberfox_home()
    cyberfox_home_bridge = cyberfox_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to CYBERFOX_HOME if needed
    if cyberfox_home_bridge.exists():
        return cyberfox_home_bridge

    # Mirror the bridge source to CYBERFOX_HOME
    try:
        cyberfox_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            cyberfox_home_bridge,
            dirs_exist_ok=False,
        )
        return cyberfox_home_bridge
    except Exception:
