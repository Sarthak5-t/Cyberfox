"""Relay/connector support package for the Cyberfox gateway.
...
See the relay connector contract doc for the formal cross-repo interface.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``cyberfox gateway enroll`` writes them to
    ``~/.cyberfox/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``CYBERFOX_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``cyberfox gateway enroll``).
...
def self_provision_relay() -> bool:
    """Boot-time relay self-provision.

    The self-provision path relied on a managed-platform access token that has
    since been removed; relay credentials must now be provided explicitly via
    ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` (env or config). This stub
    returns False so the gateway boots without relay auth when no creds are set.
