...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.cyberfox/.env``.
    ...
    sets neither ``CYBERFOX_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``cyberfox gateway enroll``: has a PINNED
    ...
    try:
        from cyberfox_cli.auth import resolve_nous_access_token
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'cyberfox'}"
