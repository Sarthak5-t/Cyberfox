...
Defenses 1 and 2 (the ``_CYBERFOX_GATEWAY`` guard on ``cyberfox gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.cyberfox.gateway")``,
...
State lives in ``<CYBERFOX_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_cyberfox_home() / "gateway" / "restart_loop.json"
