    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if cyberfox_home_str:
            takeover_path = Path(cyberfox_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(cyberfox_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded cyberfox-agent but never re-ran ``cyberfox setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../cyberfox-gateway.service"
    ...
    # that's the common case for cyberfox.
    ...
    # constant in cyberfox_cli/gateway.py.
    headroom = 30.0
