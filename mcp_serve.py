"""
Cyberfox MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Cyberfox-specific extra)

Usage:
    cyberfox mcp serve
    cyberfox mcp serve --verbose
...
logger = logging.getLogger("cyberfox.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using CYBERFOX_HOME."""
    try:
        from cyberfox_constants import get_cyberfox_home
        return get_cyberfox_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("CYBERFOX_HOME", Path.home() / ".cyberfox")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from cyberfox_constants import get_cyberfox_home
        directory_file = get_cyberfox_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("CYBERFOX_HOME", Path.home() / ".cyberfox")
...
class EventBridge:
    ...
    This is the Cyberfox equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from cyberfox_constants import get_cyberfox_home
            db_file = get_cyberfox_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("CYBERFOX_HOME", Path.home() / ".cyberfox")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Cyberfox MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "cyberfox",
        instructions=(
            "Cyberfox Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Cyberfox MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
