...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      cyberfox-agent = self'.packages.default;
      cyberfoxVenv = cyberfox-agent.cyberfoxVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "cyberfox-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${cyberfoxVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "cyberfox-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "cyberfox-build-package" { } ''
          echo "PASS: package built at ${cyberfox-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "cyberfox-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "cyberfox-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${cyberfox-agent}/bin/cyberfox || (echo "FAIL: cyberfox binary missing"; exit 1)
          test -x ${cyberfox-agent}/bin/cyberfox-agent || (echo "FAIL: cyberfox-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${cyberfox-agent}/bin/cyberfox version 2>&1 | grep -qi "cyberfox" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "cyberfox-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in cyberfox cyberfox-agent cyberfox-acp; do
            test -x ${cyberfox-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "cyberfox-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking cyberfox --help ==="
          ${cyberfox-agent}/bin/cyberfox --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${cyberfox-agent}/bin/cyberfox --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "cyberfox-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${cyberfox-agent}/share/cyberfox-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${cyberfox-agent}/share/cyberfox-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "CYBERFOX_BUNDLED_SKILLS" ${cyberfox-agent}/bin/cyberfox || \
            (echo "FAIL: CYBERFOX_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: CYBERFOX_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "cyberfox-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${cyberfox-agent}/share/cyberfox-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${cyberfox-agent}/share/cyberfox-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "CYBERFOX_BUNDLED_PLUGINS" ${cyberfox-agent}/bin/cyberfox || \
            (echo "FAIL: CYBERFOX_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: CYBERFOX_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "cyberfox-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${cyberfox-agent}/share/cyberfox-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${cyberfox-agent}/share/cyberfox-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${cyberfox-agent}/share/cyberfox-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "CYBERFOX_BUNDLED_LOCALES" ${cyberfox-agent}/bin/cyberfox || \
            (echo "FAIL: CYBERFOX_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: CYBERFOX_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (CYBERFOX_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && CYBERFOX_BUNDLED_LOCALES=${cyberfox-agent}/share/cyberfox-agent/locales \
            ${cyberfoxVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with CYBERFOX_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${cyberfoxVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${cyberfoxVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without CYBERFOX_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "cyberfox-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${cyberfox-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${cyberfox-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "CYBERFOX_TUI_DIR" ${cyberfox-agent}/bin/cyberfox || \
            (echo "FAIL: CYBERFOX_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: CYBERFOX_TUI_DIR set in wrapper"
        ...
        # Verify CYBERFOX_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        cyberfox-node = pkgs.runCommand "cyberfox-node-version" { } ''
          set -e
          echo "=== Checking CYBERFOX_NODE in wrapper ==="
          grep -q "CYBERFOX_NODE" ${cyberfox-agent}/bin/cyberfox || \
            (echo "FAIL: CYBERFOX_NODE not set in wrapper"; exit 1)
          echo "PASS: CYBERFOX_NODE present in wrapper"

          CYBERFOX_NODE=$(sed -n "s/^export CYBERFOX_NODE='\(.*\)'/\1/p" ${cyberfox-agent}/bin/cyberfox)
          test -x "$CYBERFOX_NODE" || (echo "FAIL: CYBERFOX_NODE=$CYBERFOX_NODE not executable"; exit 1)
          echo "PASS: CYBERFOX_NODE executable at $CYBERFOX_NODE"

          NODE_MAJOR=$("$CYBERFOX_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All CYBERFOX_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify CYBERFOX_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "cyberfox-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(CYBERFOX_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking CYBERFOX_MANAGED guards ==="
          check_blocked "config set" ${cyberfox-agent}/bin/cyberfox config set model foo
          check_blocked "config edit" ${cyberfox-agent}/bin/cyberfox config edit
        ...
        extra-python-packages = let
          ...
          cyberfoxWithExtra = cyberfox-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "cyberfox-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${cyberfoxWithExtra}/bin/cyberfox || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${cyberfoxWithExtra}/bin/cyberfox || \
          ...
          if grep -q "PYTHONPATH" ${cyberfox-agent}/bin/cyberfox; then
        ...
        extra-dependency-groups = let
          cyberfoxWithGroups = cyberfox-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "cyberfox-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${cyberfoxWithGroups}"
          echo "venv: ${cyberfoxWithGroups.cyberfoxVenv}"
        ...
        messaging-variant = pkgs.runCommand "cyberfox-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.cyberfoxVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "cyberfox-config-roundtrip" {
          ...
          merge_and_load() {
            local cyberfox_home="$1"
            export CYBERFOX_HOME="$cyberfox_home"
            ${configMergeScript} ${nixSettings} "$cyberfox_home/config.yaml"
            ${cyberfoxVenv}/bin/python3 -c '
import json, sys
