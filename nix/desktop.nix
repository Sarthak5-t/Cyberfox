# nix/desktop.nix — Cyberfox Desktop (Electron) app build + wrapper
#
# `cyberfoxAgent` is the fully-built `.#default` package — it ships the
# `cyberfox` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `CYBERFOX_DESKTOP_CYBERFOX` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Cyberfox CLI").
...
let
  npm = cyberfoxNpmLib.mkNpmPassthru {
    folder = "apps/desktop";
    attr = "desktop";
    pname = "cyberfox-desktop";
  };
  ...
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "cyberfox-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/cyberfox-desktop $out/bin
    cp -r ${renderer}/* $out/share/cyberfox-desktop/
    ...
    substituteInPlace $out/share/cyberfox-desktop/electron/main.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/cyberfox-desktop'"
    ...
    substituteInPlace $out/share/cyberfox-desktop/electron/git-review-ops.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/cyberfox-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # CYBERFOX_DESKTOP_CYBERFOX to the absolute path of the nix-built `cyberfox`
    # binary so the desktop's resolver step 4 ("existing Cyberfox CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/cyberfox-desktop \
      --add-flags "$out/share/cyberfox-desktop" \
      --set CYBERFOX_DESKTOP_CYBERFOX "${lib.getExe cyberfoxAgent}" \
