# nix/nixosModules.nix — NixOS module for cyberfox-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: cyberfox runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $CYBERFOX_HOME/.env
# and read by cyberfox at startup — no container recreation needed for env changes.
#
# Tool resolution: the cyberfox wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.cyberfox-agent;
    effectivePackage =
    ...
    cyberfox-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-cyberfox-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "cyberfox-config-attrs";
      description = "Cyberfox YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "cyberfox-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # CYBERFOX_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "cyberfox-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'CYBERFOX_DOC_EOF'\n${value}\nCYBERFOX_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "cyberfox-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/cyberfox";
    ...
    # cyberfox user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "cyberfox-container-entrypoint" ''
      set -eu

      CYBERFOX_UID="''${CYBERFOX_UID:?CYBERFOX_UID must be set}"
      CYBERFOX_GID="''${CYBERFOX_GID:?CYBERFOX_GID must be set}"

      # ── Group: ensure a group with GID=$CYBERFOX_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$CYBERFOX_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="cyberfox"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$CYBERFOX_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$CYBERFOX_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$CYBERFOX_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$CYBERFOX_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="cyberfox"
        TARGET_HOME="/home/cyberfox"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$CYBERFOX_UID" -g "$CYBERFOX_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$CYBERFOX_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$CYBERFOX_UID:$CYBERFOX_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure CYBERFOX_HOME is owned by the target user.
      ...
      if [ -n "''${CYBERFOX_HOME:-}" ] && [ -d "$CYBERFOX_HOME" ]; then
        find "$CYBERFOX_HOME" \! -user "$CYBERFOX_UID" -exec chown "$CYBERFOX_UID:$CYBERFOX_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/cyberfox-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/cyberfox-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/cyberfox ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/cyberfox
        chmod 0440 /etc/sudoers.d/cyberfox
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$CYBERFOX_UID" --regid="$CYBERFOX_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $CYBERFOX_HOME/.env.
    ...
    # Default: /var/lib/cyberfox/workspace → /data/workspace.
  ...
  in {
    options.services.cyberfox-agent = with lib; {
      enable = mkEnableOption "Cyberfox Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = cyberfox-agent;
        description = "The cyberfox-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/cyberfox";
        description = "State directory. Contains .cyberfox/ subdir (CYBERFOX_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Cyberfox config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $CYBERFOX_HOME/.env at activation time.
          Cyberfox reads this file on every startup via load_cyberfox_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $CYBERFOX_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $CYBERFOX_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `cyberfox gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the cyberfox user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the cyberfox plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Cyberfox discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          cyberfox_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as cyberfox (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in cyberfox-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the cyberfox CLI to environment.systemPackages and export
          CYBERFOX_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.cyberfox symlink to the service
            stateDir. These users are automatically added to the cyberfox group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.cyberfox-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the cyberfox CLI to system PATH and export CYBERFOX_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.cyberfox/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.CYBERFOX_HOME = "${cfg.stateDir}/.cyberfox";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.cyberfox-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the cyberfox user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.cyberfox-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed cyberfox
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure cyberfox is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.cyberfox        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cyberfox/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cyberfox/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cyberfox/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cyberfox/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cyberfox/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."cyberfox-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.cyberfox
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.cyberfox ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.cyberfox ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.cyberfox -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.cyberfox/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.cyberfox/$_subdir"
            chmod 2770 "${cfg.stateDir}/.cyberfox/$_subdir"
            find "${cfg.stateDir}/.cyberfox/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # cyberfox-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.cyberfox/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.cyberfox/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.cyberfox/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.cyberfox/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.cyberfox/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.cyberfox/.managed
          chmod 0644 ${cfg.stateDir}/.cyberfox/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.cyberfox/.container-mode <<'CYBERFOX_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    cyberfox_bin=${containerDataDir}/current-package/bin/cyberfox
    CYBERFOX_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.cyberfox/.container-mode
            chmod 0644 ${cfg.stateDir}/.cyberfox/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.cyberfox/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.cyberfox";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.cyberfox" ]; then
                  rm -f "${symlinkPath}"
                  echo "cyberfox-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.cyberfox -> stateDir/.cyberfox for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.cyberfox";
                target = "${cfg.stateDir}/.cyberfox";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "cyberfox-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.cyberfox/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.cyberfox/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.cyberfox/auth.json
              fi
            ''}
          ''}
          ...
          # Cyberfox reads $CYBERFOX_HOME/.env at startup via load_cyberfox_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.cyberfox/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'CYBERFOX_NIX_ENV_EOF'
    ${envFileContent}
    CYBERFOX_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
                echo "" >> "$ENV_FILE"
                cat "${f}" >> "$ENV_FILE"
        ...
        find ${cfg.stateDir}/.cyberfox/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.cyberfox/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.cyberfox/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.cyberfox-agent = {
          description = "Cyberfox Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            CYBERFOX_HOME = "${cfg.stateDir}/.cyberfox";
            CYBERFOX_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $CYBERFOX_HOME/.env by the activation script. load_cyberfox_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/cyberfox"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the cyberfox group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.cyberfox-agent = {
          description = "Cyberfox Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              CYBERFOX_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              CYBERFOX_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env CYBERFOX_UID="$CYBERFOX_UID" \
                --env CYBERFOX_GID="$CYBERFOX_GID" \
                --env CYBERFOX_HOME=${containerDataDir}/.cyberfox \
                --env CYBERFOX_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/cyberfox gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
