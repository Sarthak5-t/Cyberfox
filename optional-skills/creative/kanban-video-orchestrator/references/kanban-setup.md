...
the actual `setup.sh` that creates the project workspace, configures Cyberfox
...
> [Sarthak5-t/Cyberfox](https://github.com/Sarthak5-t/Cyberfox).
...
2. **Create profiles** — `cyberfox profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.cyberfox/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `cyberfox kanban create` assigned to the director
...
```bash
cyberfox profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.cyberfox/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.cyberfox/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Cyberfox
...
Each profile gets a `SOUL.md` at `~/.cyberfox/profiles/<name>/SOUL.md` that
...
```bash
cyberfox kanban create "Direct production of <video title>" \
    --assignee director \
...
the Cyberfox `.env` (`${CYBERFOX_HOME:-$HOME/.cyberfox}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _cyberfox_env="${CYBERFOX_HOME:-$HOME/.cyberfox}/.env"
    if grep -q "^${var}=" "$_cyberfox_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_cyberfox_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_cyberfox_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY cyberfox ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY cyberfox OPENROUTER_API_KEY || exit 1
# ...
```
