...
cyberfox kanban watch --tenant <project-slug>

# Snapshot of the board
cyberfox kanban list --tenant <project-slug>
cyberfox kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
cyberfox kanban stats --tenant <project-slug>

# Visual dashboard (browser)
cyberfox dashboard

# Inspect a specific task (includes comments + events)
cyberfox kanban show <task-id>

# Follow a single task's event stream
cyberfox kanban tail <task-id>
```

Verify available subcommands with `cyberfox kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `cyberfox kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `cyberfox kanban block <id>` then `cyberfox kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `cyberfox kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `cyberfox kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `cyberfox profile list` (profile exists?), `cyberfox status` (gateway/dispatcher up?), and verify tenant. |
...
cyberfox kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
cyberfox kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(cyberfox kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
cyberfox kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
cyberfox kanban block <task-id>
cyberfox kanban archive <task-id>

# Diagnose what happened
cyberfox kanban show <task-id>      # task body, comments, recent events
cyberfox kanban tail <task-id>      # follow the live event stream
cyberfox kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `cyberfox kanban create` for the director
...
while true; do
    clear
    cyberfox kanban list --tenant <slug>
    echo "---"
    cyberfox kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `cyberfox kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `cyberfox profile list` and start
