...
Maps each role archetype to the Cyberfox skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public cyberfox-agent
...
## Cyberfox skills relevant to video production

### Visual / rendering skills (`cyberfox-agent/skills/creative/`)
...
### Design / pre-production skills (`cyberfox-agent/skills/creative/`)
...
### Audio / media skills (`cyberfox-agent/skills/creative/` + `skills/media/`)
...
These are **not** Cyberfox skills but external CLIs / APIs that profiles invoke.
...
## Built-in Cyberfox tools for media review

These are native Cyberfox tools — not invoked via terminal but through their own
...
key is present in `${CYBERFOX_HOME:-~/.cyberfox}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Cyberfox profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${CYBERFOX_HOME:-~/.cyberfox}/.env` → environment variable.
...
When a new Cyberfox-public video skill ships:
