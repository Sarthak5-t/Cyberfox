#!/usr/bin/env python3
"""Canvas LMS API CLI for Cyberfox Agent.
...
def _check_config():
    ...
    if missing:
        cyberfox_env = os.path.join(
            os.environ.get("CYBERFOX_HOME", os.path.expanduser("~/.cyberfox")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {cyberfox_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Cyberfox Agent"
    )
