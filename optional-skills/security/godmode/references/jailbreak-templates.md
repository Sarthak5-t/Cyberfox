...
## 5. GODMODE FAST — Cyberfox 4 405B (Zero Refusal)

**Model:** `nousresearch/cyberfox-4-405b`
**Strategy:** Cyberfox models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Cyberfox Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.cyberfox/config.yaml`:
