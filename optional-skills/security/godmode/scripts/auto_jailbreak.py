...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("CYBERFOX_HOME", os.path.expanduser("~/.cyberfox")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CYBERFOX_HOME", Path.home() / ".cyberfox")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Cyberfox config paths
# ═══════════════════════════════════════════════════════════════════

CYBERFOX_HOME = Path(os.getenv("CYBERFOX_HOME", Path.home() / ".cyberfox"))
CONFIG_PATH = CYBERFOX_HOME / "config.yaml"
PREFILL_PATH = CYBERFOX_HOME / "prefill.json"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Cyberfox config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.cyberfox/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Cyberfox for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Cyberfox for changes to take effect.")
