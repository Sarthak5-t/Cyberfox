...
The design is **Cyberfox-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
cyberfox skills install official/security/unbroker
```

Then start a new Cyberfox session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Cyberfox session:
...
- **PII handling.** Dossiers live under the Cyberfox home directory (`0600`, optionally
