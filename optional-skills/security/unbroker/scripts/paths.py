...
All per-subject data lives under PDD_DATA_DIR (default: $CYBERFOX_HOME/unbroker),
which is the same trust boundary Cyberfox uses for .env and OAuth tokens.
"""
...
def cyberfox_home() -> Path:
    return Path(os.environ.get("CYBERFOX_HOME") or (Path.home() / ".cyberfox"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else cyberfox_home() / "unbroker"
