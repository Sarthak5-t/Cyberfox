...
def _handle_create(args: dict, **kw) -> str:
    try:
        ...
        return json_result(False, error=f"plan_create failed: {e}")
...
def _handle_update(args: dict, **kw) -> str:
    try:
        ...
        if status in ("completed", "failed", "skipped"):
            try:
                append_entry("plan", f"Task #{task_id} → {status}: {result or 'no details'}")
            except Exception:
                pass
        ...
        return json_result(False, error=f"plan_update failed: {e}")
...
def _handle_next(args: dict, **kw) -> str:
    try:
        ...
        return json_result(False, error=f"plan_next failed: {e}")
...
def _handle_add(args: dict, **kw) -> str:
    try:
        ...
        return json_result(False, error=f"plan_add failed: {e}")
