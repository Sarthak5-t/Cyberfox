# Cyberfox Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: cyberfox-achievements plugin
...
### `GET /api/plugins/cyberfox-achievements/achievements`
...
### `POST /api/plugins/cyberfox-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/cyberfox-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/cyberfox-achievements/overview`
...
- Persist plugin data under:
  - `~/.cyberfox/plugins/cyberfox-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.cyberfox/plugins/cyberfox-achievements/scan_checkpoint.json`
...
- `~/.cyberfox/plugins/cyberfox-achievements/`
