...
A standards-compliant OpenID Connect Relying Party for the ``cyberfox dashboard``
OAuth gate. This provider speaks **plain OIDC** so it works against any conformant
self-hosted identity provider:

    Authentik · Keycloak · Zitadel · Authelia · Auth0 · Okta · Google · …

It is a pure drop-in plugin: it implements the five
:class:`~cyberfox_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``cyberfox_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~cyberfox_cli.dashboard_auth.Session`.
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.cyberfox/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    CYBERFOX_DASHBOARD_OIDC_ISSUER
    CYBERFOX_DASHBOARD_OIDC_CLIENT_ID
    CYBERFOX_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    CYBERFOX_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
# JWKS cache (PyJWKClient handles its own caching; this 5-minute lifespan keeps
# key rotation picked up promptly).
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        cookie_payload = {
            "cyberfox_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Cyberfox does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``CYBERFOX_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "CYBERFOX_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "CYBERFOX_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("CYBERFOX_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.cyberfox/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "CYBERFOX_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(CYBERFOX_DASHBOARD_OIDC_ISSUER + CYBERFOX_DASHBOARD_OIDC_CLIENT_ID) "
