...
Auto-tracks and cleans up ephemeral files created during Cyberfox Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$CYBERFOX_HOME` and `/tmp/cyberfox-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CYBERFOX_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `CYBERFOX_HOME` or `/tmp/cyberfox-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$CYBERFOX_HOME/disk-cleanup/` is itself excluded
- `$CYBERFOX_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
