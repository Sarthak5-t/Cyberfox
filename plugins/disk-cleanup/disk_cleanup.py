"""disk_cleanup — ephemeral file cleanup for Cyberfox Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under CYBERFOX_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly CYBERFOX_HOME and /tmp/cyberfox-*
Never touches: ~/.cyberfox/logs/ or any system directory.
"""
...
try:
    ...
    def get_cyberfox_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("CYBERFOX_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".cyberfox").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$CYBERFOX_HOME/logs/``."""
    return get_cyberfox_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$CYBERFOX_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under CYBERFOX_HOME or ``/tmp/cyberfox-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    cyberfox_home = get_cyberfox_home()
    try:
        path.resolve().relative_to(cyberfox_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/cyberfox-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("cyberfox-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "cyberfox-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $CYBERFOX_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so CYBERFOX_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        cyberfox_home = get_cyberfox_home()
        for parent in ("cron", "cronjobs"):
            base = cyberfox_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside CYBERFOX_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under CYBERFOX_HOME, but never recurse into known
    # durable state trees.  Some installs place the Cyberfox checkout, venv,
    # and desktop build under CYBERFOX_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    cyberfox_home = get_cyberfox_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in cyberfox_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    cyberfox_home = get_cyberfox_home()
    try:
        rel = path.resolve().relative_to(cyberfox_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "cyberfox-agent",
        }:
            return None
        ...
        # Path isn't under CYBERFOX_HOME (e.g. /tmp/cyberfox-*) — fall through.
        pass
