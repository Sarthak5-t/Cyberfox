...
Let the cyberfox agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where cyberfox runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `cyberfox meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `cyberfox meet` CLI |
...
| `cli.py` | `cyberfox meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$CYBERFOX_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `cyberfox meet node {run,list,approve,remove,status,ping}` |
...
cyberfox meet install                                      # pip + Chromium
cyberfox meet setup                                        # preflight
cyberfox meet auth                                         # optional
cyberfox meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
cyberfox meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.cyberfox/.env
cyberfox meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
cyberfox meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
cyberfox meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.cyberfox/.env
cyberfox meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, cyberfox will **not** switch your system audio input automatically — the
...
cyberfox plugins enable google_meet
cyberfox meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
cyberfox meet node ping my-mac
