...
Wires ``cyberfox meet <subcommand>``:
...
def _auth_state_path() -> Path:
    return Path(get_cyberfox_home()) / "workspace" / "meetings" / "auth.json"
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``cyberfox meet`` argparse tree.
    ...
    join_p.add_argument("--guest-name", default="Cyberfox Agent")
    ...
    try:
        ...
        def _node_unavailable(args):
            print(f"cyberfox meet node: module unavailable ({e})")
            return 1
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if not sub:
        print("usage: cyberfox meet {setup,auth,join,status,transcript,say,stop,node}")
        return 2
    if sub == "setup":
        return _cmd_setup()
    if sub == "install":
    ...
    if sub == "node":
        ...
        if fn is None or fn is meet_command:
            print("usage: cyberfox meet node {run,list,approve,remove,status,ping}")
            return 2
        return fn(args)
...
def _cmd_setup() -> int:
    ...
    print(
        "  google auth    : "
        + (f"ok ({auth_path})" if auth_ok else "not saved — run: cyberfox meet auth")
    )
    ...
    if all_ok:
        print(
            ...
            "cyberfox meet join https://meet.google.com/abc-defg-hij"
        )
    else:
...
def _cmd_install(*, realtime: bool, assume_yes: bool) -> int:
    ...
    print("\ndone. verify with: cyberfox meet setup")
    return 0
...
def _cmd_auth() -> int:
    ...
    try:
        ...
        return 1
    print("saved. you can now run: cyberfox meet join <url>")
    return 0
...
def _cmd_stop() -> int:
    res = pm.stop(reason="cyberfox meet stop")
    print(json.dumps(res, indent=2))
    return 0 if res.get("ok") else 1
