"""`cyberfox meet node ...` subcommand tree.

Wired into the existing ``cyberfox meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``cyberfox meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="cyberfox-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``cyberfox meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             cyberfox meet node approve <name> ws://<host>:{args.port} {token}")
        try:
