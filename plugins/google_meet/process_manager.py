...
session-scoped state file under ``$CYBERFOX_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $CYBERFOX_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_cyberfox_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this cyberfox install, leave it first —
    ...
    env = os.environ.copy()
    env["CYBERFOX_MEET_URL"] = url
    env["CYBERFOX_MEET_OUT_DIR"] = str(out)
    env["CYBERFOX_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["CYBERFOX_MEET_HEADED"] = "1"
    if auth_state:
        env["CYBERFOX_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["CYBERFOX_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if CYBERFOX_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["CYBERFOX_MEET_MODE"] = mode
    if realtime_model:
        env["CYBERFOX_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["CYBERFOX_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["CYBERFOX_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["CYBERFOX_MEET_REALTIME_KEY"] = realtime_api_key
