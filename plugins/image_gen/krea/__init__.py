...
# Cyberfox uses 3 abstract aspect ratios. Map to Krea's enum (which is wider).
...
def _resolve_model(explicit: Optional[str] = None) -> Tuple[str, Dict[str, Any]]:
    ...
    Precedence: explicit caller override (e.g. a direct ``model`` kwarg) →
    ``KREA_IMAGE_MODEL`` env → ``image_gen.krea.model`` →
...
class KreaImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            "name": "Krea",
            "badge": "paid",
            "tag": "Krea 2 foundation model — Medium ($0.03), Large ($0.06), Medium Turbo ($0.015). Style transfer, moodboards, reference-guided generation. Requires a direct KREA_API_KEY.",
            "env_vars": [
                {
                    "key": "KREA_API_KEY",
                    "prompt": "Krea API key",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not auth_token:
            return error_response(
                error=(
                    "KREA_API_KEY not set. Run `cyberfox tools` → Image "
        ...
        headers = {
            ...
            "User-Agent": "Cyberfox-Agent/1.0 (krea-image-gen)",
        }
        ...
        # 2. Poll for completion. Status/result polling uses the same
        # ``KREA_API_KEY`` bearer token as the submit call.
