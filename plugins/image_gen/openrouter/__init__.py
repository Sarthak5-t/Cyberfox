...
OpenRouter speaks the OpenAI-style ``/chat/completions`` image-generation
protocol: send ``modalities: ["image", "text"]`` with an image-output model
(e.g. ``google/gemini-3-pro-image``), pass reference images as ``image_url``
...
:func:`~cyberfox_cli.runtime_provider.resolve_runtime_provider`, which already
...
# image_gen.model from ``cyberfox tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    """Image generation over the OpenRouter chat-completions endpoint."""
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``cyberfox tools``) → the quality-first default chain.
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `cyberfox tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
