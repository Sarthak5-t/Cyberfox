/**
 * Cyberfox Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.cyberfox/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__CYBERFOX_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__CYBERFOX_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "cyberfox-kanban-dot-triage",
    todo: "cyberfox-kanban-dot-todo",
    ready: "cyberfox-kanban-dot-ready",
    running: "cyberfox-kanban-dot-running",
    blocked: "cyberfox-kanban-dot-blocked",
    done: "cyberfox-kanban-dot-done",
    archived: "cyberfox-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-cyberfox-task";
  ...
  const DOCS_URL = "https://github.com/Sarthak5-t/Cyberfox/blob/main/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://github.com/Sarthak5-t/Cyberfox/blob/main/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "cyberfox.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="cyberfox-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])cyberfox-kanban-md-code\1/i.test(attrs)) {
      return ' class="cyberfox-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "cyberfox-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "cyberfox-kanban-md",
  ...
  // standard `drop` event and our `cyberfox-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("cyberfox-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("cyberfox-kanban-column--drop");
          if (target) target.classList.add("cyberfox-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("cyberfox-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("cyberfox-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("cyberfox-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__CYBERFOX_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "cyberfox-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "cyberfox-kanban-attention",
        "cyberfox-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "cyberfox-kanban-attention-bar" },
        h("span", { className: "cyberfox-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "cyberfox-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "cyberfox-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "cyberfox-kanban-attention-row",
                  "cyberfox-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "cyberfox-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "cyberfox-kanban-attention-row-id" }, task.id),
                h("span", { className: "cyberfox-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "cyberfox-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "cyberfox-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "cyberfox-kanban-diag-action-btn",
      action.suggested ? "cyberfox-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " cyberfox-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".cyberfox-kanban-drawer-comment-row input, .cyberfox-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "cyberfox-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("cyberfox-kanban-diag", sevClass) },
      h("div", { className: "cyberfox-kanban-diag-header" },
        h("span", { className: "cyberfox-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "cyberfox-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "cyberfox-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "cyberfox-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "cyberfox-kanban-diag-data-row" },
                  h("span", { className: "cyberfox-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "cyberfox-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "cyberfox-kanban-diag-data-row" },
                h("span", { className: "cyberfox-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "cyberfox-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "cyberfox-kanban-diag-reassign-row" },
            h("span", { className: "cyberfox-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "cyberfox-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "cyberfox-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "cyberfox-kanban-diag-msg",
              msg.ok ? "cyberfox-kanban-diag-msg--ok" : "cyberfox-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "cyberfox-kanban-section" },
      h("div", { className: "cyberfox-kanban-section-head-row" },
        h("span", { className: "cyberfox-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "cyberfox-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "cyberfox-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "cyberfox-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "cyberfox-kanban-docs-link",
      title: "Open Cyberfox Kanban docs in a new tab",
      "aria-label": "Cyberfox Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `cyberfox kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "cyberfox-kanban-msg-ok" : "cyberfox-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "cyberfox-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "cyberfox-kanban-boardswitcher" },
      h("div", { className: "cyberfox-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "cyberfox-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "cyberfox-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "cyberfox-kanban-dialog-title" },
        ...
        h("div", { className: "cyberfox-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Cyberfox profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "cyberfox-kanban-bulk" },
      h("span", { className: "cyberfox-kanban-bulk-count" },
      ...
      h("div", { className: "cyberfox-kanban-bulk-priority",
      ...
      h("div", { className: "cyberfox-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Cyberfox profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "cyberfox-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("cyberfox-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("cyberfox-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "cyberfox-kanban-trash",
        dragOver ? "cyberfox-kanban-trash--drop" : "",
        props.draggingTaskId ? "cyberfox-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "cyberfox-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "cyberfox-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".cyberfox-kanban-card")) return true;
      if (target.closest && target.closest(".cyberfox-kanban-column-add")) return true;
      if (target.closest && target.closest(".cyberfox-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("cyberfox-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("cyberfox-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".cyberfox-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "cyberfox-kanban-columns",
        isScrollable ? "cyberfox-kanban-columns--scrollable" : "",
        isPanning ? "cyberfox-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("cyberfox-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("cyberfox-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "cyberfox-kanban-column",
        dragOver ? "cyberfox-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "cyberfox-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "cyberfox-kanban-col-check",
        ...
        h("span", { className: cn("cyberfox-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "cyberfox-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "cyberfox-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "cyberfox-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "cyberfox-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "cyberfox-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "cyberfox-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "cyberfox-kanban-lane" },
                  h("div", { className: "cyberfox-kanban-lane-head" },
                    h("span", { className: "cyberfox-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "cyberfox-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "cyberfox-kanban-card--stale-red";
    if (age >= tier.amber) return "cyberfox-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".cyberfox-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "cyberfox-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "cyberfox-kanban-card",
        props.selected ? "cyberfox-kanban-card--selected" : "",
        props.failed ? "cyberfox-kanban-card--failed" : "",
        props.draggingSource ? "cyberfox-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "cyberfox-kanban-card-content" },
          h("div", { className: "cyberfox-kanban-card-row" },
            h("label", {
              className: "cyberfox-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "cyberfox-kanban-card-check",
            ...
            h("span", { className: "cyberfox-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or cyberfox kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "cyberfox-kanban-warning-badge",
                    "cyberfox-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "cyberfox-kanban-needs-assignee",
          ...
          h("div", { className: "cyberfox-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "cyberfox-kanban-card-row cyberfox-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "cyberfox-kanban-assignee",
                            title: `Assigned to Cyberfox profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "cyberfox-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "cyberfox-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "cyberfox-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "cyberfox-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Cyberfox profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Cyberfox profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "cyberfox-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "cyberfox-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "cyberfox-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "cyberfox-kanban-drawer-close",
        ...
        data ? h("div", { className: "cyberfox-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "cyberfox-kanban-section" },
      h("div", { className: "cyberfox-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "cyberfox-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "cyberfox-kanban-drawer-body" },
      h("div", { className: "cyberfox-kanban-drawer-title" },
        h("span", { className: cn("cyberfox-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "cyberfox-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "cyberfox-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "cyberfox-kanban-section" },
        h("div", { className: "cyberfox-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "cyberfox-kanban-section" },
        h("div", { className: "cyberfox-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "cyberfox-kanban-comment" },
                h("div", { className: "cyberfox-kanban-comment-head" },
                  h("span", { className: "cyberfox-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "cyberfox-kanban-comment-ago" },
      ...
      h("div", { className: "cyberfox-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "cyberfox-kanban-event",
              isDiag ? "cyberfox-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "cyberfox-kanban-event-header" },
                  h("span", { className: "cyberfox-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "cyberfox-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "cyberfox-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "cyberfox-kanban-event-header-plain" },
                  h("span", { className: "cyberfox-kanban-event-kind" }, e.kind),
                  h("span", { className: "cyberfox-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "cyberfox-kanban-event-phantom-row" },
                  h("span", { className: "cyberfox-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "cyberfox-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "cyberfox-kanban-event-payload" },
    ...
    return h("div", { className: "cyberfox-kanban-section" },
      h("div", { className: "cyberfox-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "cyberfox-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `cyberfox-kanban-run--${r.outcome || r.status || "ended"}`
          : "cyberfox-kanban-run--active";
        return h("div", { key: r.id, className: cn("cyberfox-kanban-run", outcomeClass) },
          h("div", { className: "cyberfox-kanban-run-head" },
            h("span", { className: "cyberfox-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "cyberfox-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "cyberfox-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "cyberfox-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "cyberfox-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "cyberfox-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "cyberfox-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "cyberfox-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "cyberfox-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "cyberfox-kanban-pre cyberfox-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "cyberfox-kanban-meta-row" },
      h("span", { className: "cyberfox-kanban-meta-label" }, props.label),
      h("span", { className: "cyberfox-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "cyberfox-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "cyberfox-kanban-meta-row" },
        h("span", { className: "cyberfox-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "cyberfox-kanban-meta-value cyberfox-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "cyberfox-kanban-meta-row" },
        h("span", { className: "cyberfox-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "cyberfox-kanban-meta-value cyberfox-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "cyberfox-kanban-section" },
      h("div", { className: "cyberfox-kanban-section-head-row" },
        h("span", { className: "cyberfox-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "cyberfox-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "cyberfox-kanban-section" },
      h("div", { className: "cyberfox-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "cyberfox-kanban-deps-row" },
        h("span", { className: "cyberfox-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "cyberfox-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "cyberfox-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "cyberfox-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "cyberfox-kanban-dep-chip-x",
      ...
      h("div", { className: "cyberfox-kanban-deps-row" },
        h("span", { className: "cyberfox-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "cyberfox-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "cyberfox-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "cyberfox-kanban-msg-ok"
          : "cyberfox-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "cyberfox-kanban-section" },
      ...
      h("div", { className: "cyberfox-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "cyberfox-kanban-home-sub cyberfox-kanban-home-sub--on"
              : "cyberfox-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__CYBERFOX_PLUGINS__ && typeof window.__CYBERFOX_PLUGINS__.register === "function") {
    window.__CYBERFOX_PLUGINS__.register("kanban", KanbanPage);
  }
})();
