...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/cyberfox>
...
```bash
cyberfox memory setup honcho   # configure Honcho directly (works on a fresh install)
cyberfox memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
cyberfox config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.cyberfox/.env
```

> `cyberfox honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `cyberfox memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$CYBERFOX_HOME/honcho.json` | Profile-local (isolated Cyberfox instances) |
| 2 | `~/.cyberfox/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Cyberfox profile: `cyberfox` (default) or `cyberfox_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `cyberfox honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Cyberfox session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`cyberfox-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`cyberfox`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-cyberfox-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening cyberfox in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Cyberfox session ID (timestamp + hex). Every `cyberfox` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Cyberfox profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`cyberfox`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Cyberfox profile: `cyberfox` (default) or `cyberfox_<profile>` (e.g. `cyberfox -p coder` -> host key `cyberfox_coder`). Older `cyberfox.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `CYBERFOX_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `cyberfox-agent`) |
...
|---------|-------------|
| `cyberfox memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `cyberfox honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `cyberfox memory setup`) |
| `cyberfox honcho status` | Show resolved config for active profile |
| `cyberfox honcho enable` / `disable` | Toggle Honcho for active profile |
| `cyberfox honcho mode <mode>` | Change recall or observation mode |
| `cyberfox honcho peer --user <name>` | Update user peer name |
| `cyberfox honcho peer --ai <name>` | Update AI peer name |
| `cyberfox honcho tokens --context <N>` | Set context token budget |
| `cyberfox honcho tokens --dialectic <N>` | Set dialectic max chars |
| `cyberfox honcho map <name>` | Map current directory to a session name |
| `cyberfox honcho sync` | Create host blocks for all Cyberfox profiles |
