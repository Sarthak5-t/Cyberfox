...
Handles: cyberfox honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "cyberfox" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Cyberfox profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'cyberfox honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'cyberfox honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `cyberfox update` -- no output, no exceptions.
    """
    try:
        from cyberfox_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Cyberfox profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $CYBERFOX_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_cyberfox_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        cyberfox_host.get("pinUserPeer"),
        cyberfox_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in cyberfox_host:
        aliases_src = cyberfox_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in cyberfox_host:
        prefix_src = cyberfox_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(cyberfox_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom cyberfox_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    cyberfox_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        cyberfox_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(cyberfox_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = cyberfox_host.get("userPeerAliases")
    prefix = cyberfox_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(cyberfox_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(cyberfox_host)
    cyberfox_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        cyberfox_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        cyberfox_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Cyberfox persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = cyberfox_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            cyberfox_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(cyberfox_host)
        ...
        if use_oauth:
            ...
            try:
                ...
                print("  Re-run 'cyberfox honcho setup' to retry, or choose an API key instead.\n")
                return
            cyberfox_host["apiKey"] = cred.access_token
            cyberfox_host["oauth"] = cred.oauth_block()
            # Default the peer prompt to the name entered at consent.
            if cred.consent_peer_name:
                cyberfox_host["peerName"] = cred.consent_peer_name
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'cyberfox honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = cyberfox_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        cyberfox_host["peerName"] = new_peer

    current_ai = cyberfox_host.get("aiPeer") or cfg.get("aiPeer", "cyberfox")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        cyberfox_host["aiPeer"] = new_ai

    current_workspace = cyberfox_host.get("workspace") or cfg.get("workspace", "cyberfox")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        cyberfox_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Cyberfox GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, cyberfox_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Cyberfox gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        peer_target = cyberfox_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(cyberfox_host)
            cyberfox_host["pinUserPeer"] = False
            if prior_aliases:
                cyberfox_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                cyberfox_host, current_prefix, prefix_from_root,
            ...
            if merged:
                cyberfox_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                cyberfox_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                cyberfox_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = cyberfox_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        cyberfox_host["observationMode"] = new_obs
    else:
        cyberfox_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(cyberfox_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        cyberfox_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        cyberfox_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = cyberfox_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        cyberfox_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = cyberfox_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        cyberfox_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                cyberfox_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(cyberfox_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            cyberfox_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        cyberfox_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        cyberfox_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        cyberfox_host["dialecticReasoningLevel"] = new_reasoning
    else:
        cyberfox_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = cyberfox_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        cyberfox_host["sessionStrategy"] = new_strat

    cyberfox_host["enabled"] = True
    cyberfox_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        cyberfox_config = load_config()
        cyberfox_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(cyberfox_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: cyberfox config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    cyberfox honcho status     -- show full config")
    print("    cyberfox honcho mode       -- change recall/observation mode")
    print("    cyberfox honcho tokens     -- tune context and dialectic budgets")
    print("    cyberfox honcho peer       -- update peer names")
    print("    cyberfox honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Cyberfox profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: cyberfox honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'cyberfox honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'cyberfox honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: cyberfox honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        cyberfox = hosts.get(_host_key(), {})
        user = cyberfox.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = cyberfox.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = cyberfox.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = cyberfox.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Cyberfox' identity in Honcho. Seed with 'cyberfox honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: cyberfox honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: cyberfox honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = cyberfox.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = cyberfox.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = cyberfox.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Cyberfox asks Honcho's AI peer a question")
        ...
        print("\n  Set with: cyberfox honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'cyberfox honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'cyberfox honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    cyberfox honcho identity --show        — show both peer representations")
        print("    cyberfox honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Cyberfox + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Cyberfox\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Cyberfox persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  cyberfox honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'cyberfox honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'cyberfox honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  cyberfox honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'cyberfox'")
        print("  with Honcho configured and no prior session history.")
        print("  (Cyberfox calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    cyberfox honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'cyberfox honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Cyberfox, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'cyberfox honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    cyberfox honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  cyberfox honcho identity <path/to/SOUL.md>")
    ...
    print("    Cyberfox:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Cyberfox:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Cyberfox:   Honcho observes every message and updates representations")
    ...
    print("    Cyberfox:   per-session by default — each run gets its own session")
    print("              Map a custom name:  cyberfox honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. cyberfox honcho setup              — configure API key (required)")
        print("  2. cyberfox honcho migrate            — re-run this walkthrough")
    else:
        print("  1. cyberfox honcho status             — verify Honcho connection")
        print("  2. cyberfox                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. cyberfox honcho identity --show    — verify AI peer representation")
        print("  4. cyberfox honcho tokens             — tune context and dialectic budgets")
        print("  5. cyberfox honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'cyberfox memory setup'...\n")
        from cyberfox_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``cyberfox honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``cyberfox honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to cyberfox memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Cyberfox Honcho",
    )
