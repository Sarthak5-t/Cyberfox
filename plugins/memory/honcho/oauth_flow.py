...
``cyberfox://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Cyberfox OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "cyberfox-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``cyberfox-desktop`` / ``cyberfox-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Cyberfox.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local CYBERFOX_HOME override can't reach.
