...
```bash
cyberfox memory setup    # select "mem0"
```
...
Behavioral settings live in `$CYBERFOX_HOME/mem0.json` (set them via `cyberfox memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.cyberfox/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `cyberfox-user` | User identifier on Mem0 |
...
```bash
cyberfox memory setup mem0 --mode oss \
...
```bash
cyberfox memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$CYBERFOX_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.cyberfox/mem0_qdrant"}}
  }
}
```
...
```bash
cyberfox memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$CYBERFOX_HOME/mem0.json`).
