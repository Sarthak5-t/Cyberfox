...
Configuration
-------------
Secret (lives in $CYBERFOX_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $CYBERFOX_HOME/mem0.json, set via `cyberfox memory
setup`):
  ...
  agent_id           — Agent identifier (default: cyberfox)
...
_DEFAULT_USER_ID = "cyberfox-user"
...
def _load_config() -> dict:
    """Load config from env vars, with $CYBERFOX_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "cyberfox"),
        "oss": {},
    }
    ...
    config_path = get_cyberfox_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, cyberfox_home):
        """Write config to $CYBERFOX_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, cyberfox_home: str, config: dict) -> None:
        ...
        post_setup(cyberfox_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $CYBERFOX_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "cyberfox")
