...
def _save_mem0_json(cyberfox_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(cyberfox_home) / "mem0.json"
    existing = {}
...
def _setup_platform(cyberfox_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "cyberfox-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "cyberfox"},
    ...
    provider.save_config(provider_config, cyberfox_home)

    if env_writes:
        _write_env(Path(cyberfox_home) / ".env", env_writes)
...
def _setup_oss(cyberfox_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(cyberfox_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "cyberfox-user")
    ...
    if env_writes:
        _write_env(Path(cyberfox_home) / ".env", env_writes)
    _save_mem0_json(cyberfox_home, {"mode": "oss", "user_id": user_id, "agent_id": "cyberfox", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, cyberfox_home: str) -> str:
...
_PGVECTOR_CONTAINER = "cyberfox-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "cyberfox"
...
def _setup_oss_interactive(cyberfox_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], cyberfox_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], cyberfox_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'cyberfox-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "cyberfox-user")

    agent_id = input("  Agent ID [cyberfox]: ").strip()
    agent_id = agent_id or "cyberfox"
    ...
    if env_writes:
        _write_env(Path(cyberfox_home) / ".env", env_writes)
    _save_mem0_json(cyberfox_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(cyberfox_home: str, config: dict) -> None:
    """Entry point called by cyberfox memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(cyberfox_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(cyberfox_home, config, flags)
        return
