...
- Supermemory API key from [app.supermemory.ai/integrations?connect=cyberfox](http://app.supermemory.ai/integrations?connect=cyberfox)
...
Config file: `$CYBERFOX_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `cyberfox` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `cyberfox-{identity}` → `cyberfox-coder`). |
...
All Supermemory API calls send `x-sm-source: cyberfox`, and document writes stamp
`metadata.sm_source: cyberfox`. This is a **functional routing key, not telemetry**:
it groups Cyberfox-written memories into a dedicated "Cyberfox" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Cyberfox profile:

```json
{
  "container_tag": "cyberfox-{identity}"
}
```

For a profile named `coder`, this resolves to `cyberfox-coder`. The default profile resolves to `cyberfox-default`. Without `{identity}`, all profiles share the same container.
