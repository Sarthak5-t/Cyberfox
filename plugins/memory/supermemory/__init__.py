...
_DEFAULT_CONTAINER_TAG = "cyberfox"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=cyberfox"
...
def _load_supermemory_config(cyberfox_home: str) -> dict:
    config = _default_config()
    config_path = Path(cyberfox_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, cyberfox_home: str) -> None:
    config_path = Path(cyberfox_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "cyberfox"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Cyberfox writes into the "Cyberfox" Space in the Supermemory
        ...
        merged = {"sm_source": "cyberfox", **(metadata or {})}
...
def _resolve_container_tag_for_setup(cyberfox_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(cyberfox_home)
...
def _probe_supermemory_connection(api_key: str, cyberfox_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(cyberfox_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(cyberfox_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `cyberfox memory setup`.
        # All other options are documented for $CYBERFOX_HOME/supermemory.json
    ...
    def save_config(self, values, cyberfox_home):
        ...
        _save_supermemory_config(sanitized, cyberfox_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, cyberfox_home)
    ...
    def post_setup(self, cyberfox_home: str, config: dict) -> None:
        ...
        from cyberfox_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(cyberfox_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from cyberfox_constants import get_cyberfox_home
        self._cyberfox_home = kwargs.get("cyberfox_home") or str(get_cyberfox_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._cyberfox_home)
