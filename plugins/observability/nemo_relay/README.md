...
Optional Cyberfox observability plugin that maps Cyberfox observer hooks to
...
not replace Cyberfox Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Cyberfox emit NeMo Relay lifecycle events for
the work Cyberfox already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Cyberfox Agent can:

- Preserve Cyberfox execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
cyberfox plugins enable observability/nemo_relay
```

The `CYBERFOX_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `CYBERFOX_HOME` that the
agent run will use:

```bash
env CYBERFOX_HOME=/tmp/cyberfox-nemo-relay-test \
  cyberfox plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`CYBERFOX_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`CYBERFOX_HOME` is the Cyberfox profile/config home used by both
`cyberfox plugins enable ...` and the later `cyberfox chat ...` run. If unset,
Cyberfox uses the user's default home, usually `~/.cyberfox`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export CYBERFOX_HOME=/tmp/cyberfox-nemo-relay-test
cyberfox plugins enable observability/nemo_relay
cyberfox chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `cyberfox` command you run is built from the
...
uv run cyberfox plugins enable observability/nemo_relay
uv run cyberfox chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/cyberfox_agent-*.whl
...
The plugin can configure exporters directly from `CYBERFOX_NEMO_RELAY_*`
...
```bash
export CYBERFOX_NEMO_RELAY_ATOF_ENABLED=1
export CYBERFOX_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export CYBERFOX_NEMO_RELAY_ATIF_ENABLED=1
export CYBERFOX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `CYBERFOX_NEMO_RELAY_ATOF_FILENAME`
- `CYBERFOX_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `CYBERFOX_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `CYBERFOX_NEMO_RELAY_ATIF_AGENT_NAME`
- `CYBERFOX_NEMO_RELAY_ATIF_AGENT_VERSION`
- `CYBERFOX_NEMO_RELAY_ATIF_MODEL_NAME`
- `CYBERFOX_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Cyberfox at it:

```bash
export CYBERFOX_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Cyberfox Agent"
agent_version = "local"
```

When `CYBERFOX_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`CYBERFOX_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`CYBERFOX_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Cyberfox does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Cyberfox keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Cyberfox routes LLM and tool
...
For the full generic Cyberfox middleware contract, see
...
export CYBERFOX_HOME=/tmp/cyberfox-nemo-relay-docs/cyberfox-home
mkdir -p "$CYBERFOX_HOME"

cat > "$CYBERFOX_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Cyberfox session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export CYBERFOX_NEMO_RELAY_ATOF_ENABLED=1
export CYBERFOX_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/cyberfox-nemo-relay-docs/subagent/atof
export CYBERFOX_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export CYBERFOX_NEMO_RELAY_ATOF_MODE=overwrite
export CYBERFOX_NEMO_RELAY_ATIF_ENABLED=1
export CYBERFOX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/cyberfox-nemo-relay-docs/subagent/atif
export CYBERFOX_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export CYBERFOX_NEMO_RELAY_ATIF_AGENT_NAME='Cyberfox Agent E2E'
export CYBERFOX_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export CYBERFOX_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"cyberfox.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Cyberfox Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Cyberfox dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/cyberfox-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/cyberfox-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/cyberfox-nemo-relay-docs/workdir/beta.txt
cd /tmp/cyberfox-nemo-relay-docs/workdir

export CYBERFOX_NEMO_RELAY_ATOF_ENABLED=1
export CYBERFOX_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/cyberfox-nemo-relay-docs/parallel/atof
export CYBERFOX_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export CYBERFOX_NEMO_RELAY_ATOF_MODE=overwrite
export CYBERFOX_NEMO_RELAY_ATIF_ENABLED=1
export CYBERFOX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/cyberfox-nemo-relay-docs/parallel/atif
export CYBERFOX_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export CYBERFOX_NEMO_RELAY_ATIF_AGENT_NAME='Cyberfox Agent E2E'
export CYBERFOX_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Cyberfox sessions map to `agent` scopes.
- Cyberfox API request hooks map to `llm` scope start/end events.
- Cyberfox tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Cyberfox execution middleware to hand
...
Enable it for Cyberfox:

```bash
export CYBERFOX_NEMO_RELAY_PLUGINS_TOML=/tmp/cyberfox-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Cyberfox routes execution
through these boundaries:

```text
Cyberfox provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Cyberfox provider adapter next_call(...)

Cyberfox tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Cyberfox tool dispatcher next_call(...)
```
...
middleware for a local Cyberfox run. This path requires a NeMo Relay runtime that
...
```bash
export CYBERFOX_HOME=/tmp/cyberfox-middleware-test/cyberfox-home
mkdir -p "$CYBERFOX_HOME" /tmp/cyberfox-middleware-test/nemo-relay
...
cat > /tmp/cyberfox-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/cyberfox-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/cyberfox-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Cyberfox Middleware E2E"
agent_version = "local"
...
export CYBERFOX_NEMO_RELAY_PLUGINS_TOML=/tmp/cyberfox-middleware-test/nemo-relay/plugins.toml
