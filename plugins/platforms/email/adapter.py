"""
Email platform adapter for the Cyberfox gateway.

Allows users to interact with Cyberfox by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "cyberfox-agent" "version" "{_cyberfox_version}" '
            '"vendor" "Sarthak5-t" '
            '"support-email" "sarthak59t@gmail.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `cyberfox config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `cyberfox gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Cyberfox Agent")
        ...
        msg_id = f"<cyberfox-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# cyberfox_cli/gateway.py, and the _send_email dispatch in
