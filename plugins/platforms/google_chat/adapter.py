...
# ``model_tools`` import time, so ``cyberfox status``, ``cyberfox chat``, etc.
...
# ``cyberfox_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Cyberfox is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a cyberfox MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${CYBERFOX_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _cyberfox_home = _get_cyberfox_home()
        except (ModuleNotFoundError, ImportError):
            _cyberfox_home = _Path.home() / ".cyberfox"
        self._thread_count_store = _ThreadCountStore(
            _cyberfox_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Cyberfox is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("CYBERFOX_HOME", str(_Path.home() / ".cyberfox"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${CYBERFOX_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a cyberfox MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Cyberfox is thinking…' marker message.
        ...
        "Cyberfox is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Cyberfox is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Cyberfox is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Cyberfox is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Cyberfox is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``cyberfox setup``.

    The setup wizard at ``cyberfox_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.cyberfox/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. cyberfox-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.cyberfox/.env")
    print_info("Restart the gateway: cyberfox gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``cyberfox cron`` running as a
    separate process from ``cyberfox gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Cyberfox plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'cyberfox-agent[google_chat]'",
        ...
        # the "Cyberfox is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Cyberfox is thinking…' marker message "
