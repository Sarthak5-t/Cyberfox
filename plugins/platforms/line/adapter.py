"""
LINE Messaging API platform adapter for Cyberfox Agent.
...
to Cyberfox Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``CYBERFOX_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            cyberfox_home = Path(get_cyberfox_home()).resolve()
        except Exception:
            cyberfox_home = Path.home().joinpath(".cyberfox").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``cyberfox status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``cyberfox status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``cyberfox setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.cyberfox/.env`` via ``cyberfox_cli.config``.
    """
    print()
    ...
    try:
        from cyberfox_cli.config import get_env_var, set_env_var
    except ImportError:
        print("cyberfox_cli.config not available; set LINE_* vars manually in ~/.cyberfox/.env")
        return
