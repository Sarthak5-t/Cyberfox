"""ntfy platform adapter (Cyberfox plugin).
...
a Cyberfox dependency.

This adapter ships as a Cyberfox platform plugin under
``plugins/platforms/ntfy/``. The Cyberfox plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "cyberfox-in"              # subscribe topic (incoming)
          publish_topic: "cyberfox-out"     # optional — defaults to topic
...
_ECHO_TAG = "cyberfox-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``cyberfox cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Cyberfox dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `cyberfox gateway status` without
