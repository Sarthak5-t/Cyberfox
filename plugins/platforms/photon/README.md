...
This plugin connects Cyberfox Agent to iMessage (and other Spectrum
...
TypeScript-only, Cyberfox runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Cyberfox-Sidecar-Token`.
...
There is no separate `login` command; like every other Cyberfox channel,
...
Run `cyberfox photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.cyberfox/.env` (the same place every other
...
Management metadata lives in `~/.cyberfox/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `cyberfox photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `cyberfox photon status`, a DM and a group roundtrip,
