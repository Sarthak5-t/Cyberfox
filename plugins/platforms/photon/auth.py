...
Credential storage mirrors every other Cyberfox channel:

    * runtime SDK creds  -> ``~/.cyberfox/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.cyberfox/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a cyberfox dependency
...
# registers Cyberfox as its own client_id.
...
# Default name of the project Cyberfox provisions for the operator.
DEFAULT_PROJECT_NAME = "Cyberfox Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of cyberfox-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.cyberfox/auth.json`` honouring the active Cyberfox profile."""
    try:
        from cyberfox_constants import get_cyberfox_home
        return Path(get_cyberfox_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.cyberfox")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.cyberfox/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.cyberfox/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.cyberfox/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: cyberfox_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `cyberfox photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `cyberfox photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `cyberfox photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `cyberfox photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `cyberfox photon setup`)"
