...
Small Node helper that bridges Cyberfox Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Cyberfox is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Cyberfox-Sidecar-Token`)
...
The Cyberfox plugin's `cyberfox photon setup` command runs `npm install`
here automatically.
