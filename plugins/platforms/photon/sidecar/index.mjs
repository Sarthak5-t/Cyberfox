// Cyberfox Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Cyberfox): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Cyberfox -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Cyberfox-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `cyberfox photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Cyberfox can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Cyberfox'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Cyberfox allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Cyberfox, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-cyberfox-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Cyberfox supplied them so a known-good
