"""SimpleX Chat platform adapter (Cyberfox plugin).
...
This adapter ships as a Cyberfox platform plugin under
``plugins/platforms/simplex/``. The Cyberfox plugin loader scans the
...
Optional environment variables:
    ...
    CYBERFOX_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``cyberfox setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Cyberfox — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("CYBERFOX_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``cyberfox setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.cyberfox/.env`` via
    ``cyberfox_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "cyberfox_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.cyberfox/.env"
        )
        return
