...
``{title, url, description, position}`` rows every other Cyberfox web
provider produces.
...
prefers Cyberfox-managed xAI Grok OAuth (via ``cyberfox auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.cyberfox/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``cyberfox tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `cyberfox auth` to sign in with "
