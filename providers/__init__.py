...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with cyberfox-agent)
2. User plugins: ``$CYBERFOX_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$CYBERFOX_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$CYBERFOX_HOME/plugins/model-providers/`` if it exists."""
    try:
        from cyberfox_constants import get_cyberfox_home

        d = get_cyberfox_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple CYBERFOX_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_cyberfox_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$CYBERFOX_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with cyberfox-agent.
    ...
    # 2. User plugins — under $CYBERFOX_HOME/plugins/model-providers/<name>/.
