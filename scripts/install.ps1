# ============================================================================
# Cyberfox Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$HermesHome = $(if ($env:CYBERFOX_HOME) { $env:CYBERFOX_HOME } else { "$env:LOCALAPPDATA\hermes" }),
    [string]$InstallDir = $(if ($env:CYBERFOX_HOME) { "$env:CYBERFOX_HOME\hermes-agent" } else { "$env:LOCALAPPDATA\hermes\hermes-agent" }),
...
function Write-Banner {
    ...
    Write-Host "|             * Cyberfox Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Resolve-UvCmd {
    ...
    # sibling process and CYBERFOX_HOME wasn't propagated).
...
function Install-Venv {
    ...
    if (Test-Path "venv") {
        ...
        if ($env:OS -eq "Windows_NT") {
            ...
            # `pythonw.exe -m cyberfox_cli.main gateway run` straight out of
            ...
            # user (see cyberfox_cli/gateway_windows.py), so the installer always
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m cyberfox_cli.main <command>"
                    } else {
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\cyberfox_cli\web_server.py" 2>&1 | Out-Null
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: cyberfox_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    ...
    # Set CYBERFOX_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\hermes instead
    # of the Unix default ~/.hermes
    $currentHermesHome = [Environment]::GetEnvironmentVariable("CYBERFOX_HOME", "User")
    if (-not $currentHermesHome -or $currentHermesHome -ne $HermesHome) {
        [Environment]::SetEnvironmentVariable("CYBERFOX_HOME", $HermesHome, "User")
        Write-Success "Set CYBERFOX_HOME=$HermesHome"
    }
...
function Copy-ConfigTemplates {
    ...
    # Create the CYBERFOX_HOME directory structure ($HermesHome, default %LOCALAPPDATA%\hermes)
    ...
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in cyberfox_cli/default_soul.py. The runtime
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m cyberfox_cli.main setup
    } else {
