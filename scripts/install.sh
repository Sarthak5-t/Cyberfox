...
# Python-driven tool session (e.g. Cyberfox terminal tool). A pre-set PYTHONPATH
...
if [ -n "${CYBERFOX_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$CYBERFOX_INSTALL_DIR"
...
#   code at /usr/local/lib/cyberfox-agent, command at /usr/local/bin/cyberfox,
#   data still at /root/.cyberfox (CYBERFOX_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$CYBERFOX_HOME/.no-bundled-skills so future"
            echo "                   'cyberfox update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Cyberfox.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.cyberfox/cyberfox-agent"
            echo "                   default (root, Linux): /usr/local/lib/cyberfox-agent"
            echo "  --cyberfox-home PATH  Data directory (default: ~/.cyberfox, or \$CYBERFOX_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Cyberfox installs the code under"
            echo "  /usr/local/lib/cyberfox-agent and links the command into"
            echo "  /usr/local/bin/cyberfox (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$CYBERFOX_HOME"
            echo "  (default /root/.cyberfox).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$CYBERFOX_HOME/cyberfox-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs cyberfox setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `cyberfox update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Cyberfox-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Cyberfox Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install cyberfox command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `cyberfox` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $CYBERFOX_HOME/cyberfox-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $CYBERFOX_HOME/cyberfox-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/cyberfox-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $CYBERFOX_HOME/cyberfox-agent — then preserve it)
#
# Always no-op when the user set --dir or $CYBERFOX_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in CYBERFOX_HOME.
    if is_termux; then
        INSTALL_DIR="$CYBERFOX_HOME/cyberfox-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$CYBERFOX_HOME/cyberfox-agent/.git" ]; then
            INSTALL_DIR="$CYBERFOX_HOME/cyberfox-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/cyberfox-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/cyberfox-agent"
        ...
        # /usr/local/bin/cyberfox wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/cyberfox"
        log_info "  Data:    $CYBERFOX_HOME (unchanged)"
...
# Point a Cyberfox-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $CYBERFOX_HOME/node/bin — which is NOT on
...
# Cyberfox's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Cyberfox-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$CYBERFOX_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$CYBERFOX_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$CYBERFOX_HOME/node/etc/npmrc"
}

get_cyberfox_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/cyberfox" ]; then
        echo "$link_dir/cyberfox"
    else
        echo "cyberfox"
    fi
}
...
install_uv() {
    ...
    # Cyberfox owns its own uv at $CYBERFOX_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (cyberfox_cli/managed_uv.py) looks in the same
    # place, so install.sh and `cyberfox update` stay in sync.
    local _managed_uv="$CYBERFOX_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $CYBERFOX_HOME/bin ..."
    mkdir -p "$CYBERFOX_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/cyberfox-uv-install.$$.log")"
    ...
    # directly into $CYBERFOX_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$CYBERFOX_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Cyberfox-
...
check_node() {
    ...
    # Repair pre-existing Cyberfox-managed installs where `npm install -g` lands
    ...
    # Prefer a Cyberfox-managed Node from a previous run over a too-old system one.
    if [ -x "$CYBERFOX_HOME/node/bin/node" ] && node_satisfies_build "$("$CYBERFOX_HOME/node/bin/node" --version)"; then
        export PATH="$CYBERFOX_HOME/node/bin:$PATH"
        log_success "Node.js $("$CYBERFOX_HOME/node/bin/node" --version) found (Cyberfox-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Cyberfox-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.cyberfox/node/..."
    ...
    # Place into ~/.cyberfox/node/ and symlink binaries into the same bin dir
    # the cyberfox command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$CYBERFOX_HOME/node"
    mkdir -p "$CYBERFOX_HOME"
    mv "$extracted_dir" "$CYBERFOX_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$CYBERFOX_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$CYBERFOX_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$CYBERFOX_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$CYBERFOX_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.cyberfox/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Cyberfox install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Cyberfox Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `cyberfox update` path
                # (#4735).
                ...
                local stash_name
                stash_name="cyberfox-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``cyberfox update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Cyberfox behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"cyberfox-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up cyberfox command..."

    if [ "$USE_VENV" = true ]; then
        CYBERFOX_BIN="$INSTALL_DIR/venv/bin/cyberfox"
    else
        CYBERFOX_BIN="$(which cyberfox 2>/dev/null || echo "")"
        if [ -z "$CYBERFOX_BIN" ]; then
            log_warn "cyberfox not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$CYBERFOX_BIN" ]; then
        log_warn "cyberfox entry point not found at $CYBERFOX_BIN"
    ...
    # Create a user-facing shim for the cyberfox command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $CYBERFOX_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$CYBERFOX_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/cyberfox"
    cat > "$command_link_dir/cyberfox" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CYBERFOX_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/cyberfox"
    log_success "Installed cyberfox launcher → $command_link_display_dir/cyberfox"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "cyberfox command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v cyberfox' \
                >/dev/null 2>&1; then
        ...
        log_info "cyberfox not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Cyberfox Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Cyberfox Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so cyberfox works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.cyberfox directory structure (config at top level, code in subdir)
    mkdir -p "$CYBERFOX_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.cyberfox/.env (top level, easy to find)
    if [ ! -f "$CYBERFOX_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$CYBERFOX_HOME/.env"
            log_success "Created ~/.cyberfox/.env from template"
        else
            touch "$CYBERFOX_HOME/.env"
            log_success "Created ~/.cyberfox/.env"
        fi
    else
        log_info "~/.cyberfox/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$CYBERFOX_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.cyberfox/config.yaml (top level, easy to find)
    if [ ! -f "$CYBERFOX_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$CYBERFOX_HOME/config.yaml"
            log_success "Created ~/.cyberfox/config.yaml from template"
        fi
    else
        log_info "~/.cyberfox/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in cyberfox_cli/default_soul.py — the
...
SOUL_EOF
    ...
    log_success "Configuration directory ready: ~/.cyberfox/"

    # Seed bundled skills into ~/.cyberfox/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `cyberfox update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'cyberfox update'." \
            > "$CYBERFOX_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $CYBERFOX_HOME/.no-bundled-skills"
        log_info "  Future 'cyberfox update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.cyberfox/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.cyberfox/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$CYBERFOX_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$CYBERFOX_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.cyberfox/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$CYBERFOX_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Cyberfox Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Cyberfox will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Cyberfox Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (cyberfox --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `cyberfox update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'cyberfox setup' after install."
        return 0
    fi
    ...
    # Run cyberfox setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m cyberfox_cli.main setup < /dev/tty
    else
        python -m cyberfox_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$CYBERFOX_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Cyberfox to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$CYBERFOX_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'cyberfox whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                CYBERFOX_CMD="$(get_cyberfox_command_path)"
                $CYBERFOX_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'cyberfox whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'cyberfox gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $CYBERFOX_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $CYBERFOX_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: cyberfox gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: cyberfox gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $CYBERFOX_CMD gateway > "$CYBERFOX_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.cyberfox/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: cyberfox gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: cyberfox gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $CYBERFOX_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $CYBERFOX_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $CYBERFOX_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}cyberfox${NC}              Start chatting"
    echo -e "   ${GREEN}cyberfox setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}cyberfox config${NC}       View/edit configuration"
    echo -e "   ${GREEN}cyberfox config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}cyberfox gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}cyberfox update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'cyberfox' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'cyberfox' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'cyberfox' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$CYBERFOX_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$CYBERFOX_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$CYBERFOX_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$CYBERFOX_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$CYBERFOX_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Cyberfox for pip install"
    ...
    CYBERFOX_CMD="$(command -v cyberfox 2>/dev/null || echo "")"
    if [ -n "$CYBERFOX_CMD" ]; then
        log_info "Running cyberfox setup..."
        "$CYBERFOX_CMD" setup
    else
        log_warn "cyberfox command not found on PATH"
        log_info "Try: python -m cyberfox_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Cyberfox` rename
...
# _purge_electron_build_cache() used by `cyberfox desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Cyberfox-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `cyberfox desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Cyberfox" ]; then
            app="$desktop_dir/release/linux-unpacked/Cyberfox"
        elif [ -x "$desktop_dir/release/linux-unpacked/cyberfox" ]; then
            app="$desktop_dir/release/linux-unpacked/cyberfox"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Cyberfox.app" \
            "$desktop_dir/release/mac/Cyberfox.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Cyberfox is damaged and can't be opened").
    ...
    # checkout stays clean for the next `cyberfox update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Cyberfox-managed Node
            # provisioned during prerequisites/node-deps (at $CYBERFOX_HOME/node/bin)
        ...
        complete)
            ...
            # $CYBERFOX_HOME. $CYBERFOX_HOME is a shared data dir (it can be
            # bind-mounted into a Docker gateway too), so a stamp there gets
            # clobbered by the container's 'docker' stamp and wrongly blocks
            # 'cyberfox update' on this host install. See detect_install_method().
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $CYBERFOX_HOME.
    # $CYBERFOX_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'cyberfox update' on this host install.
