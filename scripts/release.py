...
VERSION_FILE = REPO_ROOT / "cyberfox_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level CYBERFOX_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (cyberfox journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "cyberfox.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.cyberfox/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "brett@personalfinancelab.com": "brett539",  # PR #49369 salvage (cap Telegram initialize() with asyncio.wait_for(CYBERFOX_TELEGRAM_INIT_TIMEOUT, default 30s) per attempt so an unreachable fallback-IP connect chain can't block gateway startup indefinitely; add WARNING progress logs before DoH discovery and each connect attempt)
    ...
    "hodlclone@gmail.com": "HODLCLONE",  # PR #49351 salvage (portal token resilience: rotate refresh tokens write-through to the source auth store in profile mode, skip fallback when no local token, sync gateway session model after fallback)
    ...
    "moonsong@sarthak5t.github.io": "Tranquil-Flow",  # PR #52623 salvage (auxiliary Anthropic base_url host validation; #52608)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows cyberfox-shim resolution + prefer --update on recovery; #52378)
    ...
    "cyberfox@marian.local": "Schrotti77",
    ...
    "274096618+cyberfox-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "sarthak59t@gmail.com": "sarthak5-t",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on cyberfox mcp add)
    ...
    "cyberfox@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@cyberfox-agent": "teknium1",
    ...
    "github@nadyacyberfox.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (CYBERFOX_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (CYBERFOX_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: cyberfox-eval community link)
    ...
    "cyberfoxagent26@gmail.com": "cyberfoxagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+cyberfoxagent26@users.noreply.github.com": "cyberfoxagent26",
    ...
    "vigo@cyberfox": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.cyberfox@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Cyberfox runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level CYBERFOX_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Cyberfox
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"cyberfox-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "sarthak59t@gmail.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/Sarthak5-t/Cyberfox",
    ...
    lines.append(f"# Cyberfox Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Cyberfox Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Cyberfox Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Cyberfox Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Cyberfox Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Cyberfox Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Cyberfox Agent v{new_version} ({calver_date})' "
