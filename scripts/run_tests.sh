#!/usr/bin/env bash
# Canonical test runner for cyberfox-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.cyberfox/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.cyberfox/cyberfox-agent/venv"; do
...
if [ -f "$HOME/.cyberfox/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.cyberfox"
...
exec env -i \
  ...
  ${CYBERFOX_RUN_SLOW_PET_TESTS:+CYBERFOX_RUN_SLOW_PET_TESTS="$CYBERFOX_RUN_SLOW_PET_TESTS"} \
