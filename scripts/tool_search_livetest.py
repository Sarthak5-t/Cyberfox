#!/usr/bin/env python3
"""Live test harness for Cyberfox Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any cyberfox imports.
ORIGINAL_HOME = os.environ.get("CYBERFOX_HOME")
ORIGINAL_AUTH = Path.home() / ".cyberfox" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.cyberfox/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.cyberfox/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="cyberfox_ts_live_"))
    cyberfox_home = home_dir / ".cyberfox"
    cyberfox_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, cyberfox_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".cyberfox" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, cyberfox_home / ".env")
        ...
        load_cyberfox_dotenv(cyberfox_home=str(Path.home() / ".cyberfox"))
    ...
    (cyberfox_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new CYBERFOX_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "cyberfox_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["CYBERFOX_HOME"] = str(home)
...
def main():
    ...
    # Restore original CYBERFOX_HOME
    if ORIGINAL_HOME is not None:
        os.environ["CYBERFOX_HOME"] = ORIGINAL_HOME
    else:
