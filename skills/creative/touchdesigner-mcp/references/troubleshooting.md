...
## 5. Cyberfox Configuration

### Config location

`$CYBERFOX_HOME/config.yaml` (defaults to `~/.cyberfox/config.yaml` when `CYBERFOX_HOME` is unset)
...
Restart the Cyberfox session for changes to take effect. The MCP connection is
