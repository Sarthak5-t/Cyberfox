...
CYBERFOX_HOME_DIR="${CYBERFOX_HOME:-$HOME/.cyberfox}"
CYBERFOX_CFG="${CYBERFOX_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Cyberfox config has twozero_td MCP entry ──
if [[ ! -f "$CYBERFOX_CFG" ]]; then
    echo -e " ${FAIL} Cyberfox config not found at ${CYBERFOX_CFG}"
    manual_steps+=("Create ${CYBERFOX_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$CYBERFOX_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Cyberfox config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Cyberfox config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$CYBERFOX_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${CYBERFOX_CFG} manually"); }
    manual_steps+=("Restart Cyberfox session to pick up config change")
fi
