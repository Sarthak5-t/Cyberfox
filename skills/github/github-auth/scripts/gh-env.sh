#!/usr/bin/env bash
# GitHub environment detection helper for Cyberfox Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _cyberfox_env="${CYBERFOX_HOME:-$HOME/.cyberfox}/.env"; [ -f "$_cyberfox_env" ] && grep -q "^GITHUB_TOKEN=" "$_cyberfox_env" 2>/dev/null; then
