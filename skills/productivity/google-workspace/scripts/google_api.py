#!/usr/bin/env python3
"""Google Workspace API CLI for Cyberfox Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Cyberfox-facing JSON contract and falls back to the Python client
...
from _cyberfox_home import get_cyberfox_home
...
def _gws_binary() -> str | None:
    override = os.getenv("CYBERFOX_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Cyberfox Agent")
