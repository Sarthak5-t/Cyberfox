...
- 96vc: ACP didn't set CYBERFOX_EXEC_ASK, so `check_all_command_guards`
...
1. Setting CYBERFOX_EXEC_ASK inside _run_agent (wraps the agent call).
...
class TestThreadLocalApprovalCallback:
    ...
    def test_sudo_password_cache_isolated_across_acp_sessions_on_same_pool_thread(self):
        ...
        binds CYBERFOX_SESSION_KEY per session, so the cache scope key differs
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set CYBERFOX_INTERACTIVE so
    ...
    (CYBERFOX_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When CYBERFOX_INTERACTIVE is set and an approval callback is
        ...
        # Without CYBERFOX_INTERACTIVE: takes auto-approve path, callback NOT called
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert result["approved"] is True
        assert called_with == [], (
            "without CYBERFOX_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With CYBERFOX_INTERACTIVE: callback IS called, approval flows through it
        monkeypatch.setenv("CYBERFOX_INTERACTIVE", "1")
        called_with.clear()
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert called_with, (
            "with CYBERFOX_INTERACTIVE the approval path should consult the "
    ...
    def test_interactive_context_var_routes_to_callback_without_env(
        ...
        tok = set_cyberfox_interactive_context(True)
        try:
            result = check_all_command_guards(
            ...
            reset_cyberfox_interactive_context(tok)

        assert called_with, (
            "set_cyberfox_interactive_context(True) should route dangerous "
            "commands through the callback without CYBERFOX_INTERACTIVE in env"
        )
        assert result["approved"] is True
