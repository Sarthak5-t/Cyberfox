"""Tests for ACP Registry metadata shipped with Cyberfox."""
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"cyberfox-agent[acp]=={data['version']}"
    assert uvx["args"] == ["cyberfox-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"cyberfox-agent[acp]=={_pyproject_version()}"
    )
