...
def test_root_session_no_compression(db):
    ...
    assert prov["currentCyberfoxSessionId"] == "root1"
    assert prov["rootCyberfoxSessionId"] == "root1"
    assert prov["parentCyberfoxSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_cyberfox_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentCyberfoxSessionId"] == "old"
    assert prov["rootCyberfoxSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousCyberfoxSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootCyberfoxSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootCyberfoxSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_cyberfox_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousCyberfoxSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"cyberfox"}
    assert "sessionProvenance" in meta["cyberfox"]
    assert meta["cyberfox"]["sessionProvenance"]["currentCyberfoxSessionId"] == "root1"
