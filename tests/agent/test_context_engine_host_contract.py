...
engine plugins (e.g. cyberfox-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``cyberfox plugins`` picker."""
    from cyberfox_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "cyberfox_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "cyberfox_cli.plugins.get_plugin_context_engine",
