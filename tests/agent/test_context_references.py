...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Cyberfox Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_cyberfox_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("CYBERFOX_HOME", str(tmp_path / ".cyberfox"))

    cyberfox_env = tmp_path / ".cyberfox" / ".env"
    cyberfox_env.parent.mkdir(parents=True)
    cyberfox_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.cyberfox/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.cyberfox/auth.json` and read the
    ...
    oauth = cyberfox_home / ".anthropic_oauth.json"
    oauth.write_text('{"access_token": "OAUTH-SECRET"}\n', encoding="utf-8")

    mcp_token = cyberfox_home / "mcp-tokens" / "github.json"
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.cyberfox/auth.json and @file:.cyberfox/.anthropic_oauth.json "
        "and @file:.cyberfox/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
        allowed_root=tmp_path,
        context_length=100_000,
    )
...
async def test_canonical_guard_fails_closed_when_lookup_raises(tmp_path: Path, monkeypatch):
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.cyberfox/auth.json",
        cwd=tmp_path,
        allowed_root=tmp_path,
        context_length=100_000,
    )
