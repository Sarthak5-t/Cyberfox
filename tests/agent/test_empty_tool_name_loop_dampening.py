...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated CYBERFOX_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="cyberfox_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".cyberfox"))
    prev_home = os.environ.get("CYBERFOX_HOME")
    os.environ["CYBERFOX_HOME"] = os.path.join(test_home, ".cyberfox")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("cyberfox_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["CYBERFOX_HOME"] = prev_home
