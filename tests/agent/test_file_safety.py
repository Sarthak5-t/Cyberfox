...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_cyberfox_env(self):
        """Cyberfox' own .env inside CYBERFOX_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: cyberfox internal .env is in ~/.cyberfox/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.cyberfox/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.cyberfox/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Cyberfox cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        cyberfox_home = tmp_path / ".cyberfox"
        cache = cyberfox_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._cyberfox_home_path", return_value=cyberfox_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Cyberfox cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        cyberfox_home = tmp_path / ".cyberfox"
        hub = cyberfox_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_cyberfox_home(self, tmp_path):
        """The env basename guard does not depend on CYBERFOX_HOME resolution."""
        cyberfox_home = tmp_path / ".cyberfox"
        cyberfox_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        cyberfox_home = tmp_path / ".cyberfox"
        cache = cyberfox_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
