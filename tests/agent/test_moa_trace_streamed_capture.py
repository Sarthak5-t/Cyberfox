...
path via ``cyberfox chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp CYBERFOX_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point CYBERFOX_HOME at a temp dir and turn moa.save_traces on."""
    cyberfox_home = tmp_path / ".cyberfox"
    cyberfox_home.mkdir()
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))

    # save_moa_turn reads config via cyberfox_cli.config.load_config; stub it to
    ...
    return cyberfox_home / "moa-traces"
