...
""",
        ...
        from cyberfox_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        cyberfox_home = tmp_path / ".cyberfox"
        cyberfox_home.mkdir()
        (cyberfox_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
        from cyberfox_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from cyberfox_cli.plugins import PluginContext, PluginManifest, PluginManager
