...
def test_cyberfox_client_tag_includes_current_version():
    """The client tag must reflect cyberfox_cli.__version__ verbatim."""
    from cyberfox_cli import __version__
    from agent.portal_tags import cyberfox_client_tag

    assert cyberfox_client_tag() == f"client=cyberfox-client-v{__version__}"


def test_cyberfox_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import cyberfox_client_tag

    tag = cyberfox_client_tag()
    assert tag.startswith("client=cyberfox-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import cyberfox_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=cyberfox-agent" in tags
    assert cyberfox_client_tag() in tags
    assert len(tags) == 2
