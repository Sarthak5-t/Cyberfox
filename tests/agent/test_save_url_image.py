...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate CYBERFOX_HOME under tmp_path."""
    monkeypatch.setenv("CYBERFOX_HOME", str(tmp_path / ".cyberfox"))
    (tmp_path / ".cyberfox").mkdir()

    # Force the constants/image cache helpers to re-read CYBERFOX_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("cyberfox_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_cyberfox_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under CYBERFOX_HOME — gateway cleanup
