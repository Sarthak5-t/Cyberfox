...
class TestChatCompletionsBasic:
    ...
    def test_convert_messages_strips_internal_scaffolding_markers(self, transport):
        """Cyberfox-internal ``_``-prefixed markers must never reach the wire.
...
class TestChatCompletionsBuildKwargs:
    ...
    def test_gemma_does_not_receive_thinking_config(self, transport):
        ...
        # when Cyberfox has reasoning enabled, the field must be omitted for
...
class TestChatCompletionsNormalize:
    ...
    def test_refusal_field_promoted_to_content_filter(self, transport):
        """OpenAI-compatible proxies (e.g. cyberfox portal fronting Anthropic) can
...
class TestChatCompletionsGeminiNativeExtraBodyStrip:
    """Profile extra_body (e.g. portal tags) must not reach a native
    Gemini endpoint — Google's REST API rejects unknown fields with HTTP 400.
    """

    def _tags_profile(self):
        from providers import ProviderProfile

        class _TagProfile(ProviderProfile):
            def build_extra_body(self, *, session_id=None, **context):
                return {"tags": ["cyberfox", "alpha"]}

        return _TagProfile(name="tagged")
    ...
    def test_tags_stripped_when_endpoint_is_native_gemini(self, transport):
        kw = transport.build_kwargs(
            ...
            [{"role": "user", "content": "hi"}],
            None,
            provider_profile=self._tags_profile(),
            base_url="https://generativelanguage.googleapis.com/v1beta",
            session_id="s1",
            max_tokens=None,
        )
    ...
    def test_tags_preserved_on_openai_compat_endpoint(self, transport):
        kw = transport.build_kwargs(
            "cyberfox-3-405b",
            [{"role": "user", "content": "hi"}],
            None,
