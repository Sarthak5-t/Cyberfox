"""Shared fixtures for the cyberfox-agent test suite.
...
2. **Isolated CYBERFOX_HOME.** CYBERFOX_HOME points to a per-test tempdir so
   code reading ``~/.cyberfox/*`` via ``get_cyberfox_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".cyberfox"`` instead of the canonical
   ``get_cyberfox_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No CYBERFOX_SESSION_* inheritance** — the agent's current gateway
...
# CYBERFOX_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_CYBERFOX_BEHAVIORAL_VARS = frozenset({
    "CYBERFOX_YOLO_MODE",
    "CYBERFOX_INTERACTIVE",
    "CYBERFOX_QUIET",
    "CYBERFOX_TOOL_PROGRESS",
    "CYBERFOX_TOOL_PROGRESS_MODE",
    "CYBERFOX_MAX_ITERATIONS",
    "CYBERFOX_SESSION_PLATFORM",
    "CYBERFOX_SESSION_CHAT_ID",
    "CYBERFOX_SESSION_CHAT_NAME",
    "CYBERFOX_SESSION_THREAD_ID",
    "CYBERFOX_SESSION_SOURCE",
    "CYBERFOX_SESSION_KEY",
    "CYBERFOX_GATEWAY_SESSION",
    "CYBERFOX_CRON_SESSION",
    "_CYBERFOX_GATEWAY",
    "CYBERFOX_PLATFORM",
    "CYBERFOX_MODEL",
    "CYBERFOX_INFERENCE_MODEL",
    "CYBERFOX_INFERENCE_PROVIDER",
    "CYBERFOX_TUI_PROVIDER",
    "CYBERFOX_MANAGED",
    "CYBERFOX_MANAGED_DIR",
    "CYBERFOX_DEV",
    "CYBERFOX_CONTAINER",
    "CYBERFOX_EPHEMERAL_SYSTEM_PROMPT",
    "CYBERFOX_TIMEZONE",
    "CYBERFOX_REDACT_SECRETS",
    "CYBERFOX_BACKGROUND_NOTIFICATIONS",
    "CYBERFOX_EXEC_ASK",
    "CYBERFOX_HOME_MODE",
    "CYBERFOX_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.cyberfox/kanban.db instead of the per-test CYBERFOX_HOME.
    "CYBERFOX_KANBAN_DB",
    "CYBERFOX_KANBAN_BOARD",
    "CYBERFOX_KANBAN_HOME",
    "CYBERFOX_KANBAN_WORKSPACES_ROOT",
    "CYBERFOX_KANBAN_LOGS_ROOT",
    "CYBERFOX_KANBAN_TASK",
    "CYBERFOX_KANBAN_WORKSPACE",
    "CYBERFOX_KANBAN_RUN_ID",
    "CYBERFOX_KANBAN_CLAIM_LOCK",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and CYBERFOX_HOME to per-test tempdirs so code that
    reads ``~/.cyberfox/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral CYBERFOX_* vars that could change test semantics.
    for name in _CYBERFOX_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect CYBERFOX_HOME to a per-test tempdir. Code that reads
    #    ``~/.cyberfox/*`` via ``get_cyberfox_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.cyberfox/*`` via
    #    ``Path.home() / ".cyberfox"`` instead of ``get_cyberfox_home()``
    #    is a bug to fix at the callsite.
    fake_cyberfox_home = tmp_path / "cyberfox_test"
    fake_cyberfox_home.mkdir()
    (fake_cyberfox_home / "sessions").mkdir()
    (fake_cyberfox_home / "cron").mkdir()
    (fake_cyberfox_home / "memories").mkdir()
    (fake_cyberfox_home / "skills").mkdir()
    monkeypatch.setenv("CYBERFOX_HOME", str(fake_cyberfox_home))
    ...
    #    ~/.cyberfox/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import cyberfox_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_cyberfox_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal cyberfox config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``cyberfox-gateway`` process
...
# correlated with a ``tests/cyberfox_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> cyberfox-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart cyberfox-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting cyberfox/python patterns are also blocked.
    """
    ...
    def _matches_cyberfox_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _CYBERFOX_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_cyberfox_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch cyberfox-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m cyberfox_cli.main".
                if (
                    "cyberfox" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live cyberfox-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting cyberfox/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `cyberfox update` (or the
        # equivalent `python -m cyberfox_cli.main update`).  These commands
        ...
        if "update" in low and (
            # cyberfox update / cyberfox update --gateway / setsid bash -c ... cyberfox update
            ("cyberfox" in low and "update" in low.split())
            or
            # python -m cyberfox_cli.main update --gateway
            ("cyberfox_cli" in low and "update" in low.split())
            or
            # venv/bin/cyberfox update  (absolute path variant used in tests)
            (".venv/bin/cyberfox" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`cyberfox update` against the real checkout, fetching "
