...
cyberfox/claude-fable-5), the job would silently inherit it on its next tick and
...
resolve_runtime_provider against a temp CYBERFOX_HOME) and the create_job
...
class TestProviderDriftGuard:
    ...
    def test_b_unpinned_snapshot_differs_fails_closed(self, tmp_path):
        ...
        job = _base_job(provider_snapshot="openrouter")
        success, output, final_response, error, agent_constructed = \
            _run_with_current_provider(job, "cyberfox", tmp_path)
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("cyberfox_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
                prompt="do a thing", schedule="every 1 hour", provider="cyberfox"
            )

        # Explicit provider → pinned → no snapshot needed, and resolution skipped.
        assert job["provider"] == "cyberfox"
    ...
    def test_unpinned_model_captures_model_snapshot(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "cron.jobs.get_cyberfox_home", lambda: tmp_path, raising=True
        )
        with patch(
            "cyberfox_cli.runtime_provider.resolve_runtime_provider",
            return_value={"provider": "openrouter"},
...
def _run_with_current_provider_and_model(job, current_provider, current_model, tmp_path):
    ...
    fake_db = MagicMock()
    with patch("cron.scheduler._cyberfox_home", tmp_path), \
         patch("cron.scheduler._get_cyberfox_home", return_value=tmp_path), \
         patch("cron.scheduler._resolve_origin", return_value=None), \
         patch("cyberfox_cli.env_loader.load_cyberfox_dotenv"), \
         patch("cyberfox_cli.env_loader.reset_secret_source_cache"), \
         patch("cyberfox_state.SessionDB", return_value=fake_db), \
