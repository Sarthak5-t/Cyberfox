...
- Path resolution (absolute, relative to CYBERFOX_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp CYBERFOX_HOME."""
    cyberfox_home = tmp_path / ".cyberfox"
    cyberfox_home.mkdir()
    (cyberfox_home / "cron").mkdir()
    (cyberfox_home / "cron" / "output").mkdir()
    (cyberfox_home / "scripts").mkdir()
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "CYBERFOX_DIR", cyberfox_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", cyberfox_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", cyberfox_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", cyberfox_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Cyberfox provider env (SECURITY.md §2.3)."""
        from tools.environments.local import _CYBERFOX_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_CYBERFOX_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.cyberfox/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("CYBERFOX_SESSION_PLATFORM") is None
        assert os.environ.get("CYBERFOX_SESSION_CHAT_ID") is None
        assert os.environ.get("CYBERFOX_SESSION_CHAT_NAME") is None
