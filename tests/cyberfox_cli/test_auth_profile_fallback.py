...
When ``CYBERFOX_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.cyberfox/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.cyberfox            (has its own auth.json fixture)
    * Profile     -> tmp_path/.cyberfox/profiles/coder   (active, CYBERFOX_HOME points here)
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    cyberfox_home = tmp_path / "classic"
    cyberfox_home.mkdir()
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (CYBERFOX_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.cyberfox as the "real home" — which is NOT equal
    # to the CYBERFOX_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    cyberfox_home = tmp_path / "classic"
    ...
    _write(cyberfox_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from cyberfox_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: CYBERFOX_HOME is set to a custom path that is NOT under
    # ~/.cyberfox/profiles/ — get_default_cyberfox_root() returns CYBERFOX_HOME
