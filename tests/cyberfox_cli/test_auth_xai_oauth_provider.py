"""Tests for xAI Grok OAuth — tokens stored in Cyberfox auth store (~/.cyberfox/auth.json)."""
...
def _setup_cyberfox_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Cyberfox auth store at the given root."""
    ...
    auth_file = cyberfox_home / "auth.json"
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("cyberfox_cli.auth.httpx.Client", _factory)
    return holder
...
def test_xai_oauth_poll_device_token_waits_until_authorized(monkeypatch):
    ...
    monkeypatch.setattr("cyberfox_cli.auth.time.sleep", lambda _: None)
...
def test_save_and_read_xai_oauth_tokens_roundtrip(tmp_path, monkeypatch):
    ...
    cyberfox_home.mkdir(parents=True, exist_ok=True)
    (cyberfox_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
...
def test_read_xai_oauth_tokens_missing_access_token(tmp_path, monkeypatch):
    cyberfox_home = tmp_path / "cyberfox"
    _setup_cyberfox_auth(cyberfox_home, access_token="")
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    cyberfox_home = tmp_path / "cyberfox"
    _setup_cyberfox_auth(cyberfox_home, refresh_token="")
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    cyberfox_home = tmp_path / "cyberfox"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_cyberfox_auth(cyberfox_home, access_token=fresh)
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
    monkeypatch.delenv("CYBERFOX_XAI_BASE_URL", raising=False)
    ...
    assert creds["source"] == "cyberfox-auth-store"
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("cyberfox_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_cyberfox_auth(cyberfox_home, access_token=fresh)
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
    monkeypatch.setenv("CYBERFOX_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
# ``XAI_BASE_URL`` / ``CYBERFOX_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    cyberfox_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    cyberfox_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (cyberfox_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    cyberfox_home = tmp_path / "cyberfox"
    _seed_xai_oauth_state(cyberfox_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="cyberfox")
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
    ...
    monkeypatch.setattr("cyberfox_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((cyberfox_home / "auth.json").read_text())
    ...
    assert raw["active_provider"] == "cyberfox"
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    cyberfox_home = tmp_path / "cyberfox"
    _seed_xai_oauth_state(cyberfox_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
    ...
    monkeypatch.setattr("cyberfox_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``cyberfox model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``cyberfox model``
    ...
    assert "cyberfox model" not in rendered.lower()
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from cyberfox_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Cyberfox that didn't validate) must not be silently honored on the refresh
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("cyberfox_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `cyberfox model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `cyberfox auth list` reflects truth and
    ...
    cyberfox_home = tmp_path / "cyberfox"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_cyberfox_auth(cyberfox_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
...
def test_credential_pool_device_code_seed_respects_suppression(tmp_path, monkeypatch):
    from agent.credential_pool import load_pool
    from cyberfox_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``cyberfox auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `cyberfox auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((cyberfox_home / "auth.json").read_text())
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Cyberfox invocation."
    )


def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    """remove -> ``cyberfox model`` re-login (``_login_xai_oauth``) must clear the
    ...
    only ``auth_add_command`` cleared it — the ``cyberfox model`` re-login path did
    not. So after remove -> re-login the seed kept skipping and ``cyberfox auth
    ...
    monkeypatch.setattr(
        "cyberfox_cli.auth._update_config_for_provider",
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_cyberfox_auth(cyberfox_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
    ...
    monkeypatch.setattr("cyberfox_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
    ...
    auth_path = cyberfox_home / "auth.json"
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from cyberfox_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("cyberfox_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    cyberfox_home = tmp_path / "cyberfox"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_cyberfox_auth(cyberfox_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Cyberfox process refreshed the singleton
    ...
    Mirrors the proactive sync codex/cyberfox already perform for the same
    reason, and is what makes the pool actually safe to share across
    profiles + Cyberfox processes."""
    from agent.credential_pool import load_pool

    cyberfox_home = tmp_path / "cyberfox"
    in_memory_at = _jwt_with_exp(int(time.time()) + 30)  # near-expiry
    _setup_cyberfox_auth(cyberfox_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
    ...
    (cyberfox_home / "auth.json").write_text(json.dumps(raw))
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_cyberfox_auth(cyberfox_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``cyberfox model`` -> xAI Grok OAuth (or another process
    ...
    cooldown elapses.  Mirrors the codex/cyberfox self-heal pattern."""
    from agent.credential_pool import load_pool, STATUS_EXHAUSTED
    from dataclasses import replace

    cyberfox_home = tmp_path / "cyberfox"
    stale_at = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_cyberfox_auth(cyberfox_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
    ...
    # Simulate the user re-running `cyberfox model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``cyberfox auth add xai-oauth``) own their own refresh-token lifecycle
    ...
    _setup_cyberfox_auth(cyberfox_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`cyberfox auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_auxiliary_client_routes_xai_oauth_through_responses_api(tmp_path, monkeypatch):
    ...
    Cyberfox, etc.).  Users on xAI Grok OAuth would then see surprise charges
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    using it on the sync-back path means every xAI/Codex/Cyberfox refresh in a
    multi-provider setup silently overrides the user's chosen active
    provider (visible to ``cyberfox auth status``, ``cyberfox setup``, and the
    ``cyberfox`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
    no future refactor regresses to the legacy semantic."""
    from agent.credential_pool import load_pool

    cyberfox_home = tmp_path / "cyberfox"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_cyberfox_auth(cyberfox_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
    ...
    assert raw_after["active_provider"] == "openrouter", (
        ...
        "Cyberfox token rotations silently take over multi-provider users' "
