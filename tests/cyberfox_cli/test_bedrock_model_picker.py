...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (CYBERFOX_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via CYBERFOX_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in CYBERFOX_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from cyberfox_cli.providers import CYBERFOX_OVERLAYS
        assert "bedrock" in CYBERFOX_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from cyberfox_cli.providers import CYBERFOX_OVERLAYS
        assert CYBERFOX_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from cyberfox_cli.providers import CYBERFOX_OVERLAYS
        assert CYBERFOX_OVERLAYS["bedrock"].auth_type == "aws_sdk"
