...
def container_env(tmp_path, monkeypatch):
    """Set up a fake CYBERFOX_HOME with .container-mode file."""
    cyberfox_home = tmp_path / ".cyberfox"
    cyberfox_home.mkdir()
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
    monkeypatch.delenv("CYBERFOX_DEV", raising=False)

    container_mode = cyberfox_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=cyberfox-agent\n"
        "exec_user=cyberfox\n"
        "cyberfox_bin=/data/current-package/bin/cyberfox\n"
    )
    return cyberfox_home


def test_get_container_exec_info_returns_metadata(container_env):
    """Reads .container-mode and returns all fields including exec_user."""
    with patch("cyberfox_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "cyberfox-agent"
    assert info["exec_user"] == "cyberfox"
    assert info["cyberfox_bin"] == "/data/current-package/bin/cyberfox"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("cyberfox_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_cyberfox_dev(container_env, monkeypatch):
    """Returns None when CYBERFOX_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("CYBERFOX_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_cyberfox_dev_zero(container_env, monkeypatch):
    """CYBERFOX_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("CYBERFOX_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        cyberfox_home = Path(tmpdir) / ".cyberfox"
        cyberfox_home.mkdir()
        (cyberfox_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("cyberfox_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_cyberfox_home": lambda: cyberfox_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("CYBERFOX_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=cyberfox-custom\n"
        "exec_user=myuser\n"
        "cyberfox_bin=/opt/cyberfox/bin/cyberfox\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "cyberfox-custom"
    assert info["exec_user"] == "myuser"
    assert info["cyberfox_bin"] == "/opt/cyberfox/bin/cyberfox"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from cyberfox_cli.main import _exec_in_container
