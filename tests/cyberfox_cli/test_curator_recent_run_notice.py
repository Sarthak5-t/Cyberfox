...
The notice prints the most recent curator run summary on `cyberfox update`,
...
session start) so users normally never see the rename map. `cyberfox update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(cyberfox_main)
