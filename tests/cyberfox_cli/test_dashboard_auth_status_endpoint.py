...
def test_status_reports_auth_disabled_in_loopback_mode(loopback_client):
    ...
    # Loopback mode has no registered providers (the Cyberfox plugin's env
...
def test_status_preserves_existing_fields(loopback_client):
    ...
    body = r.json()
    expected_keys = {
        "version", "release_date", "cyberfox_home", "config_path", "env_path",
...
_HOST_DETAIL_FIELDS = frozenset({
    "cyberfox_home", "config_path", "env_path", "gateway_pid",
    "gateway_health_url",
})
...
def test_status_includes_host_detail_in_loopback_mode(loopback_client):
    ...
    the StatusPage / ``cyberfox status`` experience for local operators."""
    r = loopback_client.get("/api/status")
    assert r.status_code == 200
    body = r.json()
