...
import cyberfox_cli.doctor as doctor
import cyberfox_cli.gateway as gateway_cli
from cyberfox_cli import doctor as doctor_mod
from cyberfox_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `cyberfox doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "CYBERFOX_HOME", cyberfox_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("CYBERFOX_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("CYBERFOX_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_cyberfox_home(self, tmp_path, provider=""):
        """Create a minimal CYBERFOX_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_cyberfox_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "CYBERFOX_HOME", home)
        ...
        try:
            from cyberfox_cli import auth as _auth_mod
...
def test_run_doctor_accepts_cyberfox_provider_ids_that_catalog_aliases(
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from cyberfox_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("cyberfox_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "cyberfox_cli.doctor", raising=False)
        from cyberfox_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorXaiOAuthStatus:
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        ...
        from cyberfox_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {"logged_in": False})
        monkeypatch.setattr(_auth_mod, "get_minimax_oauth_auth_status", lambda: {"logged_in": True, "region": "global"})
        monkeypatch.delattr(_auth_mod, "get_xai_oauth_auth_status", raising=False)
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale CYBERFOX_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        cyberfox_home = tmp_path / ".cyberfox"
        cyberfox_home.mkdir(parents=True)
        (cyberfox_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"CYBERFOX_MAX_ITERATIONS={ghost}\n")
        (cyberfox_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "CYBERFOX_HOME", cyberfox_home)
        monkeypatch.setattr(doctor_mod, "get_cyberfox_home", lambda: cyberfox_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("CYBERFOX_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("CYBERFOX_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale CYBERFOX_MAX_ITERATIONS" in out
        env_after = (cyberfox_home / ".env").read_text(encoding="utf-8")
        assert "CYBERFOX_MAX_ITERATIONS" not in env_after
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`cyberfox doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken Cyberfox install.
