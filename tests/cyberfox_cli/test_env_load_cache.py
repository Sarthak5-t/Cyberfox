...
The cache exists to keep `cyberfox tools` → "All Platforms" fast: every
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from cyberfox_cli import config as config_mod
    from cyberfox_cli.config import invalidate_env_cache, load_env, save_env_value
    ...
    monkeypatch.setattr(config_mod, "ensure_cyberfox_home", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    nonexistent = Path(tempfile.gettempdir()) / "cyberfox-test-no-such-env-xyz123.env"
    nonexistent.unlink(missing_ok=True)

    try:
        with patch("cyberfox_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
