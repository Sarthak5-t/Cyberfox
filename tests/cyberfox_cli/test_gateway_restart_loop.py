...
Covers:
- Defense 1: gateway stop/restart refuse when _CYBERFOX_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "cyberfox  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "CYBERFOX GATEWAY RESTART",           # uppercase
    ])
    def test_cyberfox_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.cyberfox.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.cyberfox.gateway.plist",
        "launchctl stop ai.cyberfox.gateway",
        "systemctl restart cyberfox-gateway",
        "systemctl stop cyberfox-gateway.service",
        "systemctl start cyberfox-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill cyberfox gateway process",
        "pkill -f cyberfox.*gateway",
        "pkill -f gateway.*cyberfox",          # inverse token order
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "cyberfox cron list",
        "cyberfox update",
        "cyberfox config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `cyberfox gateway start` is benign — starting a gateway from inside a
        ...
        "cyberfox gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway cyberfox
        # services must not be falsely blocked (the old `.*cyberfox` matched any
        # cyberfox token).
        "launchctl unload ai.cyberfox.update-checker.plist",
        "launchctl restart ai.cyberfox.daemon",
        "systemctl restart cyberfox-meta.service",
        "systemctl restart cyberfox-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_cyberfox_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade cyberfox then run cyberfox gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.cyberfox.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_cyberfox_gateway_once.sh). The script must live under
        # CYBERFOX_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("CYBERFOX_HOME", str(tmp_path / ".cyberfox"))
        scripts_dir = tmp_path / ".cyberfox" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify cyberfox gateway stop/restart refuse when _CYBERFOX_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_CYBERFOX_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_CYBERFOX_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _CYBERFOX_GATEWAY=1.

    Issue #37453: systemctl --user restart cyberfox-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart cyberfox-gateway",
        "systemctl --user restart cyberfox-gateway",
        "systemctl stop cyberfox-gateway.service",
        "cyberfox gateway restart",
        "launchctl kickstart gui/501/ai.cyberfox.gateway",
        "pkill -f cyberfox.*gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart cyberfox-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-cyberfox systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _CYBERFOX_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart cyberfox-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart cyberfox-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run cyberfox gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("cyberfox gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfecyberfox gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under CYBERFOX_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.cyberfox.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    cyberfox_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run cyberfox gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CYBERFOX_HOME", str(tmp_path / ".cyberfox"))
        (tmp_path / ".cyberfox").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run cyberfox gateway restart nightly",
        ))
