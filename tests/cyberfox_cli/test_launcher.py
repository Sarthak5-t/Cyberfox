"""Tests for the top-level `./cyberfox` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./cyberfox` should use `cyberfox_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "cyberfox"
    called = []

    fake_main_module = types.ModuleType("cyberfox_cli.main")

    def fake_main():
        called.append("cyberfox_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "cyberfox_cli.main", fake_main_module)
    ...
    assert called == ["cyberfox_cli.main"]
