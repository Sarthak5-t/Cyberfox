...
cyberfox_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `cyberfox config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_cyberfox_home = get_cyberfox_home()`` at import time (module
    singleton), so monkeypatching CYBERFOX_HOME after import doesn't move it.
    ...
    this only matters for tests that swap CYBERFOX_HOME.)
    """
    import cli

    cli._cyberfox_home = home
