...
def cyberfox_home(tmp_path, monkeypatch):
    ...
    # /etc/cyberfox on the dev/CI box can't influence the test.
    monkeypatch.setenv("CYBERFOX_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(cyberfox_home, monkeypatch):
    from cyberfox_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(cyberfox_home, monkeypatch):
    from cyberfox_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(cyberfox_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_cyberfox_dotenv(cyberfox_home=str(home))
