"""Regression test: ``cyberfox mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``cyberfox_cli/_parser.py``).  The dispatcher in ``cyberfox_cli/main.py``
...
result, ``cyberfox mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the cyberfox parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`cyberfox mcp add foo --url ...` must keep args.command == "mcp".
