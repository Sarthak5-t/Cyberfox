"""Tests for the `cyberfox memory reset` CLI command.
...
- Profile-scoped reset (uses CYBERFOX_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake CYBERFOX_HOME with memory files."""
    cyberfox_home = tmp_path / ".cyberfox"
    memories = cyberfox_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nCyberfox repo is at ~/.cyberfox/cyberfox-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `cyberfox memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        cyberfox_home = tmp_path / ".cyberfox"
        (cyberfox_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's CYBERFOX_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_cyberfox_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
