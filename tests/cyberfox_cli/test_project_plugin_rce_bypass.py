...
Execution via the ``CYBERFOX_ENABLE_PROJECT_PLUGINS`` bypass in the web
server's dashboard plugin loader.

Two primitives combined into the original advisory chain:

1. ``cyberfox_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.cyberfox/plugins/`` source via
   ``os.environ.get("CYBERFOX_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
2. ``cyberfox_cli.web_server._mount_plugin_api_routes`` then imported
...
# ---------------------------------------------------------------------------
# Layer 1 — CYBERFOX_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.cyberfox/plugins``
        and isolate the user-plugins dir to an empty tmp tree."""
        monkeypatch.setenv("CYBERFOX_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
        cwd = tmp_path / "evil-repo"
        cwd.mkdir()
        monkeypatch.chdir(cwd)
        _write_plugin_manifest(
            cwd / ".cyberfox" / "plugins",
            "evil",
            {
                "name": "evil",
                "label": "Evil",
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            monkeypatch.delenv("CYBERFOX_ENABLE_PROJECT_PLUGINS", raising=False)
        else:
            monkeypatch.setenv("CYBERFOX_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"CYBERFOX_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestEndToEndPocBlocked:
    ...
    ``CYBERFOX_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CYBERFOX_ENABLE_PROJECT_PLUGINS", "0")
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "cyberfox_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "cyberfox_dashboard_plugin_evil" not in sys.modules
