"""Tests for the per-profile Projects store (cyberfox_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Cyberfox Agent", folders=["/tmp/cyberfox"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "cyberfox-agent"
    assert proj.name == "Cyberfox Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/cyberfox"
    assert [f.path for f in proj.folders] == ["/tmp/cyberfox"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "cyberfox-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Cyberfox Agent")
    pdb.create_project(conn, name="Cyberfox Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["cyberfox-agent", "cyberfox-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' CYBERFOX_HOME.
...
def test_db_path_under_cyberfox_home():
    # Resolves under CYBERFOX_HOME (set by the autouse isolation fixture).
