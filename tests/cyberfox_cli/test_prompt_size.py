"""Tests for the ``cyberfox prompt-size`` diagnostic (issue #34667)."""
...
from cyberfox_cli.prompt_size import (
    _SKILLS_BLOCK_RE,
...
def _seed_memory(cyberfox_home, memory_text="", user_text=""):
    mem_dir = cyberfox_home / "memories"
...
def _seed_skill(cyberfox_home, name, description):
    skill_dir = cyberfox_home / "skills" / "demo" / name
...
def test_runs_offline_without_credentials(isolated_home, monkeypatch):
    ...
    for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "CYBERFOX_API_KEY",
                "ANTHROPIC_API_KEY"):
