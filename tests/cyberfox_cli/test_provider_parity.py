...
the SAME provider universe as ``cyberfox model`` (the CLI/TUI picker).
...
def test_every_cyberfox_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `cyberfox model` universe."""
    ...
    assert not missing, (
        "providers shown in `cyberfox model` but not configurable in the desktop "
