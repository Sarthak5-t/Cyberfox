...
def test_named_custom_provider_does_not_shadow_builtin_provider(monkeypatch):
    """A custom_providers entry named after a canonical provider (``anthropic``)
    must NOT be used — the built-in resolution wins as before."""
    monkeypatch.setattr(
        rp,
        "load_config",
        lambda: {
    ...
    assert resolved["provider"] == "anthropic"
    assert resolved["api_key"] == "pool-token"
    assert resolved["requested_provider"] == "anthropic"
...
def test_named_custom_provider_skipped_for_canonical_built_in(monkeypatch):
    """Companion to the test above: ``anthropic`` is a canonical provider name
    (``resolve_provider('anthropic') == 'anthropic'``), so a custom entry with that name
    ...
    entry = rp._get_named_custom_provider("anthropic")
...
def test_resolve_requested_provider_precedence(monkeypatch):
    monkeypatch.setenv("CYBERFOX_INFERENCE_PROVIDER", "anthropic")
...
class TestAzureFoundryResolution:
    ...
    def test_azure_foundry_missing_api_key_raises(self, monkeypatch):
        ...
        # `get_env_value` reads from ~/.cyberfox/.env — mock it to return None
...
class TestProviderEntryApiKeyEnvAlias:
    ...
    def test_snake_case_api_key_env_normalizes_to_key_env(self):
        from cyberfox_cli.config import _normalize_custom_provider_entry
        entry = {
            "name": "vendor",
    ...
    def test_valid_fields_set_lists_key_env(self):
        ...
        from cyberfox_cli.config import _VALID_CUSTOM_PROVIDER_FIELDS
...
def test_minimax_oauth_runtime_returns_anthropic_messages_mode(monkeypatch):
    ...
    from cyberfox_cli.auth import MINIMAX_OAUTH_GLOBAL_INFERENCE
...
def test_minimax_oauth_runtime_uses_inference_base_url(monkeypatch):
    ...
    from cyberfox_cli.auth import MINIMAX_OAUTH_CN_INFERENCE
