"""Tests for cyberfox_cli.session_filters — CLI time/filter parsing for
`cyberfox sessions prune` / `cyberfox sessions archive`."""
...
class TestBuildPruneFilters:
    ...
    def test_describe_filters_extended(self):
        f = build_prune_filters(_ns(model="gpt-5", provider="cyberfox",
                                    max_cost=0.5))
        desc = describe_filters(f)
        assert "model contains 'gpt-5'" in desc
        assert "provider 'cyberfox'" in desc
        assert "<= $0.5" in desc
