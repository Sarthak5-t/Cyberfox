...
from cyberfox_cli.setup import setup_model_provider
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import cyberfox_cli.gateway as _gw_mod
    ...
    assert "cyberfox gateway" in out
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("cyberfox_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("cyberfox_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "cyberfox_cli.main._model_flow_named_custom",
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "cyberfox_cli.main._prompt_provider_choice",
...
def test_prompt_yes_no_returns_default_when_noninteractive_env_set(monkeypatch):
    """CYBERFOX_NONINTERACTIVE=1 (set by dashboard/desktop spawns) must make
    prompt_yes_no fall back to its default instead of reading stdin."""
    monkeypatch.setenv("CYBERFOX_NONINTERACTIVE", "1")
...
def test_prompt_yes_no_eof_returns_default_instead_of_exiting(monkeypatch):
    ...
    monkeypatch.delenv("CYBERFOX_NONINTERACTIVE", raising=False)
