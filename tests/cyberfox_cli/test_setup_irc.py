"""Tests for IRC gateway configuration via `cyberfox setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Cyberfox install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Cyberfox install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#cyberfox")
            monkeypatch.setenv("IRC_NICKNAME", "cyberfox-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `cyberfox setup gateway` experience with IRC present."""
