...
- Bare `cyberfox setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `cyberfox setup --quick` runs the narrower "fill in missing items" flow.
- `cyberfox setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("cyberfox_cli.setup.ensure_cyberfox_home", {}),
        ("cyberfox_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("cyberfox_cli.config.is_managed", {"return_value": False}),
        ("cyberfox_cli.setup.load_config", {"return_value": {}}),
        ("cyberfox_cli.setup.save_config", {}),
        ("cyberfox_cli.setup.get_env_value", {"return_value": None}),
        ("cyberfox_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("cyberfox_cli.setup._print_setup_summary", {}),
        ("cyberfox_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("cyberfox_cli.setup.load_config", {"return_value": {}}),
        ("cyberfox_cli.setup.save_config", {}),
        ("cyberfox_cli.auth.get_active_provider", {"return_value": None}),
        ("cyberfox_cli.setup.get_env_value", {"return_value": None}),
        ("cyberfox_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `cyberfox setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="cyberfox_cli.setup.prompt_choice",
                quick="cyberfox_cli.setup._run_quick_setup",
                model="cyberfox_cli.setup.setup_model_provider",
                terminal="cyberfox_cli.setup.setup_terminal_backend",
                agent="cyberfox_cli.setup.setup_agent_settings",
                gateway="cyberfox_cli.setup.setup_gateway",
                tools="cyberfox_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`cyberfox setup --reconfigure` behaves the same as bare `cyberfox setup`."""
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "cyberfox_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["cyberfox", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["cyberfox", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["cyberfox", "setup"])
        try:
            main()
        except SystemExit:
            pass
