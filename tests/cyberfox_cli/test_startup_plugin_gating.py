...
``cyberfox_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``cyberfox --help``, ``cyberfox version``,
``cyberfox logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``cyberfox --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from cyberfox_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["cyberfox", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["cyberfox", "-w", "--tui"], None),
        (["cyberfox", "version"], "version"),
        (["cyberfox", "--tui", "chat"], "chat"),
        (["cyberfox", "-w", "logs"], "logs"),
        (["cyberfox", "chat", "hello world"], "chat"),
        (["cyberfox", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["cyberfox", "-m", "gpt5", "chat"], "chat"),
        (["cyberfox", "--model", "gpt5", "chat", "hi"], "chat"),
        (["cyberfox", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["cyberfox", "-z", "hello world"], None),
        (["cyberfox", "-z", "hello", "chat"], "chat"),
        (["cyberfox", "--model=gpt5", "chat"], "chat"),     # inline form
        (["cyberfox", "--", "chat"], "chat"),               # -- terminator
        (["cyberfox", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["cyberfox", "some-plugin-cmd"], "some-plugin-cmd"),
        (["cyberfox", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["cyberfox"],                          # bare → chat
        ["cyberfox", "--help"],                # top-level help
        ["cyberfox", "-h"],
        ["cyberfox", "version"],               # known built-in
        ["cyberfox", "logs"],
        ["cyberfox", "gateway", "run"],
        ["cyberfox", "--tui"],
        ["cyberfox", "-w", "--tui"],
        ["cyberfox", "chat", "hi"],
        ["cyberfox", "help"],                  # accepted built-in-ish
        ["cyberfox", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["cyberfox", "meet", "join"],          # potential google_meet plugin
        ["cyberfox", "honcho", "status"],      # potential memory plugin
        ["cyberfox", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``cyberfox help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"cyberfox_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
