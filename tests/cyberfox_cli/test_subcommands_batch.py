...
def test_single_handler_builders(name, builder, kw, argv):
    parser = argparse.ArgumentParser(prog="cyberfox")
...
# ── deprecated `cyberfox login` fails gracefully, not with argparse error ────
#
# `cyberfox login` is a removed command; its handler (`login_command` in
# `cyberfox_cli/auth.py`) prints a deprecation notice pointing at `cyberfox auth` /
# `cyberfox model` and exits 0.  Two behavior contracts guard the UX:
...
@pytest.mark.parametrize("provider", ["anthropic", "cyberfox", "openai-codex", "totally-made-up"])
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `cyberfox --help`.
