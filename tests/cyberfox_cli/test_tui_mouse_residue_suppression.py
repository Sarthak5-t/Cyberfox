...
The Python launcher (`cyberfox --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Cyberfox session
...
from cyberfox_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["cyberfox", "--tui", "-c", "abc"])
        monkeypatch.delenv("CYBERFOX_TUI", raising=False)
        monkeypatch.delenv("CYBERFOX_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_writes_disable_sequence_when_cyberfox_tui_env_set(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["cyberfox"])
        monkeypatch.setenv("CYBERFOX_TUI", "1")
        monkeypatch.delenv("CYBERFOX_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.delenv("CYBERFOX_TUI", raising=False)
        monkeypatch.setenv("CYBERFOX_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `cyberfox --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
