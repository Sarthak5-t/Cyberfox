"""Tests for cyberfox_cli.auth._update_config_for_provider clearing stale fields.
...
from cyberfox_cli.config import clear_model_endpoint_credentials, get_config_path
...
class TestUpdateConfigForProviderClearsStaleCustomFields:
    ...
    def test_switching_to_cyberfox_clears_stale_api_mode(self):
        _seed_custom_provider_config()
        _update_config_for_provider("cyberfox", "https://api.openai.com/v1")
        model_cfg = _read_model_cfg()
        assert model_cfg.get("provider") == "cyberfox"
        assert "api_mode" not in model_cfg
        assert "api_key" not in model_cfg
