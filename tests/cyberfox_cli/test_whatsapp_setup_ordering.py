...
Before the fix, ``cyberfox whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``cyberfox gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    cyberfox = home / ".cyberfox"
    cyberfox.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox))
...
def _env_value(cyberfox_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from cyberfox_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("cyberfox_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``cyberfox whatsapp`` with an existing paired session and
