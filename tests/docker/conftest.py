...
Override the image with ``CYBERFOX_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("CYBERFOX_TEST_IMAGE", "cyberfox-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``CYBERFOX_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("CYBERFOX_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged cyberfox user
# ---------------------------------------------------------------------------
#
# Background: every Cyberfox runtime path inside the container drops to UID
# 10000 (the ``cyberfox`` user) via ``s6-setuidgid cyberfox``. ``docker exec``
...
# fails (PermissionError swallowed) for cyberfox, so a test that ran as root
...
# helpers below run every probe under ``-u cyberfox`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: cyberfox).
