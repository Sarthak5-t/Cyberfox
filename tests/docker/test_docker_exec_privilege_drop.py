...
The shim (docker/cyberfox-exec-shim.sh, installed at /opt/cyberfox/bin/cyberfox)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> cyberfox login` would write /opt/data/auth.json as
...
failed: Cyberfox is not logged into cyberfox portal" on every message.

These tests verify:

1. ``docker exec <c> cyberfox …`` (defaulting to root) gets dropped to the
   cyberfox user before the real binary runs.
2. ``docker exec --user cyberfox <c> cyberfox …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $CYBERFOX_HOME from a ``docker exec`` session land
   as cyberfox:cyberfox — the actual user-visible invariant.
4. The CYBERFOX_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-cyberfox-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown cyberfox:cyberfox`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to cyberfox:cyberfox,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$CYBERFOX_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (cyberfox_cli.container_boot), which s6 runs
    *strictly after* ``01-cyberfox-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_cyberfox_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `cyberfox` with a Python-style `-c` shim equivalent — there's no
    pure-cyberfox "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `cyberfox` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `cyberfox`
    do anything that writes to disk, then check the file's owner.

    Use `cyberfox config set` which writes config.yaml under CYBERFOX_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "cyberfox", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by cyberfox, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "cyberfox:cyberfox", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected cyberfox:cyberfox. "
        "The shim did not drop privileges before invoking cyberfox."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user cyberfox already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "cyberfox", sleep_container,
         "cyberfox", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user cyberfox failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up cyberfox:cyberfox — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """CYBERFOX_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", "CYBERFOX_DOCKER_EXEC_AS_ROOT=1",
         sleep_container,
         "cyberfox", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "root:root", (
        f"With CYBERFOX_DOCKER_EXEC_AS_ROOT=1, expected root:root, "
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``CYBERFOX_DOCKER_EXEC_AS_ROOT=0``) doesn't
    silently keep the user as root. Mirrors the policy used by
    ``CYBERFOX_GATEWAY_NO_SUPERVISE`` in #33583.
    """
    subprocess.run(
        ["docker", "exec", "--user", "root", sleep_container,
         "rm", "-f", "/opt/data/config.yaml"],
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", f"CYBERFOX_DOCKER_EXEC_AS_ROOT={falsy_value}",
         sleep_container,
         "cyberfox", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "cyberfox:cyberfox", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected cyberfox:cyberfox"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/cyberfox/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid cyberfox cyberfox <args>` which resolves `cyberfox`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> cyberfox login`` (root) wrote
    ...
    as a parse failure, and the runtime credential resolver raised
    "Cyberfox is not logged into cyberfox portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `cyberfox config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is cyberfox-readable.

    Specifically: pretend the operator ran `cyberfox login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the cyberfox UID. We use `cyberfox auth list` since that touches the
    ...
    # `cyberfox auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `cyberfox config set` first to
    # provoke a write into CYBERFOX_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "cyberfox", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # CYBERFOX_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "cyberfox", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the cyberfox user "
