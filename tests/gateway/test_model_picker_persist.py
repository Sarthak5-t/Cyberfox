...
closure the PR changed, against a real temp ``CYBERFOX_HOME``.
"""
...
def _fake_switch_result():
    ...
    from cyberfox_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "cyberfox_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_cyberfox_home() / config.yaml`` — point it here.
    monkeypatch.setattr("cyberfox_constants.get_cyberfox_home", lambda: cyberfox_home)
    monkeypatch.setattr("cyberfox_cli.config.get_cyberfox_home", lambda: cyberfox_home)
    return cfg_path
