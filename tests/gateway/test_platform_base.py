...
class TestSecretCaptureGuidance:
    def test_gateway_secret_capture_message_points_to_local_setup(self):
        ...
        assert "~/.cyberfox/.env" in message
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.cyberfox/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("CYBERFOX_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("CYBERFOX_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("CYBERFOX_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CYBERFOX_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_recency_trust_allows_freshly_produced_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("CYBERFOX_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("CYBERFOX_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("CYBERFOX_MEDIA_TRUST_RECENT_SECONDS", "600")
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("CYBERFOX_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("CYBERFOX_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("CYBERFOX_MEDIA_TRUST_RECENT_SECONDS", "60")
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.cyberfox/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_cyberfox_credentials(self, tmp_path, monkeypatch):
        """~/.cyberfox/.env and ~/.cyberfox/auth.json stay blocked even in
        ...
        fake_home = tmp_path / "home"
        cyberfox_dir = fake_home / ".cyberfox"
        cyberfox_dir.mkdir(parents=True)
        env_file = cyberfox_dir / ".env"
        env_file.write_text("OPENAI_API_KEY=sk-...")
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._CYBERFOX_HOME",
            cyberfox_dir,
        )
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.cyberfox/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        cyberfox_dir = fake_home / ".cyberfox"
        (cyberfox_dir / "mcp-tokens").mkdir(parents=True)
        secret = cyberfox_dir / rel
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._CYBERFOX_ROOT",
            cyberfox_dir,
        )

        assert BasePlatformAdapter.validate_media_delivery_path(str(secret)) is None

    def test_denylist_blocks_cyberfox_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        cyberfox_dir = fake_home / ".cyberfox"
        cyberfox_dir.mkdir(parents=True)
        config_file = cyberfox_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_cyberfox_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Cyberfox root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".cyberfox" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        cyberfox_root = fake_home / ".cyberfox"
        config_file = cyberfox_root / "config.yaml"
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the CYBERFOX_HOME root (google_token.json)
        ...
        cyberfox_dir = fake_home / ".cyberfox"
        cyberfox_dir.mkdir(parents=True)
        token = cyberfox_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._CYBERFOX_HOME", cyberfox_dir)
        monkeypatch.setattr("gateway.platforms.base._CYBERFOX_ROOT", cyberfox_dir)
    ...
    def test_denylist_blocks_pairing_directory_contents(self, tmp_path, monkeypatch):
        """Files under ~/.cyberfox/pairing/ (platform pairing tokens) are
    ...
    def test_cyberfox_cache_still_delivers_under_denied_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "home"
        cyberfox_dir = fake_home / ".cyberfox"
        cache_dir = cyberfox_dir / "cache" / "documents"
    ...
    def test_denylist_blocks_non_cache_file_under_cyberfox_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.cyberfox
        ...
        cyberfox_dir = fake_home / ".cyberfox"
        cyberfox_dir.mkdir(parents=True)
        artifact = cyberfox_dir / "adhoc_report.pdf"
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting CYBERFOX_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``CYBERFOX_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("CYBERFOX_MEDIA_DELIVERY_STRICT", raw)
    ...
    def test_root_home_cyberfox_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.cyberfox/.env`` stays blocked under the $HOME exception — it is a
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose CYBERFOX_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.cyberfox) emits
        ...
        cyberfox_root = denied_root / ".cyberfox"
        prof_cache = cyberfox_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._CYBERFOX_ROOT", cyberfox_root
        )
    ...
    def test_profile_scoped_credential_still_blocked_under_root(self, tmp_path, monkeypatch):
        ...
        denied_root = tmp_path / "root"
        cyberfox_root = denied_root / ".cyberfox"
        prof_dir = cyberfox_root / "profiles" / "myprof"
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"CYBERFOX_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("CYBERFOX_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"CYBERFOX_HUMAN_DELAY_MODE": "natural"}):
            delay = BasePlatformAdapter._get_human_delay()
            assert 0.8 <= delay <= 2.5

    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "CYBERFOX_HUMAN_DELAY_MODE": "natural",
            "CYBERFOX_HUMAN_DELAY_MIN_MS": "oops",
            "CYBERFOX_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            "CYBERFOX_HUMAN_DELAY_MODE": "custom",
            "CYBERFOX_HUMAN_DELAY_MIN_MS": "100",
            "CYBERFOX_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"CYBERFOX_MEDIA_DELIVERY_STRICT": "1",
                                     "CYBERFOX_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CYBERFOX_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Cyberfox
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../cyberfox/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.cyberfox/cache/media/sensitive_host_path_abc123.bin"
