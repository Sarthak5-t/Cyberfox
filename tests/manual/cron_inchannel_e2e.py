...
Uses a throwaway CYBERFOX_HOME so it never touches ~/.cyberfox. No real names.
"""
...
def _fresh_home():
    """Point CYBERFOX_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_cyberfox_home() at import time)."""
...
# Import AFTER CYBERFOX_HOME is set.
...
# a different get_cyberfox_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"CYBERFOX_HOME (throwaway): {HOME}")
