...
Run from the PR worktree:
    ...
    cd ~/.cyberfox/cyberfox-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.cyberfox/cyberfox-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.cyberfox/cyberfox-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a cyberfox-agent checkout"
)
...
# Isolated CYBERFOX_HOME for the config write.
