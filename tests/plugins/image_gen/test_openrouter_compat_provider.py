#!/usr/bin/env python3
"""Tests for the OpenRouter-compatible image gen provider (OpenRouter)."""
...
_RUNTIME = "cyberfox_cli.runtime_provider.resolve_runtime_provider"
...
class TestProviderClass:
    ...
    def test_legacy_honors_top_level_model(self):
        ...
        cfg = {"model": "openai/gpt-image-2"}
        provider = {p.name: p for p in _build_providers()}["openrouter"]
        with patch("plugins.image_gen.openrouter._load_image_gen_config", return_value=cfg):
            assert provider._resolve_model_chain() == ["openai/gpt-image-2"]
