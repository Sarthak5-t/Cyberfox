...
def test_moa_slots_routed_through_resolve_runtime_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "cyberfox_cli.runtime_provider.resolve_runtime_provider", fake_resolve
    )
...
def test_moa_provider_backed_slot_survives_aux_resolution(monkeypatch, provider):
    ...
    bedrock, cyberfox, openai-codex, xai-oauth) are intentionally NOT forwarded —
...
def test_moa_slot_runtime_falls_back_on_resolution_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "cyberfox_cli.runtime_provider.resolve_runtime_provider", boom
    )
...
def test_reference_messages_drops_system_but_renders_tools_as_text():
    ...
    messages = [
        {"role": "system", "content": "huge cyberfox system prompt"},
    ...
    assert all("huge cyberfox system prompt" not in m["content"] for m in view)
