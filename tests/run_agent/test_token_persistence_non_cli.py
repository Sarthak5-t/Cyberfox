...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    cyberfox_state = ModuleType("cyberfox_state")
    cyberfox_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "cyberfox_state", cyberfox_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Cyberfox"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Cyberfox"
