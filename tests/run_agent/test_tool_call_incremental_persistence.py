...
def _make_agent():
    cyberfox_home = Path(tempfile.mkdtemp(prefix="cyberfox-test-home-"))
    (cyberfox_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._cyberfox_home", cyberfox_home),
