...
binary download are exercised manually by `cyberfox secrets bitwarden
...
def cyberfox_home(tmp_path, monkeypatch):
    """Point Cyberfox at an isolated home directory."""
    home = tmp_path / ".cyberfox"
    home.mkdir()
    monkeypatch.setenv("CYBERFOX_HOME", str(home))
    # Some modules cache get_cyberfox_home; clear if needed.
    import cyberfox_constants
    if hasattr(cyberfox_constants, "_CYBERFOX_HOME_CACHE"):
        cyberfox_constants._CYBERFOX_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(cyberfox_home, monkeypatch):
...
def test_install_bws_happy_path(cyberfox_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(cyberfox_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(cyberfox_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from cyberfox_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under cyberfox_home/cache/."""
    home = tmp_path / ".cyberfox"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
