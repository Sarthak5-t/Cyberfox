"""Tests for cyberfox_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Cyberfox entry point
(cyberfox, cyberfox-agent, cyberfox-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Cyberfox entry point imports cyberfox_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported cyberfox_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("cyberfox_bootstrap", None)
    import cyberfox_bootstrap  # noqa: WPS433
    return cyberfox_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Cyberfox CLI
...
class TestEntryPointsImportBootstrap:
    """Every Cyberfox entry point must import cyberfox_bootstrap as its
    ...
    # Entry points that invoke Cyberfox as a process.  Each one must
    # import cyberfox_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "cyberfox_cli/main.py",   # cyberfox CLI (console_script)
        "run_agent.py",          # cyberfox-agent (console_script)
        "acp_adapter/entry.py",  # cyberfox-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import cyberfox_bootstrap' and that
        ...
        half-finished ``cyberfox update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``cyberfox_bootstrap`` as a top-level module) leaves cyberfox
        ...
        """
        # Resolve relative to the cyberfox-agent repo root.  Tests live
        # at tests/test_cyberfox_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # cyberfox start even when cyberfox_bootstrap hasn't been
        ...
        assert first_import_name == "cyberfox_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'cyberfox_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import cyberfox_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Cyberfox's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("CYBERFOX_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["CYBERFOX_PYTHON_SRC_ROOT"] = env
            elif "CYBERFOX_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["CYBERFOX_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/cyberfox")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("CYBERFOX_PYTHON_SRC_ROOT", None)
            else:
                os.environ["CYBERFOX_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/cyberfox", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Cyberfox root.  The guard must relocate Cyberfox to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/cyberfox"])
        assert result[0] == "/opt/cyberfox"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Cyberfox root.
        assert result.index("/opt/cyberfox") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/cyberfox", "/opt/cyberfox", ""])
        assert result.count("/opt/cyberfox") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["CYBERFOX_PYTHON_SRC_ROOT"] = "/env/cyberfox"
            hb.harden_import_path()
            assert sys.path[0] == "/env/cyberfox"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("CYBERFOX_PYTHON_SRC_ROOT", None)
