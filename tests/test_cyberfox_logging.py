"""Tests for cyberfox_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import cyberfox_logging
# Use whatever RotatingFileHandler class cyberfox_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. cyberfox_logging
...
from cyberfox_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    cyberfox_logging._logging_initialized = False
    ...
    cyberfox_logging._reset_queued_handlers()
    ...
    cyberfox_logging._install_session_record_factory()
    yield
    ...
    cyberfox_logging._logging_initialized = False
    cyberfox_logging.clear_session_context()


@pytest.fixture
def cyberfox_home(tmp_path, monkeypatch):
    """Provide an isolated CYBERFOX_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_cyberfox_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["CYBERFOX_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, cyberfox_home):
        log_dir = cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)
        assert log_dir == cyberfox_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in cyberfox_logging.rotating_file_handlers()
    ...
    def test_creates_errors_log_handler(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home, force=True)
    ...
    def test_custom_log_level(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, cyberfox_home):
        cyberfox_logging.setup_logging(
            cyberfox_home=cyberfox_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)
    ...
    def test_writes_to_agent_log(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)

        test_logger = logging.getLogger("test_cyberfox_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        cyberfox_logging.flush_log_queue()

        agent_log = cyberfox_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)

        test_logger = logging.getLogger("test_cyberfox_logging.warning_test")
        test_logger.warning("this is a warning")

        cyberfox_logging.flush_log_queue()

        agent_log = cyberfox_home / "logs" / "agent.log"
        errors_log = cyberfox_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)

        test_logger = logging.getLogger("test_cyberfox_logging.info_test")
    ...
    def test_reads_config_yaml(self, cyberfox_home):
    ...
    def test_explicit_params_override_config(self, cyberfox_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (cyberfox_home / "config.yaml").write_text(yaml.dump(config))

        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, cyberfox_home):
        """The custom record factory injects session_tag on all records."""
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_cyberfox_session_injector", False), (
            "Record factory should have _cyberfox_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, cyberfox_home):
        ...
        gw_log = cyberfox_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, cyberfox_home):
    ...
    def test_gateway_log_receives_gateway_records(self, cyberfox_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, cyberfox_home):
    ...
    def test_agent_log_still_receives_all(self, cyberfox_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_created_after_cli_init(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home, mode="cli")
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home, mode="gui")
    ...
    def test_gui_log_receives_only_gui_components(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home, mode="gui")

        logging.getLogger("cyberfox_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        cyberfox_logging.flush_log_queue()

        gui_log = cyberfox_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, cyberfox_home):
        """When session context is set, log lines include [session_id]."""
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)
        cyberfox_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, cyberfox_home):
    ...
    def test_clear_session_context(self, cyberfox_home):
        """After clearing, session tag disappears."""
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)
        cyberfox_logging.set_session_context("xyz789")
        cyberfox_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, cyberfox_home):
        ...
        def thread_a():
            cyberfox_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            cyberfox_logging.flush_log_queue()

        def thread_b():
            cyberfox_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        cyberfox_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        cyberfox_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = cyberfox_logging._ComponentFilter(("gateway",))
    ...
    def test_passes_nested_matching_prefix(self):
        ...
        f = cyberfox_logging._ComponentFilter(
            cyberfox_logging.COMPONENT_PREFIXES["gateway"]
        )
        record = logging.LogRecord(
    ...
    def test_multiple_prefixes(self):
        f = cyberfox_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in cyberfox_logging.COMPONENT_PREFIXES
        # The gateway component captures core gateway logs, the cyberfox_plugins
        ...
        # break this test.
        gateway_prefixes = cyberfox_logging.COMPONENT_PREFIXES["gateway"]
        assert "gateway" in gateway_prefixes
        assert "cyberfox_plugins" in gateway_prefixes
        assert "plugins.platforms" in gateway_prefixes

    def test_agent_prefix(self):
        prefixes = cyberfox_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == cyberfox_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = cyberfox_logging.COMPONENT_PREFIXES["cli"]
        assert "cyberfox_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == cyberfox_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = cyberfox_logging.COMPONENT_PREFIXES["gui"]
        assert "cyberfox_cli.web_server" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)
        cyberfox_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_cyberfox_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)
        cyberfox_logging.setup_verbose_logging()
        cyberfox_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        cyberfox_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = cyberfox_logging._ComponentFilter(("test",))
        ...
        handlers = [h for h in cyberfox_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        cyberfox_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("cyberfox_cli.config.is_managed", return_value=True):
                cyberfox_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
    ...
    def test_managed_mode_rollover_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("cyberfox_cli.config.is_managed", return_value=True):
                ...
                handler = next(
                    h for h in cyberfox_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)
                )
                logger.info("a" * 256)
                cyberfox_logging.flush_log_queue()
        finally:
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = cyberfox_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        with patch.object(cyberfox_logging.sys, "platform", "win32"):
            assert cyberfox_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
            assert not cyberfox_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("some other logging failure")
            )

        with patch.object(cyberfox_logging.sys, "platform", "linux"):
            assert not cyberfox_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, cyberfox_home):
        level, max_size, backup = cyberfox_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, cyberfox_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, cyberfox_home):
        import yaml
        config = {"model": "test"}
        (cyberfox_home / "config.yaml").write_text(yaml.dump(config))
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> cyberfox_logging._ManagedRotatingFileHandler:
        handler = cyberfox_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that cyberfox_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        cyberfox_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home, mode="gateway")
        gw_path = cyberfox_home / "logs" / "gateway.log"
        rotated = cyberfox_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_returns_stderr_on_utf8_system(self, monkeypatch):
        ...
        result = cyberfox_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_cyberfox_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in cyberfox_logging.rotating_file_handlers()
        )

    def test_records_reach_file_through_queue(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)
    ...
    def test_queue_preserves_per_handler_levels(self, cyberfox_home):
        cyberfox_logging.setup_logging(cyberfox_home=cyberfox_home)
