...
Older versions of ``install.sh`` created ``$command_link_dir/cyberfox`` as a
symlink to the pip-generated entry point at ``$CYBERFOX_BIN`` (i.e.
``venv/bin/cyberfox``). When ``setup_path()`` later switched to writing a bash
shim with ``cat > "$command_link_dir/cyberfox" <<EOF``, the redirect followed
the existing symlink and overwrote the pip entry point with the shim. The
shim's ``exec "$CYBERFOX_BIN" "$@"`` then self-recursed and ``cyberfox`` hung on
every invocation.

These tests pin the fix: ``setup_path()`` must remove ``$command_link_dir/cyberfox``
...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/cyberfox\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/cyberfox"')
    cat_idx = block.find('cat > "$command_link_dir/cyberfox" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/cyberfox before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    Layout mirrors a real install:
        ...
        tmp/
          venv/bin/cyberfox        <- pip entry point (the one we must preserve)
          local_bin/cyberfox       <- symlink → ../venv/bin/cyberfox  (old install)

    Then we run the exact shim-write block from setup_path() with
    ``CYBERFOX_BIN`` and ``command_link_dir`` pointed at this fixture. The fix
    requires that, after the run:

      * ``venv/bin/cyberfox`` still contains its original pip-script body
      * ``local_bin/cyberfox`` is a regular file (not a symlink) holding the shim
    """
    venv_bin = tmp_path / "venv" / "bin"
    venv_bin.mkdir(parents=True)
    pip_entry = venv_bin / "cyberfox"
    ...
    shim_path = command_link_dir / "cyberfox"
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/cyberfox was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/cyberfox must be replaced with a regular file, not "
