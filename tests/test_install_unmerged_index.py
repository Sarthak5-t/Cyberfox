...
The ``cyberfox update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m cyberfox_cli.main gateway run`` — image name
    ``pythonw``, not ``cyberfox.exe`` — so the ``taskkill /IM cyberfox.exe`` guard
    ...
    # The cyberfox.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM cyberfox.exe' in text
