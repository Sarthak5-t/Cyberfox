"""
Tests for mcp_serve — Cyberfox MCP server.
...
def _isolate_cyberfox_home(tmp_path, monkeypatch):
    """Redirect CYBERFOX_HOME to a temp directory."""
    monkeypatch.setenv("CYBERFOX_HOME", str(tmp_path))
    try:
        import cyberfox_constants
        monkeypatch.setattr(cyberfox_constants, "get_cyberfox_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking cyberfox_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <CYBERFOX_HOME>/state.db for its mtime gate; the autouse
        # fixture points CYBERFOX_HOME at tmp_path.
        db_path = tmp_path / "state.db"
