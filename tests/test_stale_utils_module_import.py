...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.cyberfox/cyberfox-agent/utils.py)
...
2. ``cyberfox update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``cyberfox_cli/main.py`` (the ``cyberfox update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``cyberfox_constants`` after the
