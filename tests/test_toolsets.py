...
class TestToolsetConsistency:
    ...
    def test_cyberfox_platforms_share_core_tools(self):
        """All cyberfox-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on cyberfox-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["cyberfox-cli", "cyberfox-telegram", "cyberfox-discord", "cyberfox-whatsapp", "cyberfox-slack", "cyberfox-signal", "cyberfox-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_cyberfox_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("cyberfox-whatsapp")

    def test_cyberfox_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("cyberfox-api-server")
