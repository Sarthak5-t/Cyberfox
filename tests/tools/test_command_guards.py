...
def _clean_state():
    ...
    saved = {}
    for k in ("CYBERFOX_INTERACTIVE", "CYBERFOX_GATEWAY_SESSION", "CYBERFOX_EXEC_ASK", "CYBERFOX_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["CYBERFOX_INTERACTIVE"] = "1"
        session_key = os.getenv("CYBERFOX_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No CYBERFOX_INTERACTIVE or CYBERFOX_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
