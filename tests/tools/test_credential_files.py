...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        cyberfox_home.mkdir()
        (cyberfox_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(cyberfox_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.cyberfox/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        cyberfox_home = tmp_path / ".cyberfox"
        cyberfox_home.mkdir()
        (cyberfox_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        cyberfox_home = tmp_path / ".cyberfox"
        cyberfox_home.mkdir()
        (cyberfox_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        cyberfox_home = tmp_path / ".cyberfox"
        cyberfox_home.mkdir()
        (cyberfox_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        cyberfox_home = tmp_path / ".cyberfox"
        skills_dir = cyberfox_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.cyberfox/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        cyberfox_home = tmp_path / ".cyberfox"
        (cyberfox_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"CYBERFOX_HOME": str(cyberfox_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.cyberfox")

        assert mounts[0]["container_path"] == "/home/user/.cyberfox/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.cyberfox/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.cyberfox/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape CYBERFOX_HOME."""
        monkeypatch.setenv("CYBERFOX_HOME", str(tmp_path / ".cyberfox"))
        (tmp_path / ".cyberfox").mkdir()

        # Create a sensitive file one level above cyberfox_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside cyberfox_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside CYBERFOX_HOME must still be registered."""
        cyberfox_home = tmp_path / ".cyberfox"
        cyberfox_home.mkdir()
        monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
        (cyberfox_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_cyberfox_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of CYBERFOX_HOME must be allowed."""
        cyberfox_home = tmp_path / ".cyberfox"
        cyberfox_home.mkdir()
        subdir = cyberfox_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside CYBERFOX_HOME pointing outside must be rejected."""
        cyberfox_home = tmp_path / ".cyberfox"
        cyberfox_home.mkdir()
        monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))

        # Create a sensitive file outside cyberfox_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside cyberfox_home pointing outside
        symlink = cyberfox_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes CYBERFOX_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, cyberfox_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape CYBERFOX_HOME."""
        cyberfox_home = tmp_path / ".cyberfox"
        cyberfox_home.mkdir()
        monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
        ...
        sensitive.write_text("{}")
        self._write_config(cyberfox_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(cyberfox_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside CYBERFOX_HOME via config must still mount."""
        cyberfox_home = tmp_path / ".cyberfox"
        cyberfox_home.mkdir()
        monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))

        (cyberfox_home / "oauth.json").write_text("{}")
        self._write_config(cyberfox_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        cyberfox_home = tmp_path / ".cyberfox"
        cyberfox_home.mkdir()
        (cyberfox_home / "cache" / "documents").mkdir(parents=True)
        (cyberfox_home / "cache" / "audio").mkdir(parents=True)
        (cyberfox_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.cyberfox/cache/documents" in paths
        assert "/root/.cyberfox/cache/audio" in paths
        assert "/root/.cyberfox/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.cyberfox/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_cyberfox_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        cyberfox_home = tmp_path / ".cyberfox"
        cyberfox_home.mkdir()
        # Use legacy dir name with content — get_cyberfox_dir prefers
        # populated old over new.
        legacy_doc = cyberfox_home / "document_cache"
        legacy_img = cyberfox_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(cyberfox_home / "document_cache") in host_paths
        assert str(cyberfox_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.cyberfox/cache/documents" in container_paths
        assert "/root/.cyberfox/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        cyberfox_home = tmp_path / ".cyberfox"
        img_dir = cyberfox_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.cyberfox/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.cyberfox")
            == "/home/agent/.cyberfox/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        cyberfox_home = tmp_path / ".cyberfox"
        (cyberfox_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("CYBERFOX_HOME", str(cyberfox_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(cyberfox_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        cyberfox_home = tmp_path / ".cyberfox"
        doc_dir = cyberfox_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        cyberfox_home = tmp_path / ".cyberfox"
        ss_dir = cyberfox_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.cyberfox/cache/screenshots/session_abc/screen1.png"
