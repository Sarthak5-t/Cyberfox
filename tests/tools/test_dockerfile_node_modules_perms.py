...
Older images made /opt/cyberfox/ui-tui and /opt/cyberfox/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV CYBERFOX_TUI_DIR=/opt/cyberfox/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R cyberfox:cyberfox /opt/cyberfox/ui-tui" not in text
    assert "chown -R cyberfox:cyberfox /opt/cyberfox/node_modules" not in text
