...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad CYBERFOX_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: CYBERFOX_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("CYBERFOX_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("CYBERFOX_CRON_SESSION", "1")
    monkeypatch.delenv("CYBERFOX_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_cyberfox_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "CYBERFOX_HOME": "/h", "CYBERFOX_PROFILE": "p",
        "CYBERFOX_CONFIG": "/c.yaml", "CYBERFOX_ENV": "/e",
        # other CYBERFOX_* → dropped (broad prefix removed)
        "CYBERFOX_BASE_URL": "https://x", "CYBERFOX_INTERACTIVE": "1",
        "CYBERFOX_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("CYBERFOX_HOME", "CYBERFOX_PROFILE", "CYBERFOX_CONFIG", "CYBERFOX_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "CYBERFOX_BASE_URL", "CYBERFOX_INTERACTIVE", "CYBERFOX_KANBAN_DB",
...
def test_env_scrub_logs_dropped_cyberfox_vars(caplog):
    """Dropping a non-allowlisted, non-secret CYBERFOX_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset CYBERFOX_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "CYBERFOX_HOME": "/h",          # allowlisted → kept, not logged
        "CYBERFOX_BASE_URL": "https://x",   # dropped → logged
        "CYBERFOX_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "CYBERFOX_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "CYBERFOX_HOME" in out and "PATH" in out
    assert "CYBERFOX_BASE_URL" not in out and "CYBERFOX_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "CYBERFOX_BASE_URL" in msgs and "CYBERFOX_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "CYBERFOX_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped CYBERFOX_* vars."""
    import logging
