...
def test_cyberfox_provider_subclass_exists():
    """CyberfoxMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _CYBERFOX_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _CYBERFOX_PROVIDER_CLS is not None
    assert issubclass(_CYBERFOX_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_cyberfox_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns CyberfoxMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _CYBERFOX_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("CYBERFOX_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _CYBERFOX_PROVIDER_CLS is not None
    assert isinstance(provider, _CYBERFOX_PROVIDER_CLS)
    assert provider._cyberfox_server_name == "srv"
