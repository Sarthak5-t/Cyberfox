...
def _restore_tool_modules():
    original_cyberfox_home = os.environ.get("CYBERFOX_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "cyberfox_cli"
        or name.startswith("cyberfox_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_cyberfox_home is None:
            os.environ.pop("CYBERFOX_HOME", None)
        else:
            os.environ["CYBERFOX_HOME"] = original_cyberfox_home
        _reset_modules(("tools", "cyberfox_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    cyberfox_cli = types.ModuleType("cyberfox_cli")
    cyberfox_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["cyberfox_cli"] = cyberfox_cli
    cyberfox_home = tmp_path / "cyberfox-home"
    os.environ["CYBERFOX_HOME"] = str(cyberfox_home)
    sys.modules["cyberfox_cli.config"] = types.SimpleNamespace(
        get_cyberfox_home=lambda: cyberfox_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="cyberfox-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": cyberfox_home / "modal_snapshots.json",
