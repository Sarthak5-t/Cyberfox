...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._cyberfox_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(cyberfox_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(cyberfox_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_cyberfox_bin(self):
        """After _INSTALL_FAILED, manual install in CYBERFOX_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        cyberfox_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(cyberfox_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(cyberfox_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._cyberfox_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == cyberfox_bin
            assert _tirith_mod._resolved_path == cyberfox_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# CYBERFOX_HOME isolation
# ---------------------------------------------------------------------------

class TestCyberfoxHomeIsolation:
    def test_cyberfox_bin_dir_respects_cyberfox_home(self):
        """_cyberfox_bin_dir must use CYBERFOX_HOME, not hardcoded ~/.cyberfox."""
        from tools.tirith_security import _cyberfox_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"CYBERFOX_HOME": tmpdir}):
            result = _cyberfox_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_cyberfox_home(self):
        """_failure_marker_path must use CYBERFOX_HOME, not hardcoded ~/.cyberfox."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"CYBERFOX_HOME": "/custom/cyberfox"}):
            result = _failure_marker_path()
        assert result == "/custom/cyberfox/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets CYBERFOX_HOME; verify it's active."""
        cyberfox_home = os.getenv("CYBERFOX_HOME")
        assert cyberfox_home is not None, "CYBERFOX_HOME should be set by conftest"
        assert "cyberfox_test" in cyberfox_home, "Should point to test temp dir"

    def test_get_cyberfox_home_fallback(self):
        """Without CYBERFOX_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_cyberfox_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove CYBERFOX_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".cyberfox")
            result = _get_cyberfox_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._cyberfox_bin_dir",
