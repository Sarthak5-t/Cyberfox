...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_cyberfox_home_override(self, tmp_path):
        """Auth lookup should read from CYBERFOX_HOME/auth.json, not ~/.cyberfox/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".cyberfox").mkdir(parents=True)
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "CYBERFOX_HOME": str(cyberfox_home),
        }, clear=False):
            import tools.web_tools
...
class TestBackendSelection:
    ...
    ``cyberfox tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "cyberfox_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from cyberfox_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.cyberfox/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.cyberfox/.env), not bare os.getenv. Parametrized over the four
