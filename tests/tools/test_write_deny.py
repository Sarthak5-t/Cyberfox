...
class TestWriteDenyExactPaths:
    ...
    def test_cyberfox_env(self):
        # ``.env`` under the active CYBERFOX_HOME (profile-aware, not just
        # ``~/.cyberfox``) must be write-denied. The hermetic test conftest
        # points CYBERFOX_HOME at a tempdir — resolve via get_cyberfox_home()
        # to match the denylist.
        from cyberfox_constants import get_cyberfox_home
        path = str(get_cyberfox_home() / ".env")
        assert _is_write_denied(path) is True

    def test_cyberfox_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.cyberfox/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "cyberfox_root"
        ...
        # Sanity check: CYBERFOX_HOME does point to the profile dir, not the root.
        from cyberfox_constants import get_cyberfox_home, get_default_cyberfox_root
        assert get_cyberfox_home() == profile_home
        assert get_default_cyberfox_root() == root
...
class TestWriteAllowed:
    ...
    def test_cyberfox_control_files_requested_writable(self):
        from cyberfox_constants import get_cyberfox_home
