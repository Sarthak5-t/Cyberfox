...
def server():
    with patch.dict(
        "sys.modules",
        {
            "cyberfox_constants": MagicMock(
                get_cyberfox_home=MagicMock(return_value="/tmp/cyberfox_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'cyberfox-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'cyberfox-release' patched"
    }
