...
`cyberfox skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.cyberfox.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.cyberfox.blueprint`` is a mapping containing
    ...
    cyberfox = meta.get("cyberfox") if isinstance(meta, dict) else None
    blueprint = cyberfox.get("blueprint") if isinstance(cyberfox, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.cyberfox.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.cyberfox.blueprint`` block) they can hand
    to ``cyberfox skills publish`` to share. ``body`` is the plain-language
