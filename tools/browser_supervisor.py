...
One ``CDPSupervisor`` runs per Cyberfox ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Cyberfox logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "cyberfox-dialog-bridge.invalid"
...
(() => {
  if (window.__cyberfoxDialogBridgeInstalled) return;
  window.__cyberfoxDialogBridgeInstalled = true;
  const ENDPOINT = "http://cyberfox-dialog-bridge.invalid/";
