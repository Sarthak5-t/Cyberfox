#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Cyberfox desktop GUI.
...
``read_terminal`` it is gated on ``CYBERFOX_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    """Desktop GUI only — CYBERFOX_DESKTOP is set on the gateway the app spawns."""
    return env_var_enabled("CYBERFOX_DESKTOP")
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Cyberfox desktop GUI (the tabs mirroring terminal(background=true) runs). "
