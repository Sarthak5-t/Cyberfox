...
* Never raises. Any I/O error (disk full, permission denied, missing
  CYBERFOX_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_cyberfox_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: CYBERFOX_HOME env var, then ~/.cyberfox
            home = os.environ.get("CYBERFOX_HOME") or os.path.expanduser("~/.cyberfox")
