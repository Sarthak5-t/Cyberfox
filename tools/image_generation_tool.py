...
selectable via ``cyberfox tools`` → Image Generation; the active model is
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Quality is pinned to medium to keep billing predictable across
            # all users (low is too rough, high is 3-4x more expensive).
            "quality": "medium",
            "num_images": 1,
            "output_format": "png",
        },
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.cyberfox"
    ...
    # Cyberfox cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `cyberfox tools` → Image Generation."
            )
...
def _build_no_backend_setup_message() -> str:
    ...
    lines.append(
        "  2. Configure a different image_gen provider via `cyberfox tools` "
        "→ Image Generation (run `cyberfox plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping cyberfox with a FAL key configured
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `cyberfox plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `cyberfox tools` → Image Generation."
                ),
...
def _handle_image_generate(args, **kw):
    ...
    # not the in-tree FAL path). ``image_gen.provider == "krea"`` dispatches
    # through the Krea plugin as normal.
...
# model/provider via `cyberfox tools` or `/skills`.
