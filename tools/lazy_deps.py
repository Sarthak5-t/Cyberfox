"""
Lazy dependency installer for opt-in Cyberfox Agent backends.

Many Cyberfox features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``cyberfox-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``cyberfox tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``CYBERFOX_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/cyberfox`` read-only), setting
  ``CYBERFOX_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Cyberfox, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (CYBERFOX_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
...
_LAZY_TARGET_ENV = "CYBERFOX_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``CYBERFOX_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("CYBERFOX_DISABLE_LAZY_INSTALLS") == "1":
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``cyberfox update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="cyberfox-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``cyberfox_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = cyberfox_subprocess_env(inherit_credentials=False)
...
def active_features() -> list[str]:
    ...
    Used by ``cyberfox update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``cyberfox update``. Never raises; lazy-install failures
