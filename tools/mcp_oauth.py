...
This module provides the glue:
    - ``CyberfoxTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Cyberfox Agent"
"""
...
from cyberfox_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__cyberfox_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses CYBERFOX_HOME so each profile gets its own OAuth tokens.
    Layout: ``CYBERFOX_HOME/mcp-tokens/``
    """
    try:
        from cyberfox_constants import get_cyberfox_home
        base = Path(get_cyberfox_home())
    except ImportError:
        base = Path(os.environ.get("CYBERFOX_HOME", str(Path.home() / ".cyberfox")))
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``cyberfox mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `cyberfox mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# CyberfoxTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class CyberfoxTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        CYBERFOX_HOME/mcp-tokens/<server_name>.json         -- tokens
        CYBERFOX_HOME/mcp-tokens/<server_name>.client.json   -- client info
        CYBERFOX_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Cyberfox records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Cyberfox.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://cyberfox-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `cyberfox mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Cyberfox Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`cyberfox mcp login {server_name}` interactively first to complete "
