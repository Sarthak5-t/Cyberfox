...
`mcp_tool.py`, and `cyberfox_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# CyberfoxMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_cyberfox_provider_class() -> Optional[type]:
    ...
    class CyberfoxMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._cyberfox_server_name = server_name
            ...
            self._cyberfox_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`CyberfoxTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, CyberfoxTokenStorage)
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._cyberfox_server_name, exc,
                    )
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._cyberfox_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import CyberfoxTokenStorage
                        if isinstance(storage, CyberfoxTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._cyberfox_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import CyberfoxTokenStorage
            if not isinstance(storage, CyberfoxTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``cyberfox mcp reauth``.
            ...
            back to ``cyberfox mcp reauth``.
            """
            try:
                if self._cyberfox_preregistered:
                    return
    ...
    return CyberfoxMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_CYBERFOX_PROVIDER_CLS: Optional[type] = _make_cyberfox_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`CyberfoxMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _CYBERFOX_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = CyberfoxTokenStorage(server_name)

        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                "MCP OAuth for "
                f"'{server_name}': non-interactive environment and no "
                "cached tokens found. Run `cyberfox mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``cyberfox mcp remove <name>`` and (indirectly) by
        ``cyberfox mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
