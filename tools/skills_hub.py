#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Cyberfox Skills Hub.
...
Used by cyberfox_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _cyberfox_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            cyberfox_meta = metadata.get("cyberfox", {})
            if isinstance(cyberfox_meta, dict):
                tags = cyberfox_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            cyberfox_meta = metadata.get("cyberfox", {})
            if isinstance(cyberfox_meta, dict):
                raw_tags = cyberfox_meta.get("tags", [])
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                cyberfox_meta = meta_block.get("cyberfox", {})
                if isinstance(cyberfox_meta, dict):
                    tags = cyberfox_meta.get("tags", [])
...
def _cyberfox_index_cache_file() -> Path:
    return _index_cache_dir() / "cyberfox-index.json"


def _load_cyberfox_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for CYBERFOX_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    cyberfox_index_cache_file = _cyberfox_index_cache_file()
    if cyberfox_index_cache_file.exists():
        try:
            age = time.time() - cyberfox_index_cache_file.stat().st_mtime
            if age < CYBERFOX_INDEX_TTL:
                return json.loads(cyberfox_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Cyberfox index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Cyberfox index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Cyberfox index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        cyberfox_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        cyberfox_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class CyberfoxIndexSource(SkillSource):
    """Skill source backed by the centralized Cyberfox Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_cyberfox_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "cyberfox-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "cyberfox-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        CyberfoxIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "cyberfox-index"
