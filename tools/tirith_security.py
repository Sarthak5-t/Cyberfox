...
it is automatically downloaded from GitHub releases to $CYBERFOX_HOME/bin/tirith.
...
def _get_cyberfox_home() -> str:
    """Return the Cyberfox home directory, respecting CYBERFOX_HOME env var."""
    return str(get_cyberfox_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_cyberfox_home(), ".tirith-install-failed")
...
def _cyberfox_bin_dir() -> str:
    """Return $CYBERFOX_HOME/bin, creating it if needed."""
    d = os.path.join(_get_cyberfox_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $CYBERFOX_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_cyberfox_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $CYBERFOX_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $CYBERFOX_HOME/bin/tirith
    ...
    cyberfox_bin = os.path.join(_cyberfox_bin_dir(), "tirith")
    if os.path.isfile(cyberfox_bin) and os.access(cyberfox_bin, os.X_OK):
        _resolved_path = cyberfox_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return cyberfox_bin
