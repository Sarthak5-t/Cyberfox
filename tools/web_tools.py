...
Backend is selected during ``cyberfox tools`` setup (web.backend in config.yaml).
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or via FIRECRAWL_API_URL)
...
from tools.tool_backend_helpers import prefers_gateway  # noqa: F401 — backward-compat name
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Cyberfox config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Cyberfox' config/.env layer (``cyberfox config set``, ``cyberfox tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.cyberfox/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``cyberfox tools``).
    ...
    # take precedence so a deliberate setup is honored. Free-tier backends
    # trail the paid ones.
    backend_candidates = (
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `cyberfox tools` repaint.
        try:
...
def _store_full_text(url: str, content: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_cyberfox_dir("cache/web", "web_cache")
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `cyberfox plugins enable {disabled_key}` "
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `cyberfox tools` to set one up."
                    ),
                }
        else:
            logger.info(
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`cyberfox plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    from cyberfox_cli.config import get_env_value as _gev
