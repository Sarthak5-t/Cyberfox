...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``cyberfox tools`` repaint, tool-registration scans,
    ...
    2. ``~/.cyberfox/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_cyberfox_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.cyberfox/.env`` first, then ``os.environ``.

    Wraps :func:`cyberfox_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from cyberfox_cli.config import get_env_value as _cyberfox_get_env_value

        value = _cyberfox_get_env_value(name)
...
def cyberfox_xai_user_agent() -> str:
    """Return a stable Cyberfox-specific User-Agent for xAI HTTP calls."""
    try:
        from cyberfox_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Cyberfox-Agent/{__version__}"


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Cyberfox config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Cyberfox home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_cyberfox_home() / "state"
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    Prefers Cyberfox-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``cyberfox_cli.config.get_env_value`` so keys
    stored in ``~/.cyberfox/.env`` (the standard Cyberfox location) are honored —
