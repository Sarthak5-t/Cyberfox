...
_CYBERFOX_CORE_TOOLS = [
    # Web
    "web_search", "web_extract",
    # Terminal + process management
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane, and close an agent's
    # read-only terminal tab (both gated on CYBERFOX_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (CYBERFOX_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `cyberfox tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``cyberfox tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (CYBERFOX_KANBAN_TASK env "
    ...
    # Coding posture (base Cyberfox — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in cyberfox_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Cyberfox toolsets (CLI + messaging platforms)
    #
    ...
    # the `cyberfox send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "cyberfox-cli": {
        ...
        "tools": _CYBERFOX_CORE_TOOLS,
        "includes": []
    },

    "cyberfox-cron": {
        # Mirrors cyberfox-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `cyberfox tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as cyberfox-cli; gated by `cyberfox tools`",
        "tools": _CYBERFOX_CORE_TOOLS,
        "includes": []
    },
    ...
    "cyberfox-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _CYBERFOX_CORE_TOOLS,
        "includes": []
    },

    "cyberfox-homeassistant": {
    ...
    "cyberfox-email": {
        "description": "Email bot toolset - interact with Cyberfox via email (IMAP/SMTP)",
        "tools": _CYBERFOX_CORE_TOOLS,
        "includes": []
    },
    ...
    "cyberfox-sms": {
        "description": "SMS bot toolset - interact with Cyberfox via SMS (Twilio)",
        "tools": _CYBERFOX_CORE_TOOLS,
        "includes": []
    },
    ...
    "cyberfox-webhook": {
        ...
        "tools": _CYBERFOX_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "cyberfox-gateway": {
        ...
        "tools": [],
        "includes": ["cyberfox-telegram", "cyberfox-discord", "cyberfox-whatsapp", "cyberfox-slack", "cyberfox-signal", "cyberfox-bluebubbles", "cyberfox-homeassistant", "cyberfox-email", "cyberfox-sms", "cyberfox-mattermost", "cyberfox-matrix", "cyberfox-dingtalk", "cyberfox-feishu", "cyberfox-wecom", "cyberfox-wecom-callback", "cyberfox-weixin", "cyberfox-qqbot", "cyberfox-webhook", "cyberfox-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``cyberfox-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_CYBERFOX_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``cyberfox-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (cyberfox-<name>).
        # Gives them _CYBERFOX_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("cyberfox-"):
            platform_name = name[len("cyberfox-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_CYBERFOX_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
