...
load_cyberfox_dotenv(
    cyberfox_home=_cyberfox_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.cyberfox/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_cyberfox_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("CYBERFOX_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("CYBERFOX_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("CYBERFOX_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent CyberfoxCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Cyberfox agent → needs provider credentials.
        # Tier-1 secrets (gateway/GitHub/infra) are still stripped (#29157).
        env = cyberfox_subprocess_env(inherit_credentials=True)
        if profile_home:
            ...
            # gateway's launch CYBERFOX_HOME (#40677).
            env["CYBERFOX_HOME"] = str(profile_home)
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from cyberfox_cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from cyberfox_cli.plugins import invoke_hook
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `cyberfox --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("CYBERFOX_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `cyberfox --tui`, so it must NOT count as dead here (doing
...
def _max_live_sessions() -> int:
    try:
        from cyberfox_cli.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its CYBERFOX_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_cyberfox_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s CYBERFOX_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_cyberfox_home``).
    ...
    def wrapper(rid, params):
        ...
        token = set_cyberfox_home_override(home)
        try:
            return handler(rid, params)
        finally:
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``cyberfox dashboard`` was launched) instead of the configured
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `cyberfox` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # CYBERFOX_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_cyberfox_home_override(profile_home)
                try:
                    from cyberfox_state import SessionDB
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from cyberfox_cli.runtime_provider import canonical_custom_identity
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _cyberfox_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_cyberfox_home_override()
        home = override if isinstance(override, str) and override else _cyberfox_home
...
def _apply_managed(cfg: dict) -> dict:
    ...
    cyberfox_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _save_cfg(cfg: dict):
    ...
    from cyberfox_cli.config import atomic_config_write

    path = _cyberfox_home / "config.yaml"
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["CYBERFOX_GATEWAY_SESSION"] = "1"
...
def resolve_skin() -> dict:
    try:
        from cyberfox_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("CYBERFOX_MODEL", "")
        or os.environ.get("CYBERFOX_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads CYBERFOX_MODEL /
    CYBERFOX_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`cyberfox --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads CYBERFOX_MODEL /
    # CYBERFOX_INFERENCE_MODEL, which `cyberfox --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("CYBERFOX_TUI_PROVIDER", "").strip()
    ...
    try:
        from cyberfox_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("CYBERFOX_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("CYBERFOX_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("CYBERFOX_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Cyberfox): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `cyberfox --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] CYBERFOX_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from cyberfox_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown CYBERFOX_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in CYBERFOX_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid CYBERFOX_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The desktop Project tools are off _CYBERFOX_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off cyberfox-cli's tool universe — can't
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid CYBERFOX_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    custom_provs = None
    try:
        from cyberfox_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (CYBERFOX_MODEL /
    # CYBERFOX_INFERENCE_MODEL / CYBERFOX_TUI_PROVIDER / CYBERFOX_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `cyberfox --tui -m`
...
def _get_usage(agent) -> dict:
    ...
    # CYBERFOX_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("CYBERFOX_DEV_CREDITS")):
        try:
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from cyberfox_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from cyberfox_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from cyberfox_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from cyberfox_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("CYBERFOX_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("CYBERFOX_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``CyberfoxCLI.__init__`` and ``gateway/run.py``:
...
def _resolve_runtime_with_fallback(
    ...
    ``cyberfox_cli/cli_agent_setup_mixin.py``: when the primary provider
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses cyberfox_cli.mcp_startup; TUI stdio keeps
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("CYBERFOX_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("CYBERFOX_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("CYBERFOX_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("CYBERFOX_IGNORE_RULES")),
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind CYBERFOX_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `CYBERFOX_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_cyberfox_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        raw_history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_cyberfox_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_cyberfox_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds CYBERFOX_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``cyberfox --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``cyberfox gateway`` process, whose
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Cyberfox has to support both the legacy 8-row petdex atlas and the current
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from cyberfox_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from cyberfox_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from cyberfox_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from cyberfox_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from cyberfox_cli.pets import set_pet_scale
...
def _pet_gen_root():
    ...
    root = get_cyberfox_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("CYBERFOX_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
def _(rid, params: dict) -> dict:
    ...
    Fail-open: a portal hiccup or logged-out account yields
    {logged_in: false}, never an error the user has to parse.
    """
    try:
...
# (agent/billing_view.py) — same as /credits.


@method("billing.state")
def _(rid, params: dict) -> dict:
    """Billing provider removed - return not-logged-in stub.

    Fail-open: the TUI treats {ok:true, logged_in:false} as "no billing".
    """
    return _ok(rid, {"ok": True, "logged_in": False})


@method("billing.charge")
def _(rid, params: dict) -> dict:
    """Billing provider removed - stub returning unavailable."""
    return _ok(rid, {"ok": False, "error": "unavailable", "message": "billing unavailable"})
...
def _(rid, params: dict) -> dict:
    """Billing provider removed - stub returning not granted."""
    return _ok(rid, {"ok": False, "granted": False})
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Cyberfox profile home
    # (~/.cyberfox/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $CYBERFOX_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_cyberfox_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn CYBERFOX_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_cyberfox_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from cyberfox_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from cyberfox_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: cyberfox_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from cyberfox_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _cyberfox_home / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".cyberfox" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.cyberfox/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.cyberfox/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Cyberfox Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Cyberfox/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Cyberfox Desktop. If it serves Cyberfox/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from cyberfox_cli.model_switch import parse_model_flags
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("CYBERFOX_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["CYBERFOX_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("CYBERFOX_YOLO_MODE", None)
                    nv = "0"
...
def _projects_payload(conn) -> dict:
    from cyberfox_cli import projects_db as pdb
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under CYBERFOX_HOME (~/.cyberfox by default) — config/sessions/skills,
    ...
    cyberfox_home = os.path.realpath(str(get_cyberfox_home()))

    return real == home or real == cyberfox_home or real.startswith(cyberfox_home + os.sep)
...
def _discover_repos_payload(db, *, conn=None, backfill: bool = True) -> list[dict]:
    ...
    repos even with zero cyberfox sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (cyberfox home subtree + bare
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from cyberfox_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_cyberfox_home), "display": display_cyberfox_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from cyberfox_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from cyberfox_cli.runtime_provider import resolve_runtime_provider
        from cyberfox_cli.auth import has_usable_secret
        from cyberfox_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Cyberfox provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.cyberfox/.env`` into the gateway process via
    ``cyberfox_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `cyberfox` is interactive — use `/cyberfox chat -q …` or run `cyberfox` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`cyberfox setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`cyberfox gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`cyberfox sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`cyberfox config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m cyberfox_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "cyberfox_cli.main", *argv],
            capture_output=True,
            text=True,
            timeout=min(int(params.get("timeout", 240)), 600),
            cwd=os.getcwd(),
            # cli.exec runs `python -m cyberfox_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from cyberfox_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from cyberfox_cli.commands import SlashCommandCompleter
    ...
    try:
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `cyberfox model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.cyberfox/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from cyberfox_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from cyberfox_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("CYBERFOX_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("CYBERFOX_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["CYBERFOX_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from cyberfox_cli.voice import stop_continuous
            ...
            os.environ["CYBERFOX_VOICE_TTS"] = "0"
    ...
    if action == "tts":
        ...
        os.environ["CYBERFOX_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from cyberfox_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from cyberfox_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from cyberfox_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from cyberfox_cli.browser_connect import launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("CYBERFOX_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("CYBERFOX_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_cyberfox_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``cyberfox journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from cyberfox_cli.skills_hub import do_install
        ...
        if action == "browse":
            from cyberfox_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from cyberfox_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``cyberfox plugins`` / the dashboard, so the three surfaces
