...
React + Ink terminal UI for Cyberfox. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `CYBERFOX_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `cyberfox setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage Cyberfox terminal billing — buy credits, auto-reload, limits
...
- Input history is stored in `~/.cyberfox/.cyberfox_history` or under `CYBERFOX_HOME`.
...
### Billing (`billing.ts`)
`/billing` — manage Cyberfox terminal billing — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — Cyberfox credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `cyberfox setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/cyberfox-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external cyberfox setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage Cyberfox terminal billing
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.cyberfox/tui-parent.log
    ...
    types/
      cyberfox-ink.d.ts            type declarations for @cyberfox/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent CyberfoxCLI subprocess for slash commands
```