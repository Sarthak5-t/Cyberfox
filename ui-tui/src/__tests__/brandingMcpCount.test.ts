...
import { renderSync } from '@cyberfox/ink'
...
// cyberfox_cli/banner.py) and the "connected" label on the MCP collapse toggle.
//
// Regression: branding.tsx used the raw `info.mcp_servers.length`, so a
// disabled `linear` server alongside a connected `cyberfox-support` server made
...
describe('branding MCP headline count', () => {
  it('counts only connected servers, not configured-but-disabled ones', async () => {
    const frame = await renderFooter(
      baseInfo([
        mcp({ connected: true, name: 'cyberfox-support', status: 'connected', tools: 6 }),
