import type { RunExternalProcess } from '@cyberfox/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`cyberfox ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching cyberfox: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`cyberfox ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
