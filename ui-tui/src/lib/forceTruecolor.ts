...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.CYBERFOX_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Cyberfox on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via CYBERFOX_TUI_TRUECOLOR=1.
