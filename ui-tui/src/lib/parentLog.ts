...
const logDir = join(process.env.CYBERFOX_HOME?.trim() || join(homedir(), '.cyberfox'), 'logs')
...
// a real ~/.cyberfox (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('cyberfox-tui: parent lifecycle log unavailable\n')
    }
  }
}
