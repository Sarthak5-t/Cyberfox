...
import { SelectionSwitcher } from "@cyberfox/ui/ui/components/selection-switcher";
...
import { useBelowBreakpoint } from "@cyberfox/ui/hooks/use-below-breakpoint";
...
const SIDEBAR_COLLAPSED_KEY = "cyberfox-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="cyberfox-sidebar-plugin-nav-heading"
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateCyberfox = status?.can_update_cyberfox === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'cyberfox update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "cyberfox update";
    return (
      t.status.updateCyberfoxConfirmMessage ??
      `This will run 'cyberfox update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateCyberfoxConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateCyberfox) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateCyberfox,
      runningLabel: t.status.updatingCyberfox,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Cyberfox gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateCyberfoxConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateCyberfoxConfirmTitle ?? `${t.status.updateCyberfox}?`}
    />
    </>
  );
}
