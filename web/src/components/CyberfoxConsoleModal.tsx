...
interface CyberfoxConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function CyberfoxConsoleModal({ open, onClose }: CyberfoxConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("cyberfox> ");
  const inputPromptRef = useRef("cyberfox> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "cyberfox> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Cyberfox Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="cyberfox-console-title"
    >
      <div
        className={cn(
          themedBody,
