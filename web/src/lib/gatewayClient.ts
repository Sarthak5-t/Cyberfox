...
} from "@cyberfox/shared";

import { CYBERFOX_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
export class GatewayClient extends JsonRpcGatewayClient {
  ...
  async connect(token?: string): Promise<void> {
    ...
    // override path.
    const authParam: [string, string] = token
      ? ["token", token]
      : await buildWsAuthParam();
    if (!authParam[1]) {
      throw new Error(
        "Session token not available — page must be served by the Cyberfox dashboard server",
      );
    }
