/**
 * ChatPage — embeds `cyberfox --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (cyberfox_cli/web_server.py)                          .
 *          ▼                                                              .
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__CYBERFOX_SESSION_TOKEN__ &&
    !window.__CYBERFOX_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `cyberfox dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__CYBERFOX_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Cyberfox TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[cyberfox-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__CYBERFOX_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets CYBERFOX_HOME pointed at the
    ...
    // forwarding them into Cyberfox. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="cyberfox-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
