...
export default function SystemPage() {
  ...
  // spawned `cyberfox import` runs non-interactively (stdin is /dev/null), so
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_cyberfox === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkCyberfoxUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_cyberfox],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_cyberfox === false) {
      showToast(
        "Cyberfox updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updateCyberfox();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "cyberfox-update");
  ...
  const canUpdateCyberfox = status?.can_update_cyberfox !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateCyberfox && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Cyberfox?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'cyberfox update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'cyberfox update' (${updateInfo?.update_command ?? "cyberfox update"}) and restart the gateway when it finishes.`
        }
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Cyberfox</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.cyberfox_version}</span>
                  {canUpdateCyberfox &&
                    updateInfo &&
      ...
      <section className="flex flex-col gap-3">
        <H2 variant="sm" className="flex items-center gap-2 text-muted-foreground">
          <Globe className="h-4 w-4" /> Cyberfox
        </H2>
        <Card>
          <CardContent className="flex flex-col gap-3 py-4">
            <div className="flex items-center gap-3">
              ...
              <a
                href={portal?.subscription_url || "https://portal.example.com/manage-subscription"}
                target="_blank"
                rel="noreferrer"
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Log in with <span className="font-mono">cyberfox portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="flex flex-col gap-4 py-4">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
              ...
              <span className="ml-auto">
                New credentials:{" "}
                <span className="font-mono">cyberfox memory setup</span>
              </span>
            </div>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$CYBERFOX_HOME/backups/cyberfox-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Cyberfox backup?"
              description={`This will overwrite your current Cyberfox configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Cyberfox team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
