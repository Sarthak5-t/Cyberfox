 ...
 *
 * Plugins call window.__CYBERFOX_PLUGINS__.register(name, Component)
...
import { Card, CardHeader, CardTitle, CardContent } from "@cyberfox/ui/ui/components/card";
import { Input } from "@cyberfox/ui/ui/components/input";
import { Label } from "@cyberfox/ui/ui/components/label";
import { Separator } from "@cyberfox/ui/ui/components/separator";
import { Tabs, TabsList, TabsTrigger } from "@cyberfox/ui/ui/components/tabs";
 ...
 * Exposed at runtime as ``window.__CYBERFOX_PLUGIN_SDK__.sdkVersion`` so a
...
export function exposePluginSDK() {
  window.__CYBERFOX_PLUGINS__ = {
    register: registerPlugin,
    registerSlot,
  };

  window.__CYBERFOX_PLUGIN_SDK__ = {
    ...
    // window.__CYBERFOX_SESSION_TOKEN__ directly.
    authedFetch,
    ...
    // UI components — Cyberfox DS where available, shadcn/ui primitives elsewhere.
    components: {
      Card,
      CardHeader,
      CardTitle,
