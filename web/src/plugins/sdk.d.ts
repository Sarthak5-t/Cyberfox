/**
 * Cyberfox Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__CYBERFOX_PLUGIN_SDK__`` and
 * ``window.__CYBERFOX_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__CYBERFOX_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@cyberfox/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``CyberfoxPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@cyberfox/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @cyberfox/ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__CYBERFOX_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__CYBERFOX_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__CYBERFOX_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface CyberfoxPluginSDK {
  ...
  /**
   * Shared UI primitives (Cyberfox DS / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __CYBERFOX_PLUGIN_SDK__?: CyberfoxPluginSDK;
    __CYBERFOX_PLUGINS__?: PluginRegistry;
  }
}
