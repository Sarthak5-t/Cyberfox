...
The ACP adapter wraps Cyberfox' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
cyberfox acp / cyberfox-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.cyberfox/.env
  -> configure stderr logging
  -> construct CyberfoxACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'cyberfox-agent[acp]==<version>' cyberfox-acp`, pointed at the `cyberfox-agent` PyPI release.
...
- `allow_once` -> Cyberfox `once`
- `allow_always` -> Cyberfox `always`
- reject options -> Cyberfox `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Cyberfox tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["cyberfox-acp"])
...
Instead it reuses Cyberfox' runtime resolver:

- `acp_adapter/auth.py`
- `cyberfox_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Cyberfox provider/credentials. It also always advertises a terminal setup auth method (`cyberfox-setup`, args `--setup`) so first-run registry clients can open Cyberfox' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.cyberfox/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `cyberfox-acp` toolset definition
- `cyberfox_cli/main.py` — `cyberfox acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `cyberfox-acp` script
