...
description: "How to add a new inference provider to Cyberfox Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Cyberfox can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `cyberfox model` menu entries
...
1. `cyberfox_cli/auth.py` decides how credentials are found.
2. `cyberfox_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `cyberfox_cli/models.py` and `cyberfox_cli/main.py` make the provider show up in the CLI. (`cyberfox_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `cyberfox_cli/auth.py`
2. `cyberfox_cli/models.py`
3. `cyberfox_cli/runtime_provider.py`
4. `cyberfox_cli/main.py`
...
:::tip
`cyberfox_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `cyberfox setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$CYBERFOX_HOME/plugins/model-providers/` both get picked up.
...
7. `cyberfox model` menu includes the provider
8. `cyberfox setup` wizard delegates to `main.py` automatically
...
User plugins at `$CYBERFOX_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- OAuth or token refresh (the web dashboard, Codex, Qwen Portal, Copilot)
...
- Provider-specific `cyberfox model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `cyberfox_cli/auth.py`
- `_PROVIDER_LABELS` in `cyberfox_cli/models.py`
- `_PROVIDER_ALIASES` in both `cyberfox_cli/auth.py` and `cyberfox_cli/models.py`
- CLI `--provider` choices in `cyberfox_cli/main.py`
...
## Step 2: Add auth metadata in `cyberfox_cli/auth.py`
...
- What env vars should Cyberfox check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `cyberfox_cli/models.py`
...
## Step 4: Resolve runtime data in `cyberfox_cli/runtime_provider.py`
...
Be careful with API-key precedence. Cyberfox already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `cyberfox_cli/main.py`

A provider is not discoverable until it shows up in the interactive `cyberfox model` flow.

Update these in `cyberfox_cli/main.py`:
...
:::tip
`cyberfox_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `cyberfox model` and `cyberfox setup` automatically.
:::
...
When you add a native provider, double-check that Cyberfox is only sending fields that provider actually understands.
...
- `tests/cyberfox_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/cyberfox_cli/test_model_switch_custom_providers.py` (and adjacent `tests/cyberfox_cli/test_model_switch_*.py`)
- `tests/cyberfox_cli/test_setup_model_provider.py`
...
python -m pytest tests/cyberfox_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/cyberfox_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
...
```bash
source venv/bin/activate
python -m cyberfox_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m cyberfox_cli.main model
python -m cyberfox_cli.main setup
```
...
- [ ] `ProviderConfig` added in `cyberfox_cli/auth.py`
- [ ] aliases added in `cyberfox_cli/auth.py` and `cyberfox_cli/models.py`
- [ ] model catalog added in `cyberfox_cli/models.py`
- [ ] runtime branch added in `cyberfox_cli/runtime_provider.py`
- [ ] CLI wiring added in `cyberfox_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `cyberfox model` but not `cyberfox setup`
