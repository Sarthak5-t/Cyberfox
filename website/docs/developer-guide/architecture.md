...
sidebar_position: 1
title: "Architecture"
description: "Cyberfox Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Cyberfox Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ cyberfox_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # CyberfoxCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── cyberfox_state.py           # SQLite session/state database with FTS5
├── cyberfox_constants.py       # CYBERFOX_HOME, profile-aware paths
...
│
├── cyberfox_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `cyberfox` subcommands (large file)
...
│   ├── skills_config.py      # cyberfox skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # cyberfox tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # cyberfox gateway start/stop
│
...
```text
User input → CyberfoxCLI.process_input()
...
Three discovery sources: `~/.cyberfox/plugins/` (user), `.cyberfox/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `cyberfox plugins` or `config.yaml`.
...
Exposes Cyberfox as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`cyberfox -p <name>`) gets its own CYBERFOX_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
