...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Cyberfox Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Cyberfox Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Cyberfox core? Start with [Build a Cyberfox Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Cyberfox itself? Start with [Adding Tools](./adding-tools.md)
...
The installer creates the Cyberfox venv, wires the `cyberfox` command, stamps the
install method for `cyberfox update`, and clones the full git project into
`$CYBERFOX_HOME/cyberfox-agent` (usually `~/.cyberfox/cyberfox-agent`). That keeps your
...
Use this only if you intentionally do not want Cyberfox' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `cyberfox` entrypoint from this venv; running the
system `python3 -m cyberfox_cli.main` can pick up unrelated system Python
packages.
...
uv venv ~/.cyberfox/venvs/cyberfox-dev --python 3.11
...
```bash
# The standard installer already put `cyberfox` on PATH.
cyberfox doctor
cyberfox chat -q "Hello"
```

If you used the manual clone fallback, run `./cyberfox` from the checkout or
...
Cyberfox has terminal access. Security matters.
...
2. **Test manually**: Run `cyberfox` and exercise the code path you changed
