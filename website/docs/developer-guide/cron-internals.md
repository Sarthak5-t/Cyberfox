...
description: "How Cyberfox stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `cyberfox_cli/cron.py` | CLI `cyberfox cron` subcommands |
...
Jobs are stored in `~/.cyberfox/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `cyberfox cron` commands are run or during active CLI sessions.
...
```python
# ~/.cyberfox/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.cyberfox/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `cyberfox cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `cyberfox cron` CLI provides direct job management:

```bash
cyberfox cron list                    # Show all jobs
cyberfox cron create                  # Interactive job creation (alias: add)
cyberfox cron edit <job_id>           # Edit job configuration
cyberfox cron pause <job_id>          # Pause a running job
cyberfox cron resume <job_id>         # Resume a paused job
cyberfox cron run <job_id>            # Trigger immediate execution
cyberfox cron remove <job_id>         # Delete a job
```
