...
Cyberfox Agent uses a SQLite database (`~/.cyberfox/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `cyberfox_state.py`


## Architecture Overview

```
~/.cyberfox/state.db (SQLite, WAL mode)
...
Multiple cyberfox processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.cyberfox/state.db
...
Default path: `~/.cyberfox/state.db`

This is derived from `cyberfox_constants.get_cyberfox_home()` which resolves to
`~/.cyberfox/` by default, or the value of `CYBERFOX_HOME` environment variable.
