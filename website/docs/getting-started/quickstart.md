...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Cyberfox Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Cyberfox setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Cyberfox Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Cyberfox Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Cyberfox for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Cyberfox working on my machine | `cyberfox setup` | Run a real chat and verify it responds |
| I already know my provider | `cyberfox model` | Save the config, then start chatting |
| I want a bot or always-on setup | `cyberfox gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `cyberfox model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `cyberfox model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Cyberfox cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Cyberfox Agent
### With the Cyberfox Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Cyberfox Desktop installer](https://github.com/Sarthak5-t/Cyberfox/) from our website and run it.

### Without Cyberfox Desktop:
For a command-line only install without Cyberfox Desktop, run:
...
The single most important setup step. Use `cyberfox model` to walk through the choice interactively:

```bash
cyberfox model
```

:::tip Easiest path: the web dashboard
...
:::info Setup modes
On a fresh install, `cyberfox setup` offers three modes:

- **Quick Setup (the web dashboard)** — free OAuth login, no API keys; sets up a model plus the Tool Gateway tools. The recommended fast path.
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `cyberfox update`. Re-enable anything later with `cyberfox tools`, seed skills with `cyberfox skills opt-in --sync`, or tune settings with `cyberfox setup agent`.
:::
...
|----------|-----------|---------------|
| **the web dashboard** | Subscription-based, zero-config | OAuth login via `cyberfox model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `cyberfox model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `cyberfox model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `cyberfox_cli/models.py` may change between releases) | `cyberfox model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `cyberfox model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `cyberfox model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `cyberfox model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `cyberfox model` (requires `copilot` CLI + `copilot login`) |
...
Cyberfox Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `cyberfox model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Cyberfox separates secrets from normal config:

- **Secrets and tokens** → `~/.cyberfox/.env`
- **Non-secret settings** → `~/.cyberfox/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
cyberfox config set model anthropic/claude-opus-4.6
cyberfox config set terminal.backend docker
cyberfox config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
cyberfox            # classic CLI
cyberfox --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Cyberfox ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `cyberfox` vs `cyberfox --tui`.
:::
...
- Cyberfox replies without error
...
```bash
cyberfox --continue    # Resume the most recent session
cyberfox -c            # Short form
```
...
```bash
cyberfox gateway setup    # Interactive platform configuration
```
...
- `cyberfox tools` — tune tool access per platform
- `cyberfox skills` — browse and install reusable workflows
...
```bash
cyberfox config set terminal.backend docker    # Docker isolation
cyberfox config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Cyberfox install directory (the curl installer placed it at
# ~/.cyberfox/cyberfox-agent on Linux/macOS or %LOCALAPPDATA%\cyberfox\cyberfox-agent on Windows):
cd ~/.cyberfox/cyberfox-agent
...
Skills are on-demand instruction documents that teach Cyberfox how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Cyberfox ships with a catalog of bundled skills already installed in `~/.cyberfox/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
cyberfox skills browse                      # list everything available
cyberfox skills search kubernetes           # find skills by keyword
cyberfox skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `cyberfox skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Cyberfox ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.cyberfox/cyberfox-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Cyberfox opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `cyberfox model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `cyberfox gateway setup` and check `cyberfox gateway status` |
| `cyberfox --continue` can't find old session | Switched profiles or session never saved | Check `cyberfox sessions list` and confirm you're in the right profile |
...
| `cyberfox doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `cyberfox model`
3. `cyberfox setup`
4. `cyberfox sessions list`
5. `cyberfox --continue`
6. `cyberfox gateway status`
...
| Command | Description |
|---------|-------------|
| `cyberfox` | Start chatting |
| `cyberfox model` | Choose your LLM provider and model |
| `cyberfox tools` | Configure which tools are enabled per platform |
| `cyberfox setup` | Full setup wizard (configures everything at once) |
| `cyberfox doctor` | Diagnose issues |
| `cyberfox update` | Update to latest version |
| `cyberfox gateway` | Start the messaging gateway |
| `cyberfox --continue` | Resume last session |
