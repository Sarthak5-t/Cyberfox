...
description: "Run Cyberfox Agent directly on an Android phone with Termux"
---

# Cyberfox on Android with Termux
...
Cyberfox Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Cyberfox from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Cyberfox now ships a Termux-aware installer path:
...
- links `cyberfox` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `cyberfox` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/cyberfox" "$PREFIX/bin/cyberfox"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `cyberfox` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.cyberfox/.env`.
...
### `cyberfox doctor` says ripgrep or Node is missing
