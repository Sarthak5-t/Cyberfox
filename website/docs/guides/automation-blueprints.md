...
Copy-paste blueprints for common automation patterns. Each blueprint uses Cyberfox's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`cyberfox webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `cyberfox webhook subscribe`) |
...
```bash
cyberfox cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/cyberfox-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/cyberfox-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
cyberfox webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
cyberfox cron create "0 9 * * 1" \
  "Scan the NousResearch/cyberfox-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/cyberfox-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (cyberfox_cli/commands.py, cyberfox_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (cyberfox_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
cyberfox cron create "0 6 * * *" \
  "Run a dependency security audit on the cyberfox-agent project.

1. cd ~/.cyberfox/cyberfox-agent && source .venv/bin/activate
...
```bash
cyberfox webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
cyberfox webhook subscribe alert-triage \
...
```python title="~/.cyberfox/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Cyberfox-Monitor/1.0"})
...
```bash
cyberfox cron create "every 30m" \
  ...
  --script ~/.cyberfox/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
cyberfox cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Cyberfox Agent development." \
...
```bash
cyberfox webhook subscribe github-issues \
  --events "issues" \
...
```bash
cyberfox webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
cyberfox webhook subscribe stripe-payments \
...
```bash
cyberfox cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the cyberfox-agent codebase.
