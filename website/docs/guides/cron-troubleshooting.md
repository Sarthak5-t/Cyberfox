...
description: "Diagnose and fix common Cyberfox cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`cyberfox gateway` for foreground, or `cyberfox gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `cyberfox cron tick`.
...
```bash
date
cyberfox cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.cyberfox/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.cyberfox/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.cyberfox/.env` |
...
| `local` | Write access to `~/.cyberfox/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `cyberfox cron list` for updated `last_error` field (if available).
...
```bash
cyberfox skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `cyberfox skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `cyberfox skills list`.
...
2. `~/.cyberfox/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `cyberfox cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Cyberfox config directory). Verify:
```bash
ls ~/.cyberfox/scripts/your-script.py   # Must exist
cyberfox cron edit <job_id> --script ~/.cyberfox/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `cyberfox skills install <skill-name>`.
...
```bash
ps aux | grep cyberfox
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.cyberfox/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.cyberfox/cron/jobs.json
chmod 600 ~/.cyberfox/cron/jobs.json   # Your user should own it
```
...
```bash
cyberfox cron list                    # Show all jobs, states, next_run times
cyberfox cron run <job_id>            # Schedule for next tick (for testing)
cyberfox cron edit <job_id>           # Fix configuration issues
cyberfox logs                         # View recent Cyberfox logs
cyberfox skills list                  # Verify installed skills
```
...
1. Run the job with `cyberfox cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.cyberfox/logs/agent.log` for scheduler messages and `~/.cyberfox/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/cyberfox-agent](https://github.com/NousResearch/cyberfox-agent) with:
