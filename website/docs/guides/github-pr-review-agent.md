...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Cyberfox Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Cyberfox instantly when PRs are opened or updated.
:::
...
- **Cyberfox Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  cyberfox gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.cyberfox/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Cyberfox can access GitHub. Start a chat:
...
Still in the chat, ask Cyberfox to review a real PR:
...
A skill gives Cyberfox consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.cyberfox/skills/code-review
```

Create `~/.cyberfox/skills/code-review/SKILL.md`:
...
Verify it loaded — start `cyberfox` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Cyberfox your team's standards:
...
2. Teach Cyberfox your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
cyberfox gateway status    # Is the gateway running?
cyberfox cron list         # Is the job enabled?
```
