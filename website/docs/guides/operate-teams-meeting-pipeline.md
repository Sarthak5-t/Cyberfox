...
```bash
cyberfox teams-pipeline validate
```
...
```bash
cyberfox teams-pipeline token-health
cyberfox teams-pipeline token-health --force-refresh
```
...
```bash
cyberfox teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
cyberfox teams-pipeline maintain-subscriptions
cyberfox teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Cyberfox cron (recommended if you already run the Cyberfox gateway)

Cyberfox ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.cyberfox/scripts/`. First create the script:

```bash
mkdir -p ~/.cyberfox/scripts
cat > ~/.cyberfox/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec cyberfox teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.cyberfox/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
cyberfox cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/cyberfox-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Cyberfox Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=cyberfox
EnvironmentFile=/etc/cyberfox/env
ExecStart=/usr/local/bin/cyberfox teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/cyberfox-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Cyberfox Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now cyberfox-teams-pipeline-maintain.timer
systemctl list-timers cyberfox-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/cyberfox teams-pipeline maintain-subscriptions >> /var/log/cyberfox/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.cyberfox/.env` at the top of a wrapper script that crontab calls.
...
```bash
cyberfox teams-pipeline subscriptions   # should show expirationDateTime advanced
cyberfox teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
cyberfox teams-pipeline list
cyberfox teams-pipeline list --status failed
cyberfox teams-pipeline show <job-id>
```

### Replay a stored job

```bash
cyberfox teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
cyberfox teams-pipeline fetch --meeting-id <meeting-id>
cyberfox teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `cyberfox teams-pipeline maintain-subscriptions --dry-run`
- inspect `cyberfox teams-pipeline list --status failed`
...
- run `cyberfox teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `cyberfox teams-pipeline run`
...
- [ ] `cyberfox teams-pipeline validate` returns an OK snapshot
- [ ] `cyberfox teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Cyberfox cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
