...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `cyberfox send`."
---

# Pipe Script Output to Messaging Platforms

`cyberfox send` is a small, scriptable CLI that pushes a message to any
messaging platform Cyberfox is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | cyberfox send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `cyberfox
...
cyberfox send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
cyberfox send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
cyberfox send --list

# Filter by platform
cyberfox send --list telegram
```
...
Any platform Cyberfox ships adapters for works as a target:
...
`cyberfox send` resolves the message body in this order:

1. **Positional argument** — `cyberfox send --to telegram "hi"`
2. **`--file PATH`** — `cyberfox send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | cyberfox send --to telegram`

When stdin is a TTY (no pipe), Cyberfox does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  cyberfox send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `cyberfox send` reuses your Cyberfox config, the same script works on
any host where Cyberfox is installed — no need to export bot tokens into
...
`cyberfox send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  cyberfox send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | cyberfox send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.cyberfox/bin/cyberfox send \
...
```bash
./train.py --epochs 200 && \
  cyberfox send --to telegram "training done" || \
  cyberfox send --to telegram "training failed (exit $?)"
```
...
cyberfox send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(cyberfox send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `cyberfox send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `cyberfox send` calls
the platform's REST endpoint directly using credentials from
`~/.cyberfox/.env` and `~/.cyberfox/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `cyberfox gateway start` and retry.
...
# Machine-readable
cyberfox send --list --json
```

The listing is built from `~/.cyberfox/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`cyberfox gateway
...
| Approach | Multi-platform | Reuses Cyberfox creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `cyberfox send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`cyberfox send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `cyberfox send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `cyberfox send` shares with cron delivery.
