...
- Cyberfox Agent installed ([Quickstart](/getting-started/quickstart))
...
OAuth needs a browser, but the loopback callback runs on the machine where Cyberfox is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
cyberfox setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
cyberfox auth add nous --type oauth
# Then re-run `cyberfox setup --portal` to wire the provider + gateway
```
...
You should see Cyberfox call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`cyberfox setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Cyberfox will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
cyberfox cron create "every day at 9am" \
...
If you use [Cyberfox profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.cyberfox/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `cyberfox portal info` shows "not logged in" after `cyberfox setup --portal`
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Cyberfox doesn't replay it endlessly. Just log in again:
...
`cyberfox portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Cyberfox profiles where you're using the wrong one (check `cyberfox profile list`)

### Want to revoke and start clean

```bash
cyberfox auth logout nous       # wipes the local refresh token
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Cyberfox configurations
