...
This tutorial walks you through setting up a Telegram bot powered by Cyberfox Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Cyberfox Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.cyberfox/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Cyberfox itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Cyberfox Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_cyberfox_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Cyberfox Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.cyberfox/.env`:
...
```
[Gateway] Starting Cyberfox Gateway...
...
# View live logs
journalctl --user -u cyberfox-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `cyberfox gateway install` to update the plist.
:::
...
```bash
cyberfox gateway stop && cyberfox gateway start
```
...
cyberfox pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.cyberfox/.env`:
...
Control how much detail the bot shows when using tools. In `~/.cyberfox/config.yaml`:
...
Customize how the bot communicates by editing `~/.cyberfox/SOUL.md`:

For a full guide, see [Use SOUL.md with Cyberfox](/guides/use-soul-with-cyberfox).
...
```bash
# From the CLI
cyberfox cron list          # View all scheduled jobs
cyberfox cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u cyberfox-gateway` (Linux) or `~/.cyberfox/logs/gateway.log` (macOS) |
| Cron job output | `~/.cyberfox/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.cyberfox/cron/jobs.json` |
| Pairing data | `~/.cyberfox/pairing/` |
| Session history | `~/.cyberfox/sessions/` |
