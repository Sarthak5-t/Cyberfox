---
sidebar_position: 6
title: "Use MCP with Cyberfox"
description: "A practical guide to connecting MCP servers to Cyberfox Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Cyberfox

This guide shows how to actually use MCP with Cyberfox Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Cyberfox tool
- you want Cyberfox to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Cyberfox to internal APIs, databases, or company systems without modifying Cyberfox core

Do not use MCP when:
- a built-in Cyberfox tool already solves the job well
...
- Cyberfox remains the agent
- MCP servers contribute tools
- Cyberfox discovers those tools at startup or reload time
...
If you installed Cyberfox with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Cyberfox banner/status should show MCP integration when configured
- ask Cyberfox what tools it has available
...
## WSL2: bridge Cyberfox in WSL to Windows Chrome

This is the practical setup when:

- Cyberfox runs inside WSL2
...
In this setup, Cyberfox does **not** connect to Chrome directly. Instead:

- Cyberfox runs in WSL
- Cyberfox starts a local stdio MCP server
...
```text
Cyberfox (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Cyberfox stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Cyberfox core browser transport
...
```bash
cyberfox mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
cyberfox mcp test chrome-devtools-win
```

Then start a fresh Cyberfox session or run:
...
Once loaded, Cyberfox can use the MCP-prefixed browser tools directly. For example:
...
If Cyberfox runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Cyberfox from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Cyberfox from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Cyberfox:
...
2. Cyberfox-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Cyberfox will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Cyberfox to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Cyberfox to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Cyberfox remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
cyberfox mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
cyberfox mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `cyberfox mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Cyberfox to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Cyberfox to merge, publish, deploy, or spawn runtimes.
...
Now Cyberfox can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Cyberfox core.
...
Because Cyberfox now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
