...
This page covers all commands related to [Cyberfox profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `cyberfox profile` without a subcommand shows help.
...
## `cyberfox profile use`

```bash
cyberfox profile use <name>
```

Sets `<name>` as the active profile. All subsequent `cyberfox` commands (without `-p`) will use this profile.
...
cyberfox profile use default
```

## `cyberfox profile create`

```bash
cyberfox profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `cyberfox profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `cyberfox update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.cyberfox`), use `cyberfox skills opt-out` / `cyberfox skills opt-in`. |
...
cyberfox profile create work2 --clone-from work

# Clone everything from a specific profile
cyberfox profile create work2-backup --clone-from work --clone-all
```

## `cyberfox profile describe`

```bash
cyberfox profile describe [<name>] [options]
```
...
cyberfox profile describe researcher

# Set it explicitly
cyberfox profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
cyberfox profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
cyberfox profile describe --all --auto
```

## `cyberfox profile delete`

```bash
cyberfox profile delete <name> [options]
```
...
```bash
cyberfox profile delete mybot
cyberfox profile delete mybot --yes
```
...
## `cyberfox profile show`

```bash
cyberfox profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Cyberfox home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ cyberfox profile show work
Profile: work
Path:    ~/.cyberfox/profiles/work
...
## `cyberfox profile alias`

```bash
cyberfox profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Cyberfox installation.
...
```bash
cyberfox profile alias work
# Creates/updates ~/.local/bin/work

cyberfox profile alias work --name mywork
# Creates ~/.local/bin/mywork

cyberfox profile alias work --remove
# Removes the wrapper script
```

## `cyberfox profile rename`

```bash
cyberfox profile rename <old-name> <new-name>
```
...
```bash
cyberfox profile rename mybot assistant
# ~/.cyberfox/profiles/mybot → ~/.cyberfox/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `cyberfox profile export`

```bash
cyberfox profile export <name> [options]
```
...
```bash
cyberfox profile export work
# Creates work.tar.gz in the current directory

cyberfox profile export work -o ./work-2026-03-29.tar.gz
```

## `cyberfox profile import`

```bash
cyberfox profile import <archive> [options]
```
...
```bash
cyberfox profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

cyberfox profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`cyberfox profile export` / `import` are still the right commands for
...
### `cyberfox profile install`

```bash
cyberfox profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `cyberfox -p telemetry`). |
...
cyberfox profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
cyberfox profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
cyberfox profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
cyberfox profile install ./telemetry/
```

### `cyberfox profile update`

```bash
cyberfox profile update <name> [--force-config] [--yes]
```
...
### `cyberfox profile info`

```bash
cyberfox profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Cyberfox version, author, env var requirements, the source URL/path, and
...
`cyberfox profile list` also shows the distribution name and version in a
`Distribution` column, and `cyberfox profile show <name>` / `delete <name>`
...
cyberfox profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
cyberfox profile install https://github.com/your-org/internal-assistant.git
```
...
`cyberfox_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Cyberfox version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Cyberfox can clone from.
3. Tell recipients to run `cyberfox profile install <your-repo-url>`.
...
## `cyberfox -p` / `cyberfox --profile`

```bash
cyberfox -p <name> <command> [options]
cyberfox --profile <name> <command> [options]
```

Global flag to run any Cyberfox command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
cyberfox -p work chat -q "Check the server status"
cyberfox --profile dev gateway start
cyberfox -p personal skills list
cyberfox -p work config edit
```

## `cyberfox completion`

```bash
cyberfox completion <shell>
```
...
- `cyberfox profile <TAB>` — subcommands (list, use, create, etc.)
- `cyberfox profile use <TAB>` — profile names
- `cyberfox -p <TAB>` — profile names
