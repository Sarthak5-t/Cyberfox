...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Cyberfox Agent terminal interface — commands, keybindings, personalities, and more"
---
...
:::tip
Cyberfox also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `cyberfox --tui` — see the [TUI](tui.md) guide.
:::
...
cyberfox -s cyberfox-agent-dev,github-auth
cyberfox chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
cyberfox --continue             # Resume the most recent CLI session (-c)
cyberfox --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
cyberfox chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
cyberfox -w                         # Interactive mode in worktree
cyberfox -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Cyberfox CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Cyberfox CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `CYBERFOX_YOLO_MODE` is on (either `cyberfox --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`cyberfox -c` or `cyberfox --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Cyberfox to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Cyberfox supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Cyberfox replies |
...
```bash
cyberfox -s cyberfox-agent-dev,github-auth
cyberfox chat -s github-pr-workflow -s github-auth
```

Cyberfox loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.cyberfox/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.cyberfox/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Cyberfox..."
  # Add your own!
```
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Cyberfox — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Cyberfox is working, Cyberfox prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Cyberfox to the background — just like any terminal process. The shell prints a confirmation:

```
Cyberfox Agent has been suspended. Run `fg` to bring Cyberfox Agent back.
```
...
Resume this session with:
  cyberfox --resume 20260225_143052_a1b2c3
...
```bash
cyberfox --continue                          # Resume the most recent CLI session
cyberfox -c                                  # Short form
cyberfox -c "my project"                     # Resume a named session (latest in lineage)
cyberfox --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
cyberfox --resume "refactoring auth"         # Resume by title
cyberfox -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `cyberfox sessions rename <id> <title>` from the command line. Use `cyberfox sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Cyberfox's SQLite state database under `~/.cyberfox/state.db`. The database keeps:
...
Cyberfox immediately confirms the task and gives you back the prompt:
