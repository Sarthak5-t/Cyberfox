...
Cyberfox Agent can drive your desktop — clicking, typing, scrolling,
...
on macOS/Linux, `install.ps1` on Windows. Use `cyberfox computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `cyberfox tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Cyberfox app). `cyberfox computer-use doctor` will tell you which permission is missing. |
...
```
cyberfox -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.cyberfox/config.yaml`.

## `cyberfox computer-use doctor` — your first triage stop

`cyberfox computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ cyberfox computer-use doctor
...
agent is acting here." Each Cyberfox run declares its own cua-driver
**session id** (something like `cyberfox-3a7b9c14d2e8`); the cursor's
...
Cyberfox intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Cyberfox-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Cyberfox skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Cyberfox
...
Cyberfox applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.cyberfox/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Cyberfox applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Cyberfox agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Cyberfox agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Cyberfox warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Cyberfox from inside the RDP / console
...
```
CYBERFOX_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
CYBERFOX_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Cyberfox disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Cyberfox sets
...
When it's on, `cyberfox computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Cyberfox at a binary you built from source instead
of the published release. Cyberfox resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`CYBERFOX_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Cyberfox testing (see notes).
...
### Option B — point Cyberfox straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`CYBERFOX_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
CYBERFOX_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
CYBERFOX_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Cyberfox is using your build

- `cyberfox computer-use status` prints the resolved binary path and
  version.
- `cyberfox computer-use doctor` confirms the binary is reachable and
...
- **Cyberfox spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Cyberfox
  change needed either way.
- **Local builds skip the version check.** Cyberfox warns when the
...
**First action when anything's off: run `cyberfox computer-use doctor`.**
...
Run `cyberfox computer-use install` to fetch the cua-driver binary, or
run `cyberfox tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `cyberfox computer-use doctor` will
...
- **Cyberfox-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Cyberfox `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Cyberfox (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Cyberfox among them)
