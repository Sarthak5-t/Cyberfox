...
When Cyberfox Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.cyberfox/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.cyberfox/config.yaml` (switchable per session
via `/personality`).
...
If you use Cyberfox' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.cyberfox/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Cyberfox Agent's deliverable
