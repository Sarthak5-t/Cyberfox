...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Cyberfox's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Cyberfox instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
cyberfox memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$CYBERFOX_HOME/honcho.json` (profile-local). The setup wizard handles this for you.

### Self-Hosted Honcho with Authentication

When pointing Cyberfox at a self-hosted Honcho server, `cyberfox honcho setup` (and `cyberfox memory setup`) ask for a **local JWT / bearer token** after the base URL. Paste a JWT signed with the server's `AUTH_JWT_SECRET` (the Honcho compose env var) to enable authenticated access; leave it blank for servers running with `AUTH_USE_AUTH=false`. The local token is stored under the host block (`hosts.<host>.apiKey` in `honcho.json`), separate from any cloud `apiKey`, so you can flip the `Cloud or local?` prompt back to `cloud` later without losing either credential.
...
- `per-session` — each `cyberfox` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Cyberfox syncs them back at session init.
...
The `cyberfox honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `cyberfox memory setup honcho` (or run `cyberfox memory setup` and pick it from the list); the `cyberfox honcho` subcommand then appears on the next invocation.

```bash
cyberfox memory setup honcho    # Configure Honcho directly (works before activation)
cyberfox honcho status          # Connection status, config, and key settings
cyberfox honcho setup           # Redirects to `cyberfox memory setup` (post-activation alias)
cyberfox honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
cyberfox honcho peer            # Show or update peer names + dialectic reasoning level
cyberfox honcho mode            # Show or set recall mode (hybrid/context/tools)
cyberfox honcho tokens          # Show or set token budget for context and dialectic
cyberfox honcho identity        # Seed or show the AI peer's Honcho identity
cyberfox honcho sync            # Sync Honcho config to all existing profiles
cyberfox honcho peers           # Show peer identities across all profiles
cyberfox honcho sessions        # List known Honcho session mappings
cyberfox honcho map             # Map current directory to a Honcho session name
cyberfox honcho enable          # Enable Honcho for the active profile
cyberfox honcho disable         # Disable Honcho for the active profile
cyberfox honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `cyberfox honcho`

If you previously used the standalone `cyberfox honcho setup`:
...
No re-login or re-setup needed. Run `cyberfox memory setup` and select "honcho" — the wizard detects your existing config.
