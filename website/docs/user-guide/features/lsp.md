...
Cyberfox runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Cyberfox
...
1. Cyberfox captures a baseline of current diagnostics for the file.
...
…). Cyberfox auto-detects the binary on PATH or in
`<CYBERFOX_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Cyberfox at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<CYBERFOX_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`cyberfox lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Cyberfox installs both packages together when you
run `cyberfox lsp install typescript` or auto-install fires on first
use.

## CLI

```
cyberfox lsp status          # service state + per-server install status
cyberfox lsp list            # registry, optionally --installed-only
cyberfox lsp install <id>    # eagerly install one server
cyberfox lsp install-all     # try every server with a known recipe
cyberfox lsp restart         # tear down running clients
cyberfox lsp which <id>      # print resolved binary path
```

`cyberfox lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <CYBERFOX_HOME>/lsp/bin
...
When `install_strategy: auto`, Cyberfox installs binaries into
`<CYBERFOX_HOME>/lsp/bin/`. NPM packages land in
`<CYBERFOX_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Cyberfox-owned and is
...
Servers are kept alive for the life of the Cyberfox process. There's
...
**`cyberfox lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<CYBERFOX_HOME>/lsp/bin/`. Run
`cyberfox lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `cyberfox lsp status`**
...
When `cyberfox lsp status` shows a `Backend warnings` section, install
...
`~/.cyberfox/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.cyberfox/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `cyberfox lsp restart` to clear the set;
