...
description: "Customize Cyberfox Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Cyberfox Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `CYBERFOX_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Cyberfox is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Cyberfox now seeds a default `SOUL.md` automatically in:

```text
~/.cyberfox/SOUL.md
```

More precisely, it uses the current instance's `CYBERFOX_HOME`, so if you run Cyberfox with a custom home directory, it will use:

```text
$CYBERFOX_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Cyberfox creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Cyberfox loads `SOUL.md` only from `CYBERFOX_HOME`
- Cyberfox does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Cyberfox falls back to a built-in default identity
...
If Cyberfox loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `CYBERFOX_HOME`, the personality belongs to the Cyberfox instance itself.

That also makes it easier to teach users:
- "Edit `~/.cyberfox/SOUL.md` to change Cyberfox' default personality."
...
- how Cyberfox should handle uncertainty, disagreement, or ambiguity
...
## What Cyberfox injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Cyberfox falls back to a built-in default identity ("You are Cyberfox Agent, an intelligent AI assistant created by Sarthak Khatal..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Cyberfox ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Cyberfox, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Cyberfox its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.cyberfox/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.cyberfox/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-cyberfox)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Cyberfox speaks
- `display.skin` and `/skin` affect how Cyberfox looks in the terminal
