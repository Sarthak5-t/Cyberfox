...
Cyberfox can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<CYBERFOX_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Cyberfox renders the sprite at full fidelity when your
...
cyberfox pets list
cyberfox pets list cat

# Install a pet and make it active in one step
cyberfox pets install boba --select

# Preview / animate it in your terminal (Ctrl+C to stop)
cyberfox pets show

# Check your setup
cyberfox pets doctor
```

## `cyberfox pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `cyberfox pets list [query] [--limit N]` |
| List installed pets | `cyberfox pets list --installed` |
| Install a pet | `cyberfox pets install <slug> [--select] [--force]` |
| Set the active pet | `cyberfox pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `cyberfox pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `cyberfox pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `cyberfox pets off` |
| Remove an installed pet | `cyberfox pets remove <slug>` |
| Diagnose setup | `cyberfox pets doctor` |

`cyberfox pets show` flags:
...
Beyond installing pre-made pets from the gallery, Cyberfox can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `cyberfox pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Cyberfox state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](/user-guide/features/image-generation), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **the web dashboard**, **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**. OpenRouter/Nous run a quality-first model chain by default.

- Resolution order prefers the web dashboard → OpenAI → OpenRouter.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `cyberfox tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `CYBERFOX_PET_IMAGE_PROVIDER` env var (e.g. `CYBERFOX_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Cyberfox is
...
Run `cyberfox pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Cyberfox uses its own
  profile-scoped `<CYBERFOX_HOME>/pets/` instead — install through `cyberfox pets`.
