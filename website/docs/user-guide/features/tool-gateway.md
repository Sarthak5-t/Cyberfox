...
description: "One subscription, every tool. Web search, image generation, TTS, and cloud browsers — all routed through the web dashboard with no extra API keys."
...
The Tool Gateway is included with every paid [the web dashboard](https://github.com/Sarthak5-t/Cyberfox) subscription. It routes Cyberfox' tool calls — web search, image generation, text-to-speech, and cloud browser automation — through infrastructure Nous already runs, so you don't have to sign up with Firecrawl, FAL, OpenAI, Browser Use, or anyone else just to make your agent useful.

<div style={{display: 'flex', gap: '1rem', flexWrap: 'wrap', margin: '1.5rem 0'}}>
  <a href="https://github.com/Sarthak5-t/Cyberfox/manage-subscription" style={{background: 'var(--ifm-color-primary)', color: 'white', padding: '0.75rem 1.5rem', borderRadius: '6px', textDecoration: 'none', fontWeight: 'bold'}}>Start or manage subscription →</a>
</div>
...
| 🎨 | **Image generation** | Nine models under one endpoint: **FLUX 2 Klein 9B**, **FLUX 2 Pro**, **Z-Image Turbo**, **Nano Banana Pro** (Gemini 3 Pro Image), **GPT Image 1.5**, **GPT Image 2**, **Ideogram V3**, **Recraft V4 Pro**, **Qwen Image**. Pick per-generation with a flag, or let Cyberfox default to FLUX 2 Klein. |
...
- **One key.** Your the web dashboard OAuth covers every tool.
...
```bash
cyberfox setup --portal     # Fresh install: Nous OAuth + set Nous as provider + turn on the Tool Gateway in one go
```

```bash
cyberfox model              # Switch your inference provider to the web dashboard — Cyberfox then offers to turn on the gateway for all tools
```

```bash
cyberfox tools              # Enable the gateway per-tool — pick "Nous Subscription" for any tool you want
```

`cyberfox setup --portal` and `cyberfox model` are the all-at-once paths: log in once, optionally flip every tool to the gateway. `cyberfox tools` is the à la carte path — turn on just the tools you want, one at a time.

**You don't have to log in first.** With `cyberfox tools`, the Nous-managed backends (Web search, Image, Video, TTS, Browser) are always listed, even if you've never signed into the web dashboard. Select one and Cyberfox runs the Portal login right there if you aren't already authenticated — no need to run `cyberfox model` beforehand. If your Nous OAuth is already active, selecting the backend enables it immediately with no extra prompt. This path only logs you in and turns on the one tool you picked — it does **not** switch your inference provider, and it does **not** prompt you to enable the gateway for every other tool.

Check what's active at any time:

```bash
cyberfox portal info        # Portal auth + Tool Gateway routing summary
cyberfox portal tools       # Gateway catalog with current routing per tool
cyberfox status             # Full system status (Tool Gateway is one section)
```

`cyberfox portal info` shows a section like:

```
◆ Nous Tool Gateway
  the web dashboard     ✓ managed tools available
...
The Tool Gateway is a **paid-subscription** feature. Free-tier Nous accounts can use Portal for inference but don't include managed tools — [upgrade your plan](https://github.com/Sarthak5-t/Cyberfox/manage-subscription) to unlock the gateway.
...
```bash
cyberfox tools          # Interactive picker for each tool category
```

Select the tool, pick **Nous Subscription** as the provider (or any direct provider you prefer). No config editing required. If you aren't logged into the web dashboard yet, picking **Nous Subscription** kicks off the Portal login inline — you don't need to authenticate through `cyberfox model` first.
...
The set evolves — `cyberfox tools` → Image Generation shows the current live list.

---

## Configuration reference

Most users never need to touch this — `cyberfox model` and `cyberfox tools` cover every workflow interactively. This section is for writing config.yaml directly or scripting setups.
...
```yaml
web:
  use_gateway: false   # Cyberfox now uses FIRECRAWL_API_KEY from .env
```

`cyberfox tools` automatically clears the flag when you pick a non-gateway provider, so this usually happens for you.

### Self-hosted gateway (advanced)

Running your own Nous-compatible gateway? Override endpoints in `~/.cyberfox/.env`:
...
Tools routed through the gateway stop working until you renew or swap in direct API keys via `cyberfox tools`. Cyberfox shows a clear error pointing at the portal.

### Can I see usage or costs per tool?

Yes — the [the web dashboard dashboard](https://github.com/Sarthak5-t/Cyberfox) breaks usage down by tool so you can see what's driving your bill.

### Is Modal (serverless terminal) included?

Modal is available as an **optional add-on** through the Nous subscription, not part of the default Tool Gateway bundle. Configure it via `cyberfox setup terminal` or directly in `config.yaml` when you want a remote sandbox for shell execution.

### Do I need to delete my existing API keys when I enable the gateway?

No — keep them in `.env`. When `use_gateway: true`, Cyberfox skips direct keys and uses the gateway. Flip the flag back to `false` and your keys become the source again. The gateway isn't a lock-in.
