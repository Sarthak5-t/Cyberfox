...
**Tool Search** is Cyberfox' opt-in progressive-disclosure layer for that
...
:::info Built-in Cyberfox tools never defer
The tools that make up Cyberfox' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_CYBERFOX_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Cyberfox **unwraps the bridge** and
...
- **No JS sandbox.** Cyberfox uses the simpler "structured tools" mode
