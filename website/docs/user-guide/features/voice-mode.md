...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Cyberfox Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Cyberfox Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Cyberfox](/guides/use-voice-mode-with-cyberfox).

## Prerequisites

Before using voice features, make sure you have:

1. **Cyberfox Agent installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `cyberfox model` or set your preferred provider credentials in `~/.cyberfox/.env`
3. **A working base setup** — run `cyberfox` to verify the agent responds to text before enabling voice
...
cd ~/.cyberfox/cyberfox-agent && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.cyberfox/cyberfox-agent && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.cyberfox/cyberfox-agent && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.cyberfox/cyberfox-agent && uv pip install -e ".[all]"
```
...
Voice mode is available in both the **classic CLI** (`cyberfox chat`) and the **TUI** (`cyberfox --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.cyberfox/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
cyberfox                # Start the interactive CLI
```
...
1. Start the CLI with `cyberfox` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.cyberfox/config.yaml` (default: `ctrl+b`).
:::
...
```bash
cyberfox gateway        # Start the gateway (connects to configured platforms)
cyberfox gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@cyberfoxbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.cyberfox/.env`:
```bash
...
```bash
cyberfox gateway        # Start with existing configuration
```
...
If you are running Cyberfox inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.cyberfox/.env`
...
- Review gateway logs: `tail -f ~/.cyberfox/logs/gateway.log`
