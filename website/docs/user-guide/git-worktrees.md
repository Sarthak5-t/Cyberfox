...
title: "Git Worktrees"
description: "Run multiple Cyberfox agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Cyberfox Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Cyberfox so each session has a clean, isolated working directory.

## Why Use Worktrees with Cyberfox?

Cyberfox treats the **current working directory** as the project root:

- CLI: the directory where you run `cyberfox` or `cyberfox chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.cyberfox/config.yaml`
...
git worktree add ../repo-feature feature/cyberfox-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/cyberfox-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Cyberfox there:

```bash
cd ../repo-feature

# Start Cyberfox in the worktree
cyberfox
```
...
git worktree add ../repo-experiment-a feature/cyberfox-a
git worktree add ../repo-experiment-b feature/cyberfox-b
```
...
Each Cyberfox process:

- Works on its own branch (`feature/cyberfox-a` vs `feature/cyberfox-b`).
...
- Cyberfox checkpoint data under `~/.cyberfox/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Cyberfox experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/cyberfox-checkpoints-docs`, `feature/cyberfox-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Cyberfox from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `cyberfox -w` (Automatic Worktree Mode)

Cyberfox has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `cyberfox/cyberfox-<hash>`).
...
```bash
cyberfox -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `cyberfox -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Cyberfox session its own clean checkout.
