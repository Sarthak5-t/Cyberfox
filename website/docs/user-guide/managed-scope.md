...
`~/.cyberfox/config.yaml`, `~/.cyberfox/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/cyberfox`:

```text
/etc/cyberfox/
├── config.yaml     # managed config layer (wins over ~/.cyberfox/config.yaml)
└── .env            # managed env layer (wins over ~/.cyberfox/.env + shell)
```
...
The location can be relocated with the `CYBERFOX_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `CYBERFOX_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Cyberfox.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export CYBERFOX_MANAGED_DIR=/opt/org/cyberfox-policy
```

:::warning
A user who can set `CYBERFOX_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `cyberfox doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/cyberfox/config.yaml` (managed) | `/etc/cyberfox/.env` (managed) |
| 2 | `~/.cyberfox/config.yaml` (user) | `~/.cyberfox/.env` (user) |
...
```bash
cyberfox config        # shows a header naming the managed source + the pinned keys
cyberfox doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Cyberfox refuses and names the source:

```bash
$ cyberfox config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/cyberfox/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `cyberfox config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/cyberfox

# Pin some config values for every user on this machine
sudo tee /etc/cyberfox/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/cyberfox/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/cyberfox
sudo chmod 0644 /etc/cyberfox/config.yaml /etc/cyberfox/.env
```

Changes take effect on the next Cyberfox start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`cyberfox doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Cyberfox as `root`), managed scope is advisory.
