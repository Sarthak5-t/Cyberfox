...
sidebar_position: 12
title: "Google Chat"
description: "Set up Cyberfox Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Cyberfox Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Cyberfox
...
> Run `cyberfox gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Cyberfox can read (e.g.,
`~/.cyberfox/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a cyberfox restart)
...
- Principal: `cyberfox-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Cyberfox calls
...
- **App name**: whatever you want users to see ("Cyberfox" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/cyberfox-chat-events`.
...
`ADDED_TO_SPACE` event that Cyberfox uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Cyberfox

Add the Google Chat section to `~/.cyberfox/.env`:
...
Install the dependencies the Google Chat adapter needs (no Cyberfox extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Cyberfox is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Cyberfox detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Cyberfox session.
...
3. Download the JSON. Move it onto the host that runs Cyberfox.
4. Register the client with Cyberfox (run under the profile you want it scoped to):
...
cyberfox -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Cyberfox home (e.g.
`~/.cyberfox/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.cyberfox/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.cyberfox/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Cyberfox isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `cyberfox gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Cyberfox will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.cyberfox/google_chat_user_tokens/<sanitized_email>.json` (filesystem
