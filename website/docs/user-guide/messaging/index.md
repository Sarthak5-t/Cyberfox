...
description: "Chat with Cyberfox from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Cyberfox from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.
...
```mermaid
flowchart TB
    subgraph Gateway["Cyberfox Gateway"]
...
For group chats, hooks, and automation flows, Cyberfox supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Cyberfox keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Cyberfox does not hide failures just because the text resembles a silence token.
...
```bash
cyberfox gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
cyberfox gateway              # Run in foreground
cyberfox gateway setup        # Configure messaging platforms interactively
cyberfox gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo cyberfox gateway install --system   # Linux only: install a boot-time system service
cyberfox gateway start        # Start the default service
cyberfox gateway stop         # Stop the default service
cyberfox gateway status       # Check default service status
cyberfox gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Cyberfox Agent to the latest version |
...
with the `session_reset` section in `~/.cyberfox/config.yaml`:
...
Configure per-platform overrides in `~/.cyberfox/gateway.json`:
...
cyberfox pairing approve telegram XKGH5N7P

# Other pairing commands:
cyberfox pairing list          # View pending + approved users
cyberfox pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Cyberfox appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.cyberfox/config.yaml`:
...
Off by default. When enabled, Cyberfox prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.cyberfox/config.yaml`:
...
```bash
CYBERFOX_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
cyberfox gateway install               # Install as user service
cyberfox gateway start                 # Start the service
cyberfox gateway stop                  # Stop the service
cyberfox gateway status                # Check status
journalctl --user -u cyberfox-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo cyberfox gateway install --system
sudo cyberfox gateway start --system
sudo cyberfox gateway status --system
journalctl -u cyberfox-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Cyberfox installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit cyberfox-gateway` (or `sudo systemctl edit cyberfox-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `cyberfox update`. When `cyberfox update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart cyberfox-gateway` command (it never blocks on an interactive password prompt).
...
```bash
cyberfox gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `cyberfox update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo cyberfox update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/cyberfox-gateway`:

```
cyberfox ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed cyberfox-gateway*, /usr/bin/systemctl --no-ask-password start cyberfox-gateway*, /usr/bin/systemctl --no-ask-password restart cyberfox-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Cyberfox will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Cyberfox installations on the same machine (with different `CYBERFOX_HOME` directories), each gets its own systemd service name. The default `~/.cyberfox` uses `cyberfox-gateway`; other installations use `cyberfox-gateway-<hash>`. The `cyberfox gateway` commands automatically target the correct service for your current `CYBERFOX_HOME`.
:::

### macOS (launchd)

```bash
cyberfox gateway install               # Install as launchd agent
cyberfox gateway start                 # Start the service
cyberfox gateway stop                  # Stop the service
cyberfox gateway status                # Check status
tail -f ~/.cyberfox/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.cyberfox.gateway.plist`. It includes three environment variables:
...
- **CYBERFOX_HOME** — scopes the gateway to your Cyberfox installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `cyberfox gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `CYBERFOX_HOME` directory gets its own launchd label. The default `~/.cyberfox` uses `ai.cyberfox.gateway`; other installations use `ai.cyberfox.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `cyberfox-cli` | Full access |
| Telegram | `cyberfox-telegram` | Full tools including terminal |
| Discord | `cyberfox-discord` | Full tools including terminal |
| WhatsApp | `cyberfox-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `cyberfox-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `cyberfox-slack` | Full tools including terminal |
| Google Chat | `cyberfox-google_chat` | Full tools including terminal |
| Signal | `cyberfox-signal` | Full tools including terminal |
| SMS | `cyberfox-sms` | Full tools including terminal |
| Email | `cyberfox-email` | Full tools including terminal |
| Home Assistant | `cyberfox-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `cyberfox-mattermost` | Full tools including terminal |
| Matrix | `cyberfox-matrix` | Full tools including terminal |
| DingTalk | `cyberfox-dingtalk` | Full tools including terminal |
| Feishu/Lark | `cyberfox-feishu` | Full tools including terminal |
| WeCom | `cyberfox-wecom` | Full tools including terminal |
| WeCom Callback | `cyberfox-wecom-callback` | Full tools including terminal |
| Weixin | `cyberfox-weixin` | Full tools including terminal |
| BlueBubbles | `cyberfox-bluebubbles` | Full tools including terminal |
| QQBot | `cyberfox-qqbot` | Full tools including terminal |
| Yuanbao | `cyberfox-yuanbao` | Full tools including terminal |
| Microsoft Teams | `cyberfox-teams` | Full tools including terminal |
| API Server | `cyberfox-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `cyberfox-webhook` | Full tools including terminal |
| Raft | `cyberfox-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.cyberfox/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
