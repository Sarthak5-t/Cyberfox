...
Connect Cyberfox to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Cyberfox — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Cyberfox runs it in a
...
cyberfox photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Cyberfox Agent` project on your account.
...
Runtime credentials are written to `~/.cyberfox/.env`
...
(device token, dashboard project id) lives in `~/.cyberfox/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Cyberfox
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Cyberfox replies with a pairing code. Approve it with:

```bash
cyberfox pairing approve photon <CODE>
```

Use `cyberfox pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.cyberfox/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.cyberfox/.env`):
...
By default Cyberfox responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Cyberfox` and
`@Cyberfox agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Cyberfox will reply.

## Status & troubleshooting

```bash
cyberfox photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Cyberfox uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run cyberfox photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `cyberfox photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `cyberfox photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `cyberfox photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Cyberfox sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Cyberfox wake words  | JSON list / comma / newline regex patterns for group mentions |
