...
sidebar_position: 4
title: "Slack"
description: "Set up Cyberfox Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Cyberfox Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Cyberfox instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Cyberfox uses the modern
...
The fastest path is to paste a manifest Cyberfox generates for you. It
...
### Option A: From a Cyberfox-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   cyberfox slack manifest --write
   ```
   This writes `~/.cyberfox/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Cyberfox Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Cyberfox can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `cyberfox-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Cyberfox configuration issue.
:::
...
Cyberfox uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
cyberfox gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
cyberfox gateway              # Foreground
cyberfox gateway install      # Install as a user service
sudo cyberfox gateway install --system   # Linux only: boot-time system service
```
...
Every Cyberfox command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Cyberfox command with its description.

Under the hood: Cyberfox ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/cyberfox-agent/blob/main/cyberfox_cli/commands.py)
...
When Cyberfox adds new commands (e.g. after `cyberfox update`), regenerate
...
3. Paste the new contents of `~/.cyberfox/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/cyberfox <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/cyberfox btw run the tests` — Cyberfox routes it the same way as `/btw
run the tests`. Free-form questions also work: `/cyberfox what's the
...
Slack never delivers them to Cyberfox.

As a workaround, Cyberfox recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Cyberfox treats it identically to the slash form and replies in the same
thread.
...
Cyberfox falls back to a text prompt, the prompt instructs you to reply
...
```bash
cyberfox slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Cyberfox behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Cyberfox Agent what time is it?`). In channels, Cyberfox replies in a thread attached to that message. |
| **Threads** | If you @mention Cyberfox inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.cyberfox/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Cyberfox in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Cyberfox will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Cyberfox will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Cyberfox Agent`).

---

## Multi-Workspace Support

Cyberfox can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Cyberfox also loads tokens from an **OAuth token file** at:

```
~/.cyberfox/slack_tokens.json
```
...
- When a message arrives, Cyberfox uses the correct workspace-specific client to respond.
...
Cyberfox supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Cyberfox Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Cyberfox Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Cyberfox now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Cyberfox Agent`)
...
- Tokens should be stored in `~/.cyberfox/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Cyberfox config directory
