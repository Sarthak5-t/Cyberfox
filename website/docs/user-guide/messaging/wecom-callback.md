...
Connect Cyberfox to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Cyberfox supports two WeCom integration modes:
...
> Run `cyberfox gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Cyberfox decrypts the message, queues it for the agent
...
(Use `cyberfox gateway start` only after `cyberfox gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Cyberfox and the token the
...
in `~/.cyberfox/.env` values around `=` will also break signature checks. After
fixing, restart `cyberfox gateway run`.
...
Check `cyberfox gateway run` logs for the bound host/port. If the adapter bound to
