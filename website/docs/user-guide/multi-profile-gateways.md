...
If you only run one Cyberfox agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Cyberfox agents that should all
...
(`ai.cyberfox.gateway-<name>.plist`) or systemd user service
(`cyberfox-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
cyberfox profile create coder
cyberfox profile create personal-bot
cyberfox profile create research
...
```bash
cyberfox config set gateway.multiplex_profiles true
cyberfox gateway restart
```

Equivalently, in the default profile's `~/.cyberfox/config.yaml`:
...
You do **not** run `cyberfox gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `cyberfox gateway start` / `run` is a
...
home). `cyberfox status` reports the multiplexer and the profiles it serves;
`cyberfox status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/cyberfox-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: cyberfox-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    cyberfox gateway "$action"
  else
    cyberfox -p "$profile" gateway "$action"
  fi
}
...
```bash
cyberfox-gateways start      # start every configured profile
cyberfox-gateways stop       # stop every configured profile
cyberfox-gateways restart    # restart all
cyberfox-gateways status     # status across all
cyberfox-gateways list       # delegates to `cyberfox gateway list`
```

:::tip
The `default` profile is targeted with `cyberfox gateway <action>` (no `-p`),
not `cyberfox -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `cyberfox -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.cyberfox.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/cyberfox-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.cyberfox.gateway.plist` /
`cyberfox-gateway.service`.
...
```bash
# Default profile
tail -f ~/.cyberfox/logs/gateway.log
tail -f ~/.cyberfox/logs/gateway.error.log

# Named profile
tail -f ~/.cyberfox/profiles/<name>/logs/gateway.log
tail -f ~/.cyberfox/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.cyberfox/logs/gateway.log ~/.cyberfox/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
cyberfox logs -f                  # follow default profile
cyberfox -p coder logs -f         # follow one profile
cyberfox logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
cyberfox profile list             # profiles + model + gateway state
cyberfox-gateways status          # full status across every profile
launchctl list | grep cyberfox    # macOS — PIDs and labels
systemctl --user list-units 'cyberfox-gateway-*'   # Linux — units
```
...
```
~/.cyberfox/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.cyberfox/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
cyberfox config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.cyberfox/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=cyberfox --why="gateways running" \
  sleep infinity &
...
`cyberfox-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.cyberfox/.env ~/.cyberfox/profiles/*/.env
```

## Updating the code

`cyberfox update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `cyberfox gateway start` after a previous `cyberfox gateway stop`. The
...
```bash
ps -ef | grep "cyberfox_cli.*-p <profile>"
cat ~/.cyberfox/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.cyberfox.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.cyberfox.gateway-<profile>.plist

# Linux
systemctl --user restart cyberfox-gateway-<profile>.service
```

### Health check

```bash
cyberfox doctor                  # default profile
cyberfox -p <profile> doctor     # one profile
```
