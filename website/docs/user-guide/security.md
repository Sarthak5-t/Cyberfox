...
Cyberfox Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Cyberfox checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.cyberfox/config.yaml`:
...
| `timeout` | `60` | Seconds Cyberfox waits for an approval reply before timing out. |
...
1. **CLI flag**: Start a session with `cyberfox --yolo` or `cyberfox chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `CYBERFOX_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `CYBERFOX_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Cyberfox shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Cyberfox refuses to run them **regardless** of:
...
Configure the timeout in `~/.cyberfox/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.cyberfox/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.cyberfox/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` cyberfox/gateway | Self-termination prevention |
...
The `CYBERFOX_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.cyberfox/config.yaml`:
...
:::tip
Use `cyberfox config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Cyberfox controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.cyberfox/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.cyberfox/.env to allow open access,
...
For more flexible authorization, Cyberfox includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `cyberfox pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.cyberfox/config.yaml`:
...
cyberfox pairing list

# Approve a pairing code
cyberfox pairing approve telegram ABC12DEF

# Revoke a user's access
cyberfox pairing revoke telegram 123456789

# Clear all pending codes
cyberfox pairing clear-pending
```

:::tip Docker users: run pairing commands as the `cyberfox` user
The official Docker image runs the gateway as the unprivileged `cyberfox` user
...
Always pass `-u cyberfox`:

```bash
docker exec -u cyberfox cyberfox-agent cyberfox pairing approve telegram ABC12DEF
```
...
When using the `docker` terminal backend, Cyberfox applies strict security hardening to every container.
...
Container resources are configurable in `~/.cyberfox/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.cyberfox/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `CYBERFOX_HOME`. Skills declare these in frontmatter:
...
When loaded, Cyberfox checks if these files exist in the active profile's `CYBERFOX_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Cyberfox infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Cyberfox infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Cyberfox integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Cyberfox under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.cyberfox/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.cyberfox/logs/` for unauthorized access attempts
10. **Keep updated** — run `cyberfox update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.cyberfox/.env`:
...
TERMINAL_SSH_USER=cyberfox
TERMINAL_SSH_KEY=~/.ssh/cyberfox_agent_key
```
...
Cyberfox ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `cyberfox_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `cyberfox doctor` for the full remediation.
- **`cyberfox doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
```bash
cyberfox doctor --ack <advisory-id>
```
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Cyberfox installs these **lazily** on first use rather than eagerly under `cyberfox-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `cyberfox tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `cyberfox tools`.
