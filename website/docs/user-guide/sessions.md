...
Cyberfox Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.cyberfox/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Cyberfox stores session history so it can resume conversations, but it does not
...
Cyberfox explicitly injects for that turn.
...
For example, if a user sends an image and asks Cyberfox to make a meme from it,
Cyberfox may inspect that image once with vision and run an image-processing
...
`cyberfox sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`cyberfox` or `cyberfox chat`) |
...
cyberfox -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
cyberfox -c "my project"   # → resumes "my project #3"
```
...
cyberfox --resume 20250305_091523_a1b2c3d4
cyberfox -r 20250305_091523_a1b2c3d4

# Resume by title
cyberfox --resume "refactoring auth"

# Or with the chat subcommand
cyberfox chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `cyberfox sessions list`.

### Conversation Recap on Resume

When you resume a session, Cyberfox displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Cyberfox session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.cyberfox/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `cyberfox -r "<title>"` from the shell) and pick up where the platform left off.
...
Cyberfox automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `cyberfox sessions list` or `cyberfox sessions browse`.
...
```bash
cyberfox sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Cyberfox creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`cyberfox -c "my project"`), it automatically picks the most recent session in the lineage.
...
Cyberfox provides a full set of session management commands via `cyberfox sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
cyberfox sessions list

# Filter by platform
cyberfox sessions list --source telegram

# Show more sessions
cyberfox sessions list --limit 50
```
...
cyberfox sessions export backup.jsonl

# Export sessions from a specific platform
cyberfox sessions export telegram-history.jsonl --source telegram

# Export a single session
cyberfox sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
cyberfox sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
cyberfox sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
cyberfox sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
cyberfox sessions prune

# Custom age threshold — bare numbers are days
cyberfox sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
cyberfox sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
cyberfox sessions prune --newer-than 5h

# Explicit window with absolute timestamps
cyberfox sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
cyberfox sessions prune --source telegram
cyberfox sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
cyberfox sessions prune --newer-than 5h --title "smoke test"   # title substring
cyberfox sessions prune --older-than 30 --max-messages 3        # tiny sessions
cyberfox sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
cyberfox sessions prune --model gpt-5 --older-than 1w           # by model (substring)
cyberfox sessions prune --provider openrouter --older-than 60   # by billing provider
cyberfox sessions prune --branch feature/old-experiment         # by git branch
cyberfox sessions prune --user 12345678 --chat-type group       # by messaging origin
cyberfox sessions prune --max-tokens 500 --older-than 7         # by token usage
cyberfox sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
cyberfox sessions prune --newer-than 5h --dry-run

# Skip confirmation
cyberfox sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `cyberfox sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `cyberfox sessions prune` keeps the 90-day cutoff. Every
...
`cyberfox sessions archive` takes the same filters as `prune` but soft-hides
...
cyberfox sessions archive --newer-than 5h

# Archive by title substring, preview first
cyberfox sessions archive --title "dry run" --dry-run
cyberfox sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `cyberfox sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`cyberfox sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
cyberfox sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`cyberfox insights`](/reference/cli-commands#cyberfox-insights).
...
When Cyberfox cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Cyberfox uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Cyberfox in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.cyberfox/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.cyberfox/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.cyberfox/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the gateway's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.cyberfox/sessions/sessions.json` is the **gateway routing index** — it maps
...
`cyberfox sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.cyberfox/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `cyberfox sessions list`, the cause is
`state.db` not receiving them — run `cyberfox sessions repair` and watch for a
...
`*.jsonl` files in `~/.cyberfox/sessions/`. They are no longer written or
read by Cyberfox. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Cyberfox process in the same `CYBERFOX_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.cyberfox/config.yaml`:
...
cyberfox sessions delete <session_id>
