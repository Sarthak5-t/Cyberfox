...
- Give it a name like "cyberfox-agent"
- Select scopes:
...
- Give it a title like "cyberfox-agent-&lt;machine-name>"
...
elif _cyberfox_env="${CYBERFOX_HOME:-$HOME/.cyberfox}/.env"; [ -f "$_cyberfox_env" ] && grep -q "^GITHUB_TOKEN=" "$_cyberfox_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_cyberfox_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
