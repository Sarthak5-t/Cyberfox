...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _cyberfox_env="${CYBERFOX_HOME:-$HOME/.cyberfox}/.env"; [ -f "$_cyberfox_env" ] && grep -q "^GITHUB_TOKEN=" "$_cyberfox_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_cyberfox_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Cyberfox Agent\",
    \"comments\": [
...
```bash
source "${CYBERFOX_HOME:-$HOME/.cyberfox}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Cyberfox Agent. Code looks clean — good test coverage, no security concerns."
