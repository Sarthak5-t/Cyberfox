...
description: "Configure and use Honcho memory with Cyberfox -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Cyberfox -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `cyberfox skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Cyberfox

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Cyberfox profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/cyberfox#running-honcho-locally-with-cyberfox

### Verify

```bash
cyberfox honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Cyberfox profile's AI peer
...
Honcho models conversations as interactions between **peers**. Cyberfox creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Cyberfox instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Cyberfox run |
| `global` | Single session across all directories |

Manual override: `cyberfox honcho map my-project-name`
...
Each Cyberfox profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block cyberfox.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `cyberfox.coder` host block in `honcho.json`
...
```bash
cyberfox honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Cyberfox when Honcho memory is active.
...
Config file: `$CYBERFOX_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `cyberfox honcho setup`. Ensure `memory.provider: honcho` is in `~/.cyberfox/config.yaml`.

### Memory not persisting across sessions
Check `cyberfox honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `cyberfox profile create <name> --clone`. For existing profiles: `cyberfox honcho sync`.
...
| Command | Description |
|---------|-------------|
| `cyberfox honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `cyberfox honcho status` | Show resolved config, connection test, peer info for active profile |
| `cyberfox honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `cyberfox honcho disable` | Disable Honcho for the active profile |
| `cyberfox honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `cyberfox honcho peers` | Show peer identities across all profiles |
| `cyberfox honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `cyberfox honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `cyberfox honcho sessions` | List known directory-to-session-name mappings |
| `cyberfox honcho map <name>` | Map current working directory to a Honcho session name |
| `cyberfox honcho identity` | Seed AI peer identity or show both peer representations |
| `cyberfox honcho sync` | Create host blocks for all Cyberfox profiles that don't have one yet |
| `cyberfox honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Cyberfox + Honcho |
| `cyberfox memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `cyberfox memory status` | Show active memory provider and config |
| `cyberfox memory off` | Disable external memory provider |
