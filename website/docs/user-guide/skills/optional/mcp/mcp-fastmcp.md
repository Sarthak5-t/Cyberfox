...
| | |
|---|---|
| Source | Optional — install with `cyberfox skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Cyberfox or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Cyberfox. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.cyberfox/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.cyberfox/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Cyberfox integration, either:

- configure the server in `~/.cyberfox/config.yaml` using the `native-mcp` skill, or
...
### Cyberfox cannot see the deployed server

The server-building part may be correct while the Cyberfox config is not. Load the `native-mcp` skill and configure the server in `~/.cyberfox/config.yaml`, then restart Cyberfox.
