...
| | |
|---|---|
| Source | Optional — install with `cyberfox skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Cyberfox Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${CYBERFOX_HOME:-~/.cyberfox}/.env`:
...
```bash
CANVAS="python $CYBERFOX_HOME/skills/productivity/canvas/scripts/canvas_api.py"
