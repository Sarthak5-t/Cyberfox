...
| | |
|---|---|
| Source | Optional — install with `cyberfox skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${CYBERFOX_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${CYBERFOX_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${CYBERFOX_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client cyberfox
```
...
```bash
PUBLISH="${CYBERFOX_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client cyberfox
```
...
```bash
DRIVE="${CYBERFOX_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `cyberfox`)    |
