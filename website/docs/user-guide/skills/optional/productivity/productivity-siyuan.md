...
| | |
|---|---|
| Source | Optional — install with `cyberfox skills install official/productivity/siyuan` |
...
3. Store it in `${CYBERFOX_HOME:-~/.cyberfox}/.env`:
   ```
...
```yaml
# In ~/.cyberfox/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
