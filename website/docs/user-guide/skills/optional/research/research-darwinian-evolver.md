...
| | |
|---|---|
| Source | Optional — install with `cyberfox skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Cyberfox Agent |
| License | MIT |
...
import upstream classes into Cyberfox itself.
...
```bash
mkdir -p ~/.cyberfox/cache/darwinian-evolver && cd ~/.cyberfox/cache/darwinian-evolver
...
```bash
cd ~/.cyberfox/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.cyberfox/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.cyberfox/skills/research/darwinian-evolver
DE_DIR=~/.cyberfox/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Cyberfox core.
   Custom driver scripts under `~/.cyberfox/skills/...` are user-side and fine.
