...
| | |
|---|---|
| Source | Optional — install with `cyberfox skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Cyberfox Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Cyberfox terminal calls (desktop app flow only)
...
### Service Account (recommended for Cyberfox)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${CYBERFOX_HOME:-~/.cyberfox}/.env` (the skill will prompt for this on first load).
...
## Cyberfox Execution Pattern (desktop app flow)

Cyberfox terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/cyberfox-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/cyberfox-op.sock"
