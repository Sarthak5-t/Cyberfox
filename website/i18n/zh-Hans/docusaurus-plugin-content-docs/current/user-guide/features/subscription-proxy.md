...
这会打开浏览器进行 Nous Portal OAuth 流程。Cyberfox 将刷新令牌存储在 `~/.cyberfox/auth.json` 中——与所有 Cyberfox 提供商登录信息存放在同一位置。
...
当前已内置：`nous`（Nous Portal）。更多 OAuth 提供商可通过在 `cyberfox_cli/proxy/adapters/` 中实现 `UpstreamAdapter` 接口来添加。
