...
运行 `cyberfox model` 并选择 Nous Portal 作为提供商时，Cyberfox 会主动询问是否启用 Tool Gateway：
...
网关使用 Nous Portal 凭据认证（在 `cyberfox model` 完成后写入 `~/.cyberfox/auth.json`）。
...
经网关路由的工具会停止工作，直到你 [续订](https://portal.nousresearch.com/manage-subscription) 或通过 `cyberfox tools` 改回直连 Key。
