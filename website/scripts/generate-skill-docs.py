...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/cyberfox-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    cyberfox_meta = (fm.get("metadata") or {}).get("cyberfox") or {}
    tags = cyberfox_meta.get("tags") or []
    related = cyberfox_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `cyberfox skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Cyberfox Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Cyberfox ships with a large built-in skill library copied into `~/.cyberfox/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Cyberfox also syncs bundled skills on `cyberfox update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.cyberfox/skills/` tree, it is still shipped with Cyberfox; restore it with `cyberfox skills reset <name> --restore`.",
        "",
